...
一種參與度量，根據指定給網站上瀏覽頁面的值，從訪客第一眼看到活動的第一個顯示mbox開始計算匯總分數。
...
記錄測試或活動結果，例如註冊、訂單或銷售。通常來說，轉換 mbox 不會顯示任何內容而僅負責監聽。轉換 mbox 一般會位於註冊完成網頁、感謝訂購網頁，或是點進連結。您也可以使用全域 mbox 進行轉換計量。
...
可以屬於任何類型的測試或活動類型。無須額外進行程式設計。系統會保留您網域上的訪客作業。

**動態 mbox**

使用DHTML和AJAX等技術，讓Rich Internet Applications(RIA)在頁面載入後操控HTML。
...
訪客對網站感興趣的度量，依網站逗留時間、點按次數、轉換和其他參與度量測量。
...
著陸頁面活動可讓您使用定位來顯示不同瀏覽的不同著陸頁面內容。否則，著陸頁面會為每位訪客顯示相同的內容。著陸頁面活動會比較不同版本的頁面，以協助您查看哪個著陸頁面版本產生更成功的結果。在 Target Standard 中，已由*體驗鎖定目標*取代。
...
在頁面中而不是在 URL 中傳遞。參數可用來從資料庫傳遞資料，以及追蹤銷售資料或執行Adobe Recommendations。又叫做頁面參數。
...
選件是活動和測試期間mbox中顯示的內容。體驗決定當符合特殊條件時所顯示的選件。
...
一種參與度量，從訪客第一眼看到活動的第一個顯示mbox開始，測量在瀏覽中檢視的頁數。
...
將值傳遞給目的地頁面的 mbox，作為轉介 URL 參數和值。例如，使用這些項目定位訪客的來源。
...
報表提供有關活動的進度和結果的資訊，可協助您根據資料進行決策。報表資料可幫助您決定何時結束測試、向您顯示哪個體驗或選件是贏家，並提供您所需的深入資訊來決定下一個動作。

## S {#section_9482B06EBB09486784492F838289E54D}

**指令碼描述檔參數**

類似in-mbox描述檔參數，但以JavaScript定義，可在Target後端執行，而非在瀏覽器中執行。

**區段**

用於定位活動的指定標準集。只有符合標準的訪客才會看到定位至該群體之活動的內容。有些區段可重復用於多個活動，有些則用於活動。
...
用來測量活動成功的參數。成功度量包含關鍵業務度量，可讓您判斷特定體驗或選件在Target活動中是否成功。例如，您可以判斷新選件或新增項目至購物車是否增加了每位訪客的收入。成功度量有助於找出註冊、訂購或購買漏斗的問題，但同時也只是訪客或客戶參與的問題。

## T {#section_1841144841AF41169AF1E83B90C18292}

**test**

比較兩個或更多體驗與您指定之成功度量的活動，讓您可以選擇最有望提供所需結果的體驗。

**網站逗留時間**

一種參與類型，代表瀏覽中逗留時間(以秒為單位)，從訪客看到活動的第一個顯示mbox到載入mbox的作業中最後一頁載入。

## U {#section_6407B81326FF43049D92265746E6C402}

**URL 參數**

當具有 mbox 的頁面經任何訪客瀏覽時，所有反向連結頁面和頁面 URL 變數都會自動傳遞到 mbox。然後這些URL變數可透過顯示目標或報表中的群體篩選來使用。

## V {#section_0B85CE959BA948CDAC3BAEFB4EEDFAD2}

**訪客**

訪客是存取您的網站的任何人。對活動條件評估訪客，以判斷訪客是否包含在活動中。請參閱 *加入者*。

**可視化體驗撰寫器**

Target Standard是一種WYSIWYG使用者介面，可讓您輕鬆建立並測試網站內容中的個人化體驗和選件。您可以借由拖放、交換及修改網頁的版面和內容，建立適用於Target活動的體驗和選件。
