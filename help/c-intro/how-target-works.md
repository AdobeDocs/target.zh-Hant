---
keywords: Adobe Experience Platform Web SDK;aep web sdk;aep sdk；搜尋引擎最佳化；搜尋引擎最佳化；seo;edge叢集， central clusters;at.js;mbox.js;
...
活動和內容資料庫、[!DNL Analytics]資料、API和行銷人員使用者介面都存放在Adobe的Central Clusters中。 然後，更新會傳送至[!DNL Target] Edges。 Central叢集和Edge叢集會自動同步，以持續更新快取的活動資料。 所有1:1模型也會儲存在每個邊緣，因此這些更複雜的請求也可以在邊緣上處理。
...
[!DNL Target]解決方案代管於全球Adobe擁有和Adobe租賃的資料中心。
...
[!DNL Target] Central Clusters，代管於Amazon Web Services(AWS)，包括：
...
Adobe 盡可能確保定位基礎建設的可用度和效能值得信賴。不過，訪客瀏覽器與Adobe伺服器之間的通訊中斷可能會造成內容傳送中斷。
...
* **只要有必要，就能執行實驗**:Adobe認為「只要有必要」，就能達到統計意義。[!DNL Target][ 提供最佳作法](https://docs.adobe.com/content/target-microsite/testcalculator.html)來判斷測試何時達到此點。Adobe建議您將成功測試的硬式編碼實作整合到測試工作流程中，並分配適當的資源。
...
Adobe [!DNL Target]使用[DeviceAtlas](https://deviceatlas.com/device-data/user-agent-tester/)量度「isRobot」，根據「請求標題」中傳遞的使用者代理字串來偵測已知機器人。
