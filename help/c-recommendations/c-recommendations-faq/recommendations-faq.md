...
在查詢字串中，您可以傳遞要從建議中排除之實體的實體ID。例如，您可以排除已在購物車中的項目。

若要啟用排除功能，請使用 `excludedIds` mbox 參數。此參數指向逗號分隔實體ID的清單。例如, `mboxCreate(..., "excludedIds=1,2,3,4,5")`。要求建議時會傳送值。

>[!NOTE]
>
>如果排除太多實體，建議會像沒有足夠實體來填滿建議範本一樣運作。

若要排除 `entityIds`，請附加 `&excludes=${mbox.excludedIds}` Token至選件內容URL。擷取內容 URL 時，目前的 mbox 請求參數會替代必要參數。

依預設，新建立的建議會啓用此功能。必須儲存現有建議，以支援「動態排除」實體。
