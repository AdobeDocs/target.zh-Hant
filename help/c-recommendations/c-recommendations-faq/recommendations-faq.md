...
## 目錄中項目的更新需要多久才會反映在我的網站上？

匯入動態消息檔案，或透過API或mbox接收實體更新後，下列變更將會反映在60分鐘以下：

* 在「設計」範本中傳回的項目屬性。
* 全域排除規則中使用的項目屬性，可防止項目納入傳回建議中。
* 包含規則中用於包含規則的項目屬性，其影響項目是否已包含或排除在傳回建議中。

下一個演算法執行發生(在12-24小時內)時，才會反映下列變更：
...
>[!NOTE]
>
>當饋送檔案從「匯入項目」變更為「準備搜尋索引更新」時，會將其視為匯入檔案。更新可能需要超過60分鐘才能反映在目錄搜尋使用者介面中；動態消息狀態變更為「已完成的更新」時，「目錄搜尋」就會更新。即使目錄搜尋尚未更新，您的網站仍會反映上述時間範圍的更新。最新目錄搜尋索引更新時間會顯示在「目錄搜尋」頁面上。
...
若要排除 `entityIds`，請將 `&excludes=${mbox.excludedIds}` Token 附加至選件內容 URL。擷取內容 URL 時，目前的 mbox 請求參數會替代必要參數。
