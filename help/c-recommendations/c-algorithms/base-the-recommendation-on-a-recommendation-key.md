...
description: 了解如何根據使用訪客行為上下文的索引鍵來使用建議，以在Adobe [!DNL Target] Recommendations活動中顯示相關結果。
title: 如何讓建議以建議金鑰為依據？
...
基於演算法的Recommendations會使用訪客行為上下文來顯示[!DNL Adobe Target] [!DNL Recommendations]活動中的相關結果。

[!DNL Target Recommendations]中有四種演算法類型：
...
>[!NOTE]
>
>下表說明[!UICONTROL Cart-Based]類型，且即將推出。
...
如需建立條件及定義其演算法類型和演算法的詳細資訊，請參閱[建立條件](/help/c-recommendations/c-algorithms/create-new-algorithm.md)。

不同的建議演算法適用於放置在不同類型的頁面上。 請參閱下列章節，以取得關於每個演算法類型及其可用演算法的詳細資訊。

## [!UICONTROL 人氣]

[!UICONTROL 以人氣為基礎的]演算法類型可讓您根據網站上某個項目的整體人氣，或根據使用者最喜愛或最常檢視的類別、品牌、類型等內項目的人氣，提出建議。

下列演算法適用於[!UICONTROL Processy-Based]演算法類型：
...
[!UICONTROL Item-Based]建議類型可讓您根據找到與使用者目前正在檢視或最近已檢視的項目類似的項目來提出建議。

下列演算法適用於[!UICONTROL Item-Based]演算法類型：
...
下列演算法適用於[!UICONTROL 使用者型]演算法類型：

### 最近查看的項目 {#recently-viewed}

根據設計中的位置數量，使用訪客的歷史 (跨工作階段) 以呈現訪客已檢視的前 *x* 個項目。

最近查看的項目演算法返回指定[環境](/help/administrating-target/hosts.md)的特定結果。 如果兩個網站分屬於不同環境，當訪客在這兩個網站之間進行切換時，每個網站都只會顯示適用網站最近檢視過的項目。如果兩個網站位於相同環境中，當訪客在這兩個網站之間進行切換時，訪客會看到兩個網站最近檢視的相同項目。

>[!NOTE]
>
>備份建議不能使用[!UICONTROL 最近查看的項目]條件。
...
>[!NOTE]
>
>[!UICONTROL 「最近查] 看的項目」會遵循排除全域設定和活動的選取收集設定。如果全域排除排除了某個項目，或選取的集合中未包含該項目，則不會顯示該項目。因此，使用[!UICONTROL 最近查看的項目]條件時，通常應使用「所有集合」設定。
...
[!UICONTROL 建議索引鍵]下拉式清單提供下列建議索引鍵：
...
在您的網站上使用[!UICONTROL 上次購買的項目]建議索引鍵：
...
在您的網站上使用[!UICONTROL 上次檢視的項目]建議索引鍵：
