...
[!UICONTROL Adobe Target] [!UICONTROL Recommendations]中的條件可控制[!UICONTROL Recommendations]活動的內容。 建立條件以顯示最適合您的活動的建議。這些條件會使用訪客的動作來決定要顯示的內容或產品。
...
* 使用[!UICONTROL 可視化體驗撰寫器](VEC)建立[!DNL Recommendations]活動時，在您頁面上選取元素並按一下[!UICONTROL 取代w/ Recommendations]、 [!UICONTROL 插入Recommendations在]或[!UICONTROL 插入Recommendations在]之後，系統會立即帶您進入[!UICONTROL 選取條件]畫面。 然後，您可以選擇可用的條件，或按一下「建立條件」****。 如果您建立新條件，則可以選擇儲存條件以搭配其他[!DNL Recommendations]活動使用。 如需詳細資訊，請參閱[建立Recommendations活動](/help/c-recommendations/t-create-recs-activity/create-recs-activity.md)。
* 編輯 [!DNL Recommendations] 活動時，請在頁面上的[!UICONTROL 「Recommendations 位置」]方塊中按一下，然後選取&#x200B;**[!UICONTROL 「變更條件」]**。在[!UICONTROL 選擇條件]螢幕上，按一下&#x200B;**[!UICONTROL 建立條件]**。 您將可以選擇儲存您的新條件以搭配其他 [!DNL Recommendations] 活動使用。
...
1. 選取&#x200B;**[!UICONTROL 演算法類型]**&#x200B;和&#x200B;**[!UICONTROL 演算法]**:
   ...
   >[!NOTE]
   >
   >如果您選取「**[!UICONTROL 項目]**/ **[!UICONTROL 具有類似屬性的媒體]**」，您便可以選擇設定[內容相似度規則](#similarity)。

1. 視需要選取&#x200B;**項目屬性**&#x200B;和&#x200B;**要比對的設定檔屬性**、**建議索引鍵**、**篩選索引鍵**&#x200B;及/或&#x200B;**Analytics度量**，以設定演算法。

如需有關選擇建議索引鍵的詳細資訊，請參閱[讓建議以建議索引鍵](/help/c-recommendations/c-algorithms/base-the-recommendation-on-a-recommendation-key.md)為依據。

## [!UICONTROL 資料來源] {#data-source}

1. 選擇所需的&#x200B;**[!UICONTROL 行為資料源]**:[!UICONTROL Adobe Target]或[!UICONTROL Analytics]。

   >[!NOTE]
   >
   >只有在您的實作使用[Analytics for Target](/help/c-integrating-target-with-mac/a4t/a4t.md)(A4T)時，才會顯示[!UICONTROL 行為資料來源]區段。

   ![行為資料來源區段](assets/data-source.png)

   如果您選擇 [!UICONTROL Analytics]，請選取需要的報表套裝。

   如果條件使用[!DNL Adobe Analytics]作為行為資料來源，建立後，條件可用時間會根據其他條件是否已使用選取的報表套裝和回顧期間而定，如下所述：
...
1. 設定&#x200B;**[!UICONTROL 回顧期間]**，以決定要顯示哪些建議時，要使用的可用歷史使用者行為資料的時間範圍。 此選項適用於所有演算法，但具有類似屬性的項目和自訂演算法除外。
   ...
   | --- | --- | --- |
   | 六小時 | 演算法每3到6小時執行一次 | [!UICONTROL 所選行] 為資料來源時的以人氣為 [!UICONTROL 基礎的] 演算法  [!DNL Adobe Target] |
   | 一天 | 演算法每12到24小時執行一次 | [!UICONTROL 基於人氣] 的演算法 |
   | 兩天 | 演算法每12到24小時執行一次 | <ul><li>[!UICONTROL 基於人氣] 的演算法</li><li>[!UICONTROL 項目型] 演算法</li><li>[!UICONTROL 使用者] 型演算法</li><li>[!UICONTROL 購物車] 型演算法</li></ul> |
   | 一週 | 演算法每24到48小時執行一次 | <ul><li>[!UICONTROL 基於人氣] 的演算法</li><li>[!UICONTROL 項目型] 演算法</li><li>[!UICONTROL 使用者] 型演算法</li><li>[!UICONTROL 購物車] 型演算法</li></ul> |
   | 兩週 | 演算法每24到48小時執行一次 | <ul><li>[!UICONTROL 基於人氣] 的演算法</li><li>[!UICONTROL 項目型] 演算法</li><li>所有[!UICONTROL 使用者型]演算法</li><li>[!UICONTROL 購物車] 型演算法</li></ul> |
   | 一個月（30天） | 演算法每24到48小時執行一次 | <ul><li>[!UICONTROL 基於人氣] 的演算法</li><li>[!UICONTROL 項目型] 演算法</li><li>[!UICONTROL 使用者] 型演算法</li><li>[!UICONTROL 購物車] 型演算法</li></ul> |
   | 兩個月（61天） | 演算法每24到48小時執行一次 | <ul><li>[!UICONTROL 基於人氣] 的演算法</li><li>[!UICONTROL 項目型] 演算法</li><li>[!UICONTROL 使用者] 型演算法</li><li>[!UICONTROL 購物車] 型演算法</li></ul> |

## [!UICONTROL 備份內容] {#content}

[!UICONTROL 備] 份內容規則會決定如果建議的項目數未填入建議設計，會發生什麼 [情況](/help/c-recommendations/c-design-overview/design-overview.md)。[!DNL Recommendations] 條件可能傳回較您的設計所需更少的建議。例如，如果您的設計有四個項目的位置，但您的條件僅導致建議兩個項目，您可以將其餘的位置保留為空白，您可以使用備用建議來填入額外的位置，或選擇不顯示建議。
...
1. （可選）滑動「**[!UICONTROL 顯示備份內容]**」切換至「開啟」位置。
   ...
   假設您的條件僅導致建議兩個項目。 如果啟用「[!UICONTROL 部分設計呈現]」選項，則前兩個槽將被填充，但其餘兩個槽將保持空。 不過，如果啟用[!UICONTROL 顯示備份Recommendations]選項，則前兩個插槽會根據您指定的條件填入，其餘兩個插槽則會根據您的備份建議填入。

   下列矩陣顯示使用[!UICONTROL 部分設計呈現]和[!UICONTROL 備份內容]選項時將觀察到的結果：
...
1. （條件性）如果您在上一步驟中選取了&#x200B;**[!UICONTROL 顯示備份內容]**，則可以啟用&#x200B;**[!UICONTROL 將包含規則套用至備份建議]**。

   包含規則會決定建議中包含哪些項目。 可用的選項取決於您的垂直產業。

   如需更多詳細資料，請參閱  [請在下方指定](#inclusion) 包含規則。

1. （選用）滑動「**[!UICONTROL 建議先前購買的項目]**」切換至「開啟」位置。
...
>[!NOTE]
>
>如果您選取「**[!UICONTROL Item-Based]**/ **[!UICONTROL 具有類似屬性的媒體]**」作為演算法類型和演算法，便可選擇設定內容相似度規則。
...
選取&#x200B;**[!UICONTROL Item-Based]**/ **[!UICONTROL 具有類似屬性的媒體]**&#x200B;時，您可以選擇建立規則，以增加或減少在決定建議時特定項目屬性的重要性。 對於書籍之類的項目，您可能想要增強&#x200B;*風格*、*作者*、*系列*&#x200B;之類屬性的重要性，以建議類似的書籍。
...
## 訓練影片：在Recommendations (12:33)中建立條件![教學課程徽章](/help/assets/tutorial.png)
