...
description: 了解儲存在訪客設定檔中的訪客專屬屬性，以提供可用於Adobe的相關資訊 [!DNL Target] 活動。
title: 什麼是設定檔屬性？
feature: Audiences
...
中的設定檔屬性 [!DNL Adobe Target] 是訪客專屬的參數。 這些屬性儲存於訪客的描述檔中，以提供可用於活動的訪客資訊。

使用者設定檔包含網頁訪客的人口統計和行為資訊。 此資訊可能包括年齡、性別、購買的產品、上次造訪時間等， [!DNL Target] 使用來個人化其提供給訪客的內容。
...
1. 按一下 **[!UICONTROL 建立指令碼]**.
   ...
   | 參數類型 | 說明 |
   |--- |--- |
   | mbox | 建立 mbox 時透過頁面程式碼直接傳遞。請參閱[傳遞參數至全域 mbox](/help/c-implementing-target/c-implementing-target-for-client-side-web/t-mbox-download/c-understanding-global-mbox/pass-parameters-to-global-mbox.md)。<br>****&#x200B;注意:  具有每個 mbox 呼叫 50 個獨特設定檔屬性的限制。[!DNL Target]如果您必須將超過50個設定檔屬性傳遞至 [!DNL Target]，請使用設定檔更新API方法來傳遞。 如需詳細資訊，請參閱 [中的設定檔更新 [!DNL Adobe Target] API檔案](https://developers.adobetarget.com/api/#updating-profiles). |
...
您可以使用設定檔指令碼來跨多次造訪擷取訪客屬性。設定檔指令碼是內定義的程式碼片段 [!DNL Target] 使用伺服器端JavaScript的形式。 例如，您可能會使用設定檔指令碼來擷取訪客瀏覽您網站的頻率，以及該訪客上次瀏覽的時間。

設定檔指令碼與設定檔參數不同。設定檔參數會擷取關於使用的mbox程式碼實作的訪客的資訊 [!DNL Target].

## 建立設定檔指令碼 {#section_CB02F8B97CAF407DA84F7591A7504810}

在 [!UICONTROL  介面的]「對象」[!DNL Target]標籤下可以使用設定檔指令碼。

若要新增設定檔指令碼，請按一下 **[!UICONTROL 設定檔指令碼]** 標籤 **[!UICONTROL 建立指令碼]**，然後編寫指令碼。

或

若要複製現有設定檔指令碼，請從 [!UICONTROL 設定檔指令碼] 清單中，按一下所需指令碼的刪節號表徵圖，然後按一下 **[!UICONTROL 複製]**.

然後您可以編輯對象以建立類似的對象。

設定檔指令碼會針對每個位置請求，執行設定檔屬性「捕捉器」。收到位置請求時， [!DNL Target] 決定應執行的活動，並顯示適合該活動及該體驗的內容。 [!DNL Target] 也會追蹤活動的成功，並執行任何相關的設定檔指令碼。 此程式可讓您追蹤有關造訪的資訊，例如訪客的位置、每日時間、訪客造訪網站的次數、訪客之前是否已購買等。 這些資訊會接著加到該訪客的描述檔中，以便您更有效追蹤訪客在您網站上的活動。
...
* 在程式碼中，使用 `user.get('parameterName')`.
* 儲存下次執行指令碼時（下一個mbox請求時）可存取的變數，具有 `user.setLocal('variable_name', 'value')`. 參考包含 `user.getLocal('variable_name')` &#39;) 的變數。如果您想要參考上次請求的日期和時間，此程式就十分實用。
...
例如，您可以按一下 [!UICONTROL 資訊] 圖示([!UICONTROL 對象] > [!UICONTROL 設定檔指令碼])。

此 [!UICONTROL 指令碼資訊] 標籤包含下列資訊：名稱、說明和指令碼代碼。

![設定檔指令碼資訊卡](assets/profile_script_info_card.png)

按一下 **[!UICONTROL 查看完整詳細資訊]** 以查看參考所選設定檔指令碼的對象和活動。

![設定檔指令碼資訊卡 > 指令碼使用方式標籤](assets/profile_script_info_card_usage_tab.png)

>[!NOTE]
>
>此 [!UICONTROL 指令碼用法] 索引標籤不會在下列情況下顯示參考所選設定檔指令碼的活動：
>
...
* 在規則運算式中，開頭有點星號(例如： `/.*match/`, `/a|.*b/`)幾乎永遠不需要。 規則運算式搜尋會從字串中的所有位置開始 (除非受到 `^` 限制)，因此已假設點星號。如果此類規則運算式符合長度足夠的輸入資料 (可能至少有數百個字元)，指令碼執行可能會中斷。
...
* 下列建議可協助您限制設定檔指令碼的複雜度。 設定檔指令碼可執行有限的指令數。
   ...
   * 如果設定檔指令碼變得太複雜，請考慮使用 [回應token](/help/administrating-target/response-tokens.md) 。
...
>[!NOTE]
>
>使用 [!DNL console.log] 在設定檔指令碼內不會輸出設定檔值，因為設定檔指令碼執行伺服器端。

* **新增設定檔指令碼作為回應Token以偵錯設定檔指令碼：**

   在 [!DNL Target]，按一下 **[!UICONTROL 管理]**，按一下 **[!UICONTROL 回應Token]**，然後啟用您要除錯的設定檔指令碼。

   每當您載入網站的頁面時， [!DNL Target] 在上面， [!DNL Target] 包含指定設定檔指令碼的值，如下所示：

   ![](assets/debug_profile_script_1.png)

* **使用mboxTrace除錯工具來除錯設定檔指令碼。**

   此方法需要授權Token，您可以按一下 **[!UICONTROL 目標]** > **[!UICONTROL 管理]** > **[!UICONTROL 實作]** > **[!UICONTROL 產生授權Token]** 在 [!UICONTROL 除錯工具] 區段。
...
由於設定檔指令碼於伺服器端執行，因此其無法直接讀取頁面。必須透過 mbox 要求或其他[將資料傳入 Target 的方法](/help/c-implementing-target/c-considerations-before-you-implement-target/c-methods-to-get-data-into-target/methods-to-get-data-into-target.md#concept_0069C0EFB56C4700BB33F2F35C2B9B17)傳遞資料。資料傳入後 [!DNL Target]，設定檔指令碼可將資料讀取為mbox參數或設定檔參數。

## 指令碼設定檔參數的 JavaScript 參考資料

要有效使用指令碼設定檔參數，需要有簡單的Javascript知識。 本節可做為快速參考資料，只需要幾分鐘，就能讓您提高使用此功能的效率。
...
建立一個日的變數 (以毫秒為測量單位)。如果mbox名稱為 `orderThankyouPage`，設定名為的本地（不可見）用戶配置檔案屬性 `lastPurchaseTime` 以顯示目前日期和時間的值。 系統會讀取上次購買時間的值，若已定義， [!DNL Target] 傳回自上次購買時間以來經過的時間，除以一天中的毫秒數（這會產生自上次購買的天數）。
...
建立以下變數： `frequency`，將其初始化為先前的值，或是0（如果沒有先前的值）。 如果 mbox 名稱為 `orderThankyouPage`，則會傳回遞增的值。
...
建立以下變數： `adobeQA` 若要追蹤使用者， [活動QA](/help/c-activities/c-activity-qa/activity-qa.md).
