---
description: 描述檔屬性為訪客專屬的參數。這些屬性儲存於訪客的描述檔中，以提供可用於促銷活動的訪客資訊。
keywords: 設定檔指令碼;設定檔指令碼屬性;設定檔指令碼最佳作法;除錯
seo-description: 描述檔屬性為訪客專屬的參數。這些屬性儲存於訪客的描述檔中，以提供可用於促銷活動的訪客資訊。
seo-title: 設定檔屬性
solution: Target
title: 設定檔屬性
...
描述檔屬性為訪客專屬的參數。這些屬性儲存於訪客的描述檔中，以提供可用於促銷活動的訪客資訊。

## 設定檔屬性 {#concept_01A30B4762D64CD5946B3AA38DC8A201}

描述檔屬性為訪客專屬的參數。這些屬性儲存於訪客的描述檔中，以提供可用於促銷活動的訪客資訊。

當訪客瀏覽或返回另一個作業時，可以使用儲存的描述檔屬性來定位內容，或是記錄群體篩選資訊。
...
| 參數類型 | 說明 |
|--- |--- |
| mbox | 建立 mbox 時透過頁面程式碼直接傳遞。請參閱[傳遞參數至全域 mbox](/help/c-implementing-target/c-implementing-target-for-client-side-web/t-mbox-download/c-understanding-global-mbox/pass-parameters-to-global-mbox.md)。<br>注意: Target 具有每個 mbox 呼叫 50 個獨特設定檔屬性的限制。如果您需要傳遞超過 50 個設定檔屬性至 Target，則可以使用 設定檔更新 API 方法來傳遞它們。如需詳細資訊，請參閱 [Adobe Target API文件](http://developers.adobetarget.com/api/#updating-profiles)中的設定檔更新。 |
...
## 建立描述檔指令碼 {#section_CB02F8B97CAF407DA84F7591A7504810}
...
* 參數和值區分大小寫。請符合促銷活動或測試期間所收到參數與值的大小寫。
* 請參閱以下的指令碼描述檔參數「JavaScript參考」，以取得更多JavaScript語法。

## 檢視描述檔指令碼資訊卡 {#section_18EA3B919A8E49BBB09AA9215E1E3F17}
...
## Target會在某些情況下停用描述檔指令碼 {#section_C0FCB702E60D4576AD1174D39FBBE1A7}
...
* 如需詳細資訊，請參閱JS Rhino引擎文件： [https://www.mozilla.org/rhino/doc.html](https://www.mozilla.org/rhino/doc.html)。

## 測試互斥活動的描述檔指令碼 {#section_FEFE50ACA6694DE7BF1893F2EFA96C01}
...
## 除錯描述檔指令碼 {#section_E9F933DE47EC4B4E9AF2463B181CE2DA}
...
## 描述檔指令碼常見問題 {#section_1389497BB6D84FC38958AE43AAA6E712}
...
## 指令碼描述檔參數的JavaScript參考

必須使用簡單的Javascript知識，才能有效使用指令碼描述檔參數。本節提供快速參考，讓您只需幾分鐘的時間就能使用此功能。

指令碼描述檔參數位於mbox/描述檔索引標籤下。您可以編寫可傳回任何Javascript類型(字串、整數、陣列等)的Javascript程式。
...
建立一個日期變數，以毫秒為單位測量。如果mbox名稱是 `orderThankyouPage`，請設定名稱 `lastPurchaseTime` 為要採用目前日期和時間值的本機(不可見)使用者描述檔屬性。會讀取最後一次購買時間的值，如果已定義，我們會傳回自上次購買時間後所經過的時間，除以一天內的毫秒數(結果為上次購買後的天數)。
...
建立名為頻率的變數，將其初始化為先前值，如果沒有先前值則加以初始化。如果mbox名稱是 `orderThankyouPage`，則會傳回遞增值。
...
建立呼叫的變數 `monetaryValue`，查詢指定訪客的目前值(若沒有先前值則設定為0)。如果mbox名稱是 `orderThankyouPage`，則會借由新增前一個貨幣值及傳遞給mbox的 `orderTotal` 參數值來傳回新的貨幣值。
...
| --- | --- |
| `page.url` | 目前的URL。 |
| `page.protocol` | 用於頁面(http或https)的通訊協定。 |
| page.domain | 目前的URL網域(第一斜線之前)。`www.acme.com``http://www.acme.com/categories/men_jeans?color=blu e&size=small`例如，In. |
| `page.query` | 目前頁面的查詢字串。之後所有的一切？&#39;.`blue&size=small``http://www.acme.com/categories/mens_jeans?color=blue&size=small`例如，In. |
| `page.param(‘<par_name>’)` | 所指示參數的值 `<par_name>`。如果您目前的URL是Google的搜尋頁面，而您輸入 `page.param('hl')`了，則URL `http://www.google.com/search?hl=en& q=what+is+asdf&btnG=Google+Search`會得到「en」。 |
| `page.referrer` | 上述作業集也適用於反向連結和著陸(亦即referrer. url將是反向連結的URL位址)。 |
| `landing.url`, `landing.protocol`, `landing.query`, 和 `landing.param` | 類似於頁面，但用於登陸頁面。 |
| `mbox.name` | 作用中mbox的名稱。 |
| `mbox.param(‘<par_name>’)` | 由作用中mbox中指定名稱的mbox參數。 |
| `profile.get(‘<par_name>’)` | 用戶端建立的使用者描述檔參數，依名稱 `<par_name>`而定。例如，如果使用者設定名為「性別」的描述檔參數，則可使用「profile. genocation」擷取該值。傳回目前訪客之「`profile.<par_name>`」設定的值；若未設定任何值，則會傳回null。 |
| `user.get(‘<par_name>’)` | 傳回目前訪客之「`user.<par_name>`」設定的值；若未設定任何值，則會傳回null。 |
| `user.categoryAffinity` | 傳回最佳類別的名稱。 |
| `user.categoryAffinities` | 傳回具有最佳類別的陣列。 |
| `user.isFirstSession` | 如果是訪客的第一個作業，則傳回true。 |
| `user.browser` | 傳回HTTP標題中的user-Agent。例如，您可以建立運算式定位，僅定位Safari使用者： `if (user.browser != null && user.browser.indexOf('Safari') != -1) { return true; }` |

### 常用運算子


所有標準JavaScript運算子都存在且可使用。JavaScript運算子可用於字串和數字(以及其他資料類型)。快速簡報：

| 運算元 | 說明 |
| --- | --- |
| `==` | 表示相等。當任一側的運算元等於時，維持true。 |
| `!=` | 表示不公平。當任一側的運算元不相等時，仍維持true。 |
| `<` | 指出左側的變數小於右側的變數。如果變數等於，將會評估為false。 |
| `>` | 指出左側的變數大於右側的變數。如果變數等於，將會評估為false。 |
| `<=` | 與 `<` 變數相同，但變數會評估為true。 |
| `>=` | 與 `>` 變數相同，但變數會評估為true。 |
| `&&` | logical「Nandes」the expleft and right of it- is only when both both when both both both(false other). |
| `||` | logical「Oers」the expleft and right of it- is thonly if one of the side of true(otherwise other false). |
| `//` | Checks if source contains all elements from target布林contains(Array source，Array target).<br>`//` 擷取來自目標的子字串(對應於regexp)並解碼它 `Array/*String*/ decode(String encoding, String regexp, String target)`。<br>此功能也支援使用常數字串值、分組(`condition1 || condition2) && condition3`和規則運算式(`/[^a-z]$/.test(landing.referring.url)`. |
