---
keywords: 設定檔指令碼；設定檔指令碼屬性；互斥的活動
description: 了解如何使用設定檔屬性來在Adobe [!DNL Target] 中設定測試，以比較多個活動，但不要讓相同的訪客參與每個活動。
title: 我可以使用設定檔指令碼來測試互斥的活動嗎？
feature: 對象
...
您可以在[!DNL Adobe Target]中使用設定檔屬性來設定測試，以比較兩個或多個活動，但不讓相同的訪客參與每個活動。

測試互斥的活動可避免某個活動中的訪客影響到其他活動的測試結果。當訪客同時參與多個活動時，很難判斷正提升或負提升是否來自訪客在某個活動中的體驗，或者，多個活動之間的互動是否影響一個或多個活動的結果。

例如，您可以測試您電子商務系統的兩個區域。您可能想要測試「新增至購物車」按鈕是否為紅色而非藍色。 您也想要測試一套新的結帳程序，以將結帳的程序從五個步驟減為兩個步驟。如果兩個活動具有相同的成功事件（已完成的購買），就很難判斷紅色按鈕是否改善了轉換，或者因為改善了結帳程式，這些相同的轉換是否也增加了。 透過將測試分成互斥的活動，您可以單獨測試每個變更。

使用下列其中一個設定檔指令碼時，請注意下列資訊:

* 設定檔指令碼必須在活動啟動之前執行，而指令碼在活動期間必須保持不變。
* 此技術可減少活動中的流量，而可能需要活動執行更久。 當預估活動的持續時間，您必須考量此事實。
...
建立設定檔屬性後，請設定第一個活動，要求使用者設定檔參數`user.twogroups`符合為GroupA指定的值，以鎖定所需的母體。

>[!NOTE]
>
>在頁面上方的位置選擇一個 mbox。此程式碼會判斷訪客是否體驗活動。 只要瀏覽器會在先處理 mbox 的情況下，您便可以使用代碼來設定此值。
