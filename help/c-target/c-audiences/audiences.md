...
seo-description: 對象會決定將看見已鎖定目標活動中內容和體驗的人。
seo-title: 建立對象
solution: Target
title: 建立對象
...
可使用鎖定目標之處皆可使用對象。在鎖定活動的對象時，您可以從[!UICONTROL 「對象」]清單中選取可重複使用的對象、建立活動專屬對象並鎖定為目標，或[結合多個對象](../../c-target/combining-multiple-audiences.md#concept_A7386F1EA4394BD2AB72399C225981E5)以建立隨選對象。
...
* **報表對象:**用於決定不同類型的訪客如何回應相同內容，以利於您分析測試結果。
   ...
   在 [!DNL Target] 中，只有在使用 [!DNL Target] 作為報表來源時，才可設定報表對象。如果您使用 [ [!DNL Adobe Analytics] 作為報表來源 (A4T)](../../c-integrating-target-with-mac/a4t/a4t.md#concept_7540C8C04259434AB6EE33B09F47A1DE)，必須在 [!DNL Analytics] 內設定報表對象。
...
下列對象定義卡是屬於從 Adobe Experience Cloud 匯入的對象。在此情況中，對象是從 Adobe Audience Manager (AAM) 匯入。請注意，Target 不會顯示詳細的對象定義資訊，因為 Target 中不存在此資訊。
