...
* 與團隊成員分享的連結，無論體驗或活動如何更新，這些連結皆不可能變更或需要重新產生。這可讓您完整測試整個使用者歷程中的活動。
* 對象條件可選擇性地受到重視，因此行銷人員可以測試定位條件，或忽略QA的定位條件，而不需符合觀眾條件。
* QA報告是擷取的，因此行銷人員可以確認量度是否正如預期遞增，而QA報表資料會與生產報告分開(適用於非A4T報表)。
* 能夠單獨預覽體驗，或結合其他滿足傳送條件 (頁面/mbox/對象) 的已上線活動一起預覽。
* QA整個使用者歷程的能力。您可以利用 QA 連結來存取一次您的網站，然後在活動 QA 期間瀏覽整個網站。您會停留在活動 QA 中直到結束工作階段，或直到您使用[QA Target 書籤小程式](../../c-activities/c-activity-qa/activity-qa-bookmark.md#concept_A8A3551A4B5342079AFEED5ECF93E879)強迫自己離開活動 QA。如果活動橫跨多個網頁，此功能特別有用。
...
* 如果活動使用多個體驗受眾(例如，同一活動中包含的美國和英國網站)，QA連結不會產生四個組合(體驗A/美國網站、體驗A/英國網站、體驗B/英國網站、體驗B/英國網站)。只會建立兩個 QA 連結 (體驗 A 和體驗 B)，使用者必須屬於適當的對象，才能看到頁面。UK QA 的人無法看見 US 網站。
...
* Target QA模式UI只顯示多頁活動中體驗的第一個URL。假設您要建立歷程測試，而您將從URL移至URL2。不過，如果您想要單獨前往URL2，請複製所有針對URL提供的URL參數，並在放置「？」後將它們套用至URL2。就像您在URL中看到的一樣。