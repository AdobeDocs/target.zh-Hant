...
[!DNL Adobe Target]中的自動分配活動會從兩個或多個體驗中識別獲勝者，並自動重新分配更多流量給獲勝者以增加轉換，同時測試會繼續執行和學習。
...
>[!NOTE]
>
>如果活動只有兩個體驗，則兩個體驗會獲得相等的流量，直到[!DNL Target]找到具有75%信賴的成功體驗為止。 此時，2/3的流量會分配給獲勝者，1/3的流量會分配給失敗者。 之後，當體驗達到95%信賴度時，90%的流量會分配給獲勝者，10%會分配給失敗者。 我們一律會維持將某些流量傳送至「遺失」體驗，以避免長期出現誤判（亦即維持一些探索）。
...
### Analytics for Adobe Target(A4T)支援自動分配活動嗎？
...
您可以使用現有的[樣本大小電腦](https://experienceleague.adobe.com/tools/calculator/testcalculator.html)來取得測試執行時間長短的預估值。 （如同傳統A/B測試一樣，如果您要測試兩個以上選件或多個轉換量度/假設，請套用Bonferroni校正。） 請注意，此計算器專為傳統固定水準A/B測試而設計，僅提供估計值。 使用自動分配活動的計算器是選用的，因為自動分配會為您宣告獲勝者（您不需要挑選固定的時間點來查看測試結果），所提供的值一律在統計上有效。 在我們的實驗中，我們發現了下列內容：
* 在僅測試兩個體驗時，當體驗之間的效能差異較大時，自動分配比固定視界測試（即樣本大小電腦建議的時間範圍）更快找到獲勝者，但當體驗之間的效能差異較小時，可能需要額外的時間來識別獲勝者。 在這些情況下，固定水準測試通常不會產生統計顯著結果。
* 在測試超過兩個體驗時，當單一體驗執行力強而無法執行所有其他體驗時，自動分配比固定範圍測試（即樣本大小計算器建議的時間範圍）更快找到獲勝者。 當兩個或多個體驗對其他體驗均為「勝出」，但彼此非常相符時，自動分配可能需要額外的時間來判斷哪個體驗較優秀。 在這些情況下，固定水準測試通常會以下結論結束： 「成功」體驗優於表現較差的體驗，但並未確定哪個體驗優於前者。

### 是否應從自動分配活動中移除表現欠佳的體驗，以加速決定獲勝者的程式？

沒有理由移除表現欠佳的體驗。 自動分配會更頻繁地自動提供表現優異的體驗，並較少地提供表現欠佳的體驗。 在活動中保留表現欠佳的體驗，不會大幅影響決定獲勝者的速度。
...
### 我在執行自動分配活動時可以使用「重設報告資料」選項嗎？

不建議對[!UICONTROL 自動分配]活動使用[!UICONTROL 重設報表資料]選項。 雖然會移除可見的報表資料，但此選項不會從[!UICONTROL 自動分配]模型中移除所有訓練記錄。 針對[!UICONTROL 自動分配]活動，請建立新活動並取消啟動原始活動，而非使用[!UICONTROL 重設報表資料]選項。 (注意：本指南也適用於[!UICONTROL 自動鎖定目標]和[!UICONTROL Automated Personalization]活動。)

### 自動分配如何建立環境相關模型？

[!UICONTROL 自動分] 配僅根據預設環境中記錄的流量和轉換行為來建立模型。預設情況下，[!UICONTROL Production]為預設環境，但可在Target [Administration > Environments](/help/administrating-target/environments.md)中變更。

如果點擊發生在其他（非預設）環境，則流量將根據預設環境中觀察到的轉換行為進行分配。 該點擊（轉換或非轉換）的結果將記錄以用於報告用途，但未在[!UICONTROL 自動分配]模型中考慮。

選取其他環境時，報表會顯示該環境的流量和轉換。 報表的預設選取環境將一律為選取的帳戶範圍預設值。 預設環境無法根據每個活動設定。
