...
自動分配可識別兩個以上體驗之間的成功者，並自動分配更多流量給成功者，以在測試繼續執行和學習時增加轉換。
...
自動分配以智慧式方法分配體驗給新訪客，直到最佳體驗與任何其他體驗的信賴區間不重疊為止。這個過程通常會產生誤判，但自動分配會根據補償重複評估的 [Bernstein 不等式](https://en.wikipedia.org/wiki/Bernstein_inequalities_(probability_theory))來使用信賴區間。現在，我們有真正贏家。當「自動分配」停止時，若到達頁面的訪客沒有顯著時間依賴性，則自動分配將至少傳回一個體驗，其真正回應不會低於成功體驗的真正回應。
...
**使用「自動分配」時，我可以使用「自動分配」來估計活動要花多久時間識別成功者？**

您可以利用現有的[樣本大小計算機](https://docs.adobe.com/content/target-microsite/testcalculator.html)，並適當套用 Bonferroni 校正，以取得測試執行時間長短的預估值。在我們的範例中，已看到自動分配活動遠比此樣本大小更早結束。

**我應該從「自動分配」活動移除效能不佳的體驗，以加速決定成功者的程序嗎？**

我們沒有理由移除效能不佳的體驗。自動分配會更頻繁地提供高效能的體驗，並較常提供效能不佳的體驗。在活動中留下表現不佳的體驗並不會大幅影響判斷成功者的速度。

20%的訪客會隨機指派給所有體驗。支援效能不佳的體驗量最低(20%除以體驗數)。
