...
1. 若要使用可視化體驗撰寫器 (VEC)，請按一下**[!UICONTROL 「視覺 (預設)」]**。
   ...
   如果您偏好使用表單式體驗撰寫器，請選取**[!UICONTROL 「表單」]**。

   如需關於這兩種撰寫器的詳細資訊，請參閱[體驗](../../c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D)。
...
1. 修改[體驗](../../c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D)中說明的頁面元素，然後按一下**[!UICONTROL 「下一步」]**。
...
1. 按一下**[!UICONTROL 「管理內容」]**來設定可用的組合。
   ...
   [!UICONTROL 體驗]清單會顯示為活動選取的每個內容片段，及其獲指派的位置。您可以將游標移至需要的體驗上，然後按一下排除圖示來排除特定體驗，或您可以透過選取相關體驗的核取方塊，然後按一下對話方塊右上角的「排除」圖示來批次排除/包括體驗。

   ![](assets/ap_content_batch_exclude.png)

   您可以按一下**「狀態」**下拉式清單來篩選此清單檢視，以查看僅限排除或僅限包括的活動。
1. 按一下**[!UICONTROL 「選件」]**來選取內容片段，並將它們指派給[報表群組](../../c-reports/offer-reporting-groups-in-automated-personalization.md#concept_194128C0B56B4B26AAB57DB49892960C)。
   ...
   您可以將游標移至需要的選件上，然後按一下資料夾圖示，將特定體驗新增至報表群組，或您可以選取相關體驗的核取方塊，然後按一下對話方塊右上角的「報表群組」資料夾圖示按鈕，將體驗批次加入報表群組。
...
1. (依條件) 按一下**[!UICONTROL 「選件」]**，選取內容片段並將它們指派給報表群組，或只允許特定訪客查看特定具有鎖定目標的選件。
   ...
   如需將選件鎖定在特定對象的相關資訊，請參閱 [Target 鎖定自動個人化選件](../../c-activities/t-automated-personalization/ap-target-offers.md#task_F207ED7A41B84FD39BB6FCBFABF4B23E)。
1. (依條件) 按一下**[!UICONTROL 「排除群組」]**來選擇您要從活動中排除的任何元素組合。

   ![步驟結果](assets/exclusion_groups.png)
...
1. 完成活動內容的設定時，請按一下**[!UICONTROL 「下一步」]**。
...
1. 按一下**[!UICONTROL 「繼續」]**以顯示**[!UICONTROL 「目標與設定」]**頁面。
1. 使用下列設定來設定活動，然後按一下**[!UICONTROL 「儲存並關閉」]**。
   ...
   | 轉換量度 | 根據預設，轉換度量與最佳化目標度量相同。不過，您可以取消勾選 [!UICONTROL 「與最佳化目標」] 選項來定義個別轉換度量。 |
