...
* 開始展開活動之前，根據目標決定控制與目標之間的分配。
   ...
   * 自動個人化活動的目標是判斷個人化演算法的整體運作情況，或是在頁面上執行「隨時待命」個人化最佳化? 根據此問題的回答而定，您需要在控制與目標之間採用不同的流量分配。
   * 如果目標是測試演算法，請在控制與目標演算法之間採取 50/50 百分比的訪客分割。此分割可提供提升度更準確的預估。
   * 如果目標是建立「隨時待命」活動，請將 10% 至 30 % 的訪客放入控制，以確保有足夠的資料供演算法隨著時間而繼續學習。請注意，這裡的取捨是以更大比例的流量交換個人化，因此在確切的提升度方面較不精確。
