...
description: 瞭解 [!DNL Target] 中的「自動定位」活動如何根據客戶個人檔案和類似訪客的行為，為每位訪客提供最自訂的體驗。
title: 什麼是自動定位活動？
feature: 自動鎖定目標
...
[!UICONTROL 自動定] 位活動使 [!DNL Adobe Target] 用進階機器學習，從多個高效能行銷人員定義的體驗中選擇，以個人化內容並推動轉化。Auto-Target會根據個別客戶描述檔和先前具有類似描述檔的訪客行為，為每位訪客提供最自訂的體驗。
...
* 您無法將特定活動從[!UICONTROL Auto-Target]切換至Automated Personalization，反之亦然。
* 您無法從手動流量分配（傳統A/B測試）切換至[!UICONTROL 自動目標]，活動生效後亦然。
* 建立一個模型，用以識別個人化策略的效能與隨機提供的流量，以及將所有流量傳送至整體成功體驗。 此模型僅會在預設環境中考慮點擊和轉換。

   來自第二組模型的流量會針對每個模型群組(AP)或體驗(AT)建立。 對於這些模型，會考慮所有環境的點擊和轉換。

   無論環境如何，請求都以相同的模型提供，但是多個流量應來自預設環境，以確保所識別的整體成功體驗與實際行為一致。
...
| 隨機森林 | 隨機森林是先進的機器學習方法。在資料科學中，它是一種整合分類或回歸方法，通過基於訪客和訪問屬性構建多個決策樹來發揮作用。 在 Target 內，隨機森林可用來針對每個特定訪客，決定何種體驗預期轉換的可能性最高 (或每次造訪帶來的收入最高)。如需關於 Target 中隨機森林的資訊，請參閱[隨機森林演算法](/help/c-activities/t-automated-personalization/algo-random-forest.md)。 |
...
| 術語 | 詳細資料 |
|--- |--- |
| [隨機森林演算法](/help/c-activities/t-automated-personalization/algo-random-forest.md) | Target 在[!UICONTROL 自動鎖定目標]和自動個人化中採用的主要個人化演算法是「隨機森林」。隨機森林之類的整體方法會使用多個學習演算法，相較於任何建構式學習演算法可獲得更好的預測性效能。Automated Personalization系統中的隨機森林算法是一種分類或回歸方法，它通過在訓練時構造多個決策樹來操作。 |
...
| [Target 個人化演算法的資料收集](/help/c-activities/t-automated-personalization/ap-data.md) | Target的個人化演算法會自動收集各種資料。 |
...
| 活動目標 | 建議的流量分配 | 取捨 |
|--- |--- |--- |
| **評估個人化演算法 (50/50)**: 如果您的目標是要測試演算法，請在控制與鎖定的演算法之間使用訪客的 50/50 百分比分割。此分割可提供提升度更準確的預估。建議「隨機體驗」搭配使用以作為控制。 | 50% 控制/ 50% 個人化體驗分割 | <ul><li>使控制與個人化之間的提升度達到最高準確性</li><li>擁有個人化體驗的訪客相對較少</li></ul> |
| **最大化個人化流量 (90/10)**: 如果您的目標是要建立「一律開啟」的活動，請將 10% 的訪客放入控制，以確保有足夠的資料供演算法隨著時間繼續學習。請注意，這裡的權衡點是，為了個人化流量的較大比例，精確提升度的精確度較低。 無論您的目標為何，這是使用特定體驗作為控制時的建議流量分割。 | 最佳作法是採用 10% - 30% 控制/ 70% - 90% 個人化體驗分割 | <ul><li>將具有個人化體驗的訪客人數最大化</li><li>將提升度最大化</li><li>活動的提升度較不準確</li></ul> |
...
* 如果您想使用[!UICONTROL 自動個人化]不支援的完整Visual Experience Composer(VEC)功能集：自訂程式碼編輯器、多個體驗受眾等。
* 如果您想對不同體驗中的頁面進行結構性變更。例如，如果您想要重新排列首頁上的元素，[!UICONTROL Auto-Target]將比Automated Personalization更適合使用。
...
* 訪客在現有作業結束時可享有新體驗（除非訪客位於控制群組中，在這種情況下，訪客在首次瀏覽時指派的體驗在後續瀏覽時仍維持不變）。
* 在工作階段內，預測不變，以維持視覺一致性。

**演算法會適應訪客行為的變化。**

* 多臂強盜可確保模型始終「花費」少量流量，以便在活動學習的整個生命週期中繼續學習，並防止過度利用先前學到的趨勢。
* 使用最新訪客行為資料，每24小時重建基礎模型，以確保Target始終利用不斷變更的訪客偏好設定。
