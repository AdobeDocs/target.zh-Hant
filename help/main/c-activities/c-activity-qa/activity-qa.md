...
1. 與您的組織成員共用活動連結URL以進行測試。
   ...
   您可以瀏覽網站以檢視其他頁面，因為[!UICONTROL Activity QA]模式有粘性。 此情況適用於版本2.*x*&#x200B;或更新版本的at.js實作。 針對at.js 1.*x*&#x200B;實作，只有在訪客的瀏覽器未封鎖第三方Cookie時，才會發生這種情況。
...
如果您的網站已部署at.js 2.*x*，請使用[Target QA書籤小程式](/help/main/c-activities/c-activity-qa/activity-qa-bookmark.md#concept_A8A3551A4B5342079AFEED5ECF93E879)強迫自己離開[!UICONTROL Activity QA]。 在網站上載入具有空白值的頁面（如下一個專案符號所述），在部署at.js 2.*x*&#x200B;時，*不會*&#x200B;從瀏覽器中移除QA Cookie。
...
* 如果您有at.js 1.*x*，如果您使用Safari或其他封鎖第三方Cookie的瀏覽器，[!UICONTROL Activity QA]模式不會產生粘性。 在這些情況下，您必須將預覽引數新增至您導覽到的每個URL。 如果您已實作[CNAME](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/implement-cname-support-in-target.html?lang=zh-Hant){target=_blank}，也同樣如此。
