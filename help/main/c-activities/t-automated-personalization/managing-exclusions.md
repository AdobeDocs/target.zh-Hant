...
description: 在中管理排除專案 [!DNL Adobe Target] [!UICONTROL Automated Personalization] (AP)活動。 建立排除群組並排除重複選件、特定體驗和預設內容。
title: 我該如何在中管理排除 [!UICONTROL Automated Personalization] 活動？
...
管理排除的方法包括建立排除群組、排除重複選件、排除特定體驗，以及排除中的預設內容。 [!UICONTROL Automated Personalization] (AP)中的活動 [!DNL Adobe Target].

## 建立排除群組 {#task_AAAA6C7239A84F7696C8492F04B575A2}

在中建立排除群組 [!UICONTROL Automated Personalization] (AP)活動，以確保會自動排除具有指定選件的體驗。
...
1. 在 [!UICONTROL 管理內容] 對話方塊，按一下 **[!UICONTROL 排除群組]**.
...
按一下 **[!UICONTROL 設定]** 齒輪選項> **[!UICONTROL 重複選件]**，然後按一下 **[!UICONTROL 允許重複專案]** 或 **[!UICONTROL 不允許重複專案]**.
...
如果您想從您的網站排除特定選件組合，請排除特定體驗。 [!UICONTROL Automated Personalization] 活動。
...
1. 視需要排除體驗。
   ...
   或者，您可以選取相關體驗的核取方塊，然後按一下 **[!UICONTROL 排除]** 圖示來顯示這個對話方塊的右上角。 此 [!UICONTROL 排除] 圖示會在勾選一或多個體驗時顯示。
   ...
   體驗現在已從活動中排除，其 [!UICONTROL 狀態] 顯示為 [!UICONTROL 已排除].
...
有時候，您可能不想將預設內容加入您的檔案 [!UICONTROL Automated Personalization] 活動。 您存取此設定的方式與建立排除群組不同。您可以使用此方法以在一個位置中只有一個選件 (與您的預設內容不同) 作為 AP 活動的一部分。
...
1. 在對話方塊中，建立新內容，並取消勾選預設內容右邊 **「包含」** (或在「選取內容」畫面中取消勾選「預設影像/影片」)。
   ...
   根據內容或選件型別， [!UICONTROL 包含] 核取方塊的位置稍有不同。
...
1. 按一下&#x200B;**[!UICONTROL 儲存]**。
   ...
   您可以在[!UICONTROL 「管理內容」]下看到透過您指定的選件建立的體驗。您注意到中不會建立任何體驗 [!UICONTROL 管理內容] 使用您已排除的預設選件。

   ![exclude_content_vec_4圖片](assets/exclude_content_vec_4.png)

**[!UICONTROL 若要使用表單式體驗撰寫器來排除預設內容]:**
