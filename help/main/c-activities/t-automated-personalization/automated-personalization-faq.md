...
description: 探索有關Adobe Target中Automated Personalization (AP)活動的常見問題(FAQ)和答案清單。
title: 如何尋找Automated Personalization活動的常見問題集？
...
在使用時查詢下列常見問答集 [!UICONTROL Automated Personalization] 活動：
...
使用 [!UICONTROL 重設報表資料] 選項 [!UICONTROL Automated Personalization] 不建議使用活動。 雖然此選項會移除可見的報表資料，但不會移除 [!UICONTROL Automated Personalization] 模型。 不要使用 [!UICONTROL 重設報表資料] 選項 [!UICONTROL Automated Personalization] 活動、建立新活動和停用原始活動。 (附註：本指引亦適用於 [!UICONTROL 自動分配] 和 [!UICONTROL 自動鎖定目標] 活動。)
