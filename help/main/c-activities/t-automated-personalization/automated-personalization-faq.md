---
keywords: 故障排除；常見問題；常見問題；FAQ;FAQs；自動個性化；控制；預設經驗；最佳實踐
description: 瀏覽有關Automated Personalization(AP)在Adobe Target活動的常見問題(FAQ)和解答清單。
title: 如何查找有關Automated Personalization活動的常見問題？
...
# ![高級](/help/main/assets/premium.png) Automated Personalization常見問題
...
使用時請參考以下常見問題和答案 [!UICONTROL Automated Personalization] 活動：
...
### 我能否在Automated Personalization活動中途更改目標指標？ {#change-metric}
...
使用 [!UICONTROL 重置報告資料] 選項 [!UICONTROL Automated Personalization] 不建議活動。 儘管它刪除了可見的報告資料，但此選項不會從 [!UICONTROL Automated Personalization] 模型。 而不是使用 [!UICONTROL 重置報告資料] 選項 [!UICONTROL Automated Personalization] 活動，建立新活動並取消激活原始活動。 (注：本指南也適用於 [!UICONTROL 自動分配] 和 [!UICONTROL 自動目標] 活動。)
...
因此，無論環境如何，請求都將使用相同的模型來提供，但是多個流量應來自預設環境，以確保所識別的總體贏取體驗與真實世界行為相一致。
