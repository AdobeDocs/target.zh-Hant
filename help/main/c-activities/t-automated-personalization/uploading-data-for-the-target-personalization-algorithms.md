...
有數種方式可以在[!UICONTROL Automated Personalization] (AP)和[!UICONTROL Auto-Target]個人化演演算法中輸入資料。 除了[將資料傳入Target](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/methods/methods-to-get-data-into-target.html){target=_blank}的方法以外，[!DNL Target]演演算法中也使用[!DNL Experience Cloud]共用對象([!UICONTROL Adobe Analytics]、[!DNL Audience Manager])和活動內報表對象。
