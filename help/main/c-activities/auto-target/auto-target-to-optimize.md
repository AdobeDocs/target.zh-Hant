...
* 請留意活動形狀。 相較於原始體驗計數，效能更多地取決於位置×選件的數量。 大型笛卡爾組合可能會拖慢[!UICONTROL Visual Experience Composer]中的載入和編輯速度，即使低於記錄的體驗限制也是如此。

  依據最佳做法的要求，將[!UICONTROL Auto-Target]和[!UICONTROL Automated Personalization]活動保留在4-6個位置，每個位置有4-6個選件。 不建議使用較大的設定。 因為這些活動是直接在[!UICONTROL Experiences]步驟中建置，所以當設定超過支援的臨界值時，[!DNL Target] UI可能會顯示內嵌警告或封鎖儲存。
