...
| 符合裝置上決策資格 | 建立符合裝置上決策資格的活動後，活動的概觀頁面中會顯示讀取符合裝置上決策資格的標籤。<br>此標籤並不表示活動將一律透過裝置上決策傳送。 只有當at.js 2.5.0+設定為使用裝置上決策時，此活動才會在裝置上執行。 如果at.js 2.5.0+未設定為使用裝置上，則此活動仍會透過從at.js進行的伺服器呼叫傳送。<br>另請參閱 [裝置上決策](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/on-device-decisioning/on-device-decisioning.html){target=_blank}. |
...
| 刪除 | 刪除草稿或活動。<BR>**注意**: 已刪除的活動無法復原。除非您非常確定您不會再需要此活動，否則請使用[!UICONTROL 封存]動作。之後，您可以視需要重新啟用活動。 |
...
## 屬性已匯入至 [!DNL Target] 適用於在外部更新的活動 [!DNL Target] {#section_802B0D174E6A44E1A96F404CA81AAE44}
