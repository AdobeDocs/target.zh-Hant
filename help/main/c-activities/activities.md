...
| [!UICONTROL 位置] | 活動的 URL 可識別活動出現的位置。此欄可協助您快速識別活動，並判斷特定頁面上是否已有活動在執行。<P>如果活動在多個URL上執行，則連結會顯示還有使用多少個URL。 按一下連結以檢視該活動的完整 URL 清單。<P>您可以根據URL來搜尋。 使用搜尋方塊旁的下拉式清單，然後選取 [!UICONTROL URL]. |
| 作者 | 建立活動的使用者名稱。 |
