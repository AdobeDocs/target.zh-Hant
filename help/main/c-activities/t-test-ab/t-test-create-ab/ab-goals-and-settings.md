...
[!UICONTROL 目標與設定]頁面的[!UICONTROL 活動設定]區段可讓您設定下列選項：
...
| [!UICONTROL 持續時間] | 活動可以在核准後開始，或者您可以設定特定的日期和時間。 同樣地，活動可以在停用時結束，或者您可以設定日期和時間。 時間選擇器使用 24 小時時鐘，午夜為 00:00。 時區會設為瀏覽器中設定的時區。 若要使用不同的時區，請將您的瀏覽器設定為其他時區並重新啟動瀏覽器。 |

## [!UICONTROL 報表設定] {#section_13119392051044FBA6387D9B3B1C43CF}

[!UICONTROL 目標與設定]頁面的[!UICONTROL 報告設定]區段可讓您設定下列選項：
...
| [!UICONTROL 統計方法] | 選擇分析實驗結果的方式。 這個選擇會決定用於此活動的[!UICONTROL 轉換]目標量度的統計架構、結果量度和決定條件：<ul><li>**[!UICONTROL Bayesian]**：以機率為基礎的決策，可在資料到達時更新信念，並支援持續監控和較小的樣本大小。 結果是以一個變體勝過另一個變體的機率來表示，例如「92%的機率是變體B勝過控制」。 [!UICONTROL Bayesian]最適合小樣本量、持續監控、偏好機率的利害關係人，或您先前已掌握要套用的測試知識。 如果您需要嚴格的法規或法規簽核，或需要傳統的p值，這個選項並不理想。 報表顯示[!UICONTROL 擊敗控制項]的機會和可信的間隔，而不是頻率最高的[!UICONTROL 信賴度]欄。</li><li>**[!UICONTROL Welch的t檢定(Frequentist)]**：現有的傳統假設測試方法。 使用p值並需要固定的樣本大小。 報表顯示[!UICONTROL 信賴]百分比和信賴區間。</li></ul>此設定僅適用於主要目標量度為[!UICONTROL 轉換]的活動，具有[!UICONTROL 收入]或[!UICONTROL 參與]主要目標的活動一律使用[!UICONTROL Welch的t檢定]。 如需關於如何計算及報告每個方法的詳細資訊，請參閱[A/Bn測試中的統計計算](/help/main/c-reports/statistical-methodology/statistical-calculations.md)。 |
| [!UICONTROL 貝葉斯組態] | 只有在選取[!UICONTROL Bayesian]做為[!UICONTROL 統計方法]時才會顯示。 設定以下選項:<ul><li>[!UICONTROL 可信區間]：設定預期實際效果落下的範圍。</li><li>[!UICONTROL 決定臨界值]：設定變體在宣告獲勝者之前必須達到的最小機率（相對於[!UICONTROL 控制項]）。 </li><li>[!UICONTROL 使用過去測試的先前知識？]：開啟此選項以提供過去效能的先前知識，以通知[!UICONTROL Bayesian]計算。</li></ul> |


## 進階設定 {#section_E2FE441AFB324E498793ABB025ED9974}

[!UICONTROL 目標與設定]頁面的[!UICONTROL 進階設定]區段可讓您設定進階選項。

若要指定進階設定，請按一下[!UICONTROL 我的主要目標]區段中的&#x200B;**[!UICONTROL 更多]**&#x200B;圖示（![更多圖示](/help/main/assets/icons/MoreSmallList.svg)），然後按一下&#x200B;**[!UICONTROL 進階設定]**。
...
[!UICONTROL 目標與設定]頁面的[!UICONTROL 其他中繼資料]區段可讓您指定有關您的活動的任何資訊，這些資訊有助於您自己或其他團隊成員隨時掌握。 註釋窗格可調整大小。|
