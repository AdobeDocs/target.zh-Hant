...
description: 瞭解如何在Adobe中使用Multivariate Testing (MVT) [!DNL Target] 比較頁面上元素中的選件組合，以判斷哪個組合效果最佳。
title: 什麼是多變數測試？
...
* 使用 MVT 測試來判斷最重要的元素，接著對該元素進行更聚焦的 A/B 測試。
   ...
   如果不同體驗的數量超過5個，且橫跨兩個或多個元素，在執行您的A/B測試之前最好考慮MVT測試。 MVT 測試會顯示頁面上的哪些區域最可能改善轉換。這些是市場行銷人員應該專注的元素。例如，MVT 測試可能指出呼叫動作是達成目標最重要的元素。一旦您確定哪些元素和內容對於協助您達成目標最有用，就可以執行A/B測試來進一步調整結果，例如測試兩個特定影像彼此對照，或比較行動號召的措辭或顏色。 遵循下列具有一或多個 A/B 測試的 MVT 測試，您可以判斷所需結果的最佳可能內容。
...
* 因為體驗數目會增加，請預留額外時間給 QA。您也可以使用部分階乘測試，以減少多變數測試所需的流量量。 如需詳細資訊，請參閱下列部分因子測試：
