---
keywords: 多元test;mvt；完全階乘；mvt或a/b；多元a/b；流量估計器；何時使用mvt;mvt考慮；多元；部分階乘；全階乘；
description: 瞭解如何在Adobe中使用Multivariate Testing(MVT) [!DNL Target] 比較頁面上元素中的選項組合，以確定哪個組合執行得最好。
title: 什麼是多元Test?
...
* 事先規劃測試設計。建議在test開始運行並開始收集和分析資料後不要編輯它。
* 建議您將元素與元素之間分開測試。

   例如，請勿在相同測試中測試版面配置與內容。

* 因為體驗數目會增加，請預留額外時間給 QA。您還可以使用部分階乘測試來減少多元test所需的流量。 有關詳細資訊，請參閱下面的部分階乘測試：

## 部分階乘檢驗

[!DNL Target] 提供全階乘多變數測試當作內建活動選項。在統計學中，「實驗設計」提供許多方法 (或設計) 來判斷會影響結果的因素。一種是 [田口方法](https://en.wikipedia.org/wiki/Taguchi_methods) 部分階乘測試。 「田口方法」可讓市場行銷人員建立一組假設，以減少需要測試的體驗排列，進而降低多變數測試的流量需求。在 [!DNL Target] 中，可利用此[離線試算表](/help/main/assets/MVT-Taguchi-Partial-Factorial-Design-02102017.xlsx)來運用此功能與測試方法。
...
此概述視頻說明了目標標準/高級版中可用的活動類型。 4:20 開始討論多變數測試。
...
### 建立多元Test(9:25) ![教程徽章](/help/main/assets/tutorial.png)
