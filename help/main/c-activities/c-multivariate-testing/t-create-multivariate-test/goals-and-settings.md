...
description: 瞭解如何使用 [!UICONTROL Goals & Settings] 頁面位置 [!DNL Adobe Target] 若要指定目標的相關資訊，請執行下列動作： [!UICONTROL Multivariate Test] (MVT)活動。
title: 如何在「 」中指定目標與設定 [!UICONTROL Multivariate Test] (MVT)活動？
...
此 [!UICONTROL Goals & Settings] 頁面位置 [!DNL Adobe Target] 是您輸入目標相關資訊的位置 [!UICONTROL Multivariate Test] (MVT)活動。
...
如果未在中啟用此選項 [!UICONTROL Administration] > [!UICONTROL Reporting] （預設值），指定優先順序： [!UICONTROL Low]， [!UICONTROL Medium]，或 [!UICONTROL High].
...
針對在舊版中建立的活動 [!DNL Target]， [!UICONTROL Low] 優先順序已轉換為0， [!UICONTROL Medium] 優先順序會轉換為5，而 [!UICONTROL High] 優先順序會轉換為10。 您可以視需要調整這些值。
...
進階設定適用於 [!UICONTROL Multivariate Test] 目標量度。
...
* [!UICONTROL Select Increment Count & Keep User in Activity] 以指定計數的遞增方式。
* [!UICONTROL Select Increment Count, Release User & Allow Reentry] 以指定使用者重新進入活動時看到的體驗。
