---
keywords: 活動設定；目標和設定；movilator;mvt
description: 瞭解如何在Adobe中使用「目標和設定」頁 [!DNL Target] 指定有關「多變數Test」(MVT)活動的目標的資訊。
title: 如何在多元Test(MVT)活動中指定目標和設定？
...
在Adobe Target的「目標和設定」頁面中，您可以輸入有關目標的資訊 [!UICONTROL 多元] (MVT)test。
...
如果S中未啟用此選項[!UICONTROL 管理] > [!UICONTROL 報告] （預設值），指定優先順序：低、中或高。

要啟用細粒度優先順序，請按一下 [!UICONTROL 管理] > [!UICONTROL 報告]，然後將「啟用細粒度優先順序」選項切換到「開啟」位置。
