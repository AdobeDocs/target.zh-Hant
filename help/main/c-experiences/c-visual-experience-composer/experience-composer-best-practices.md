...
description: 瞭解在Adobe Target中使用視覺化體驗撰寫器(VEC)時，讓您的體驗如預期運作的最佳實務。
title: 視覺化體驗撰寫器有哪些最佳作法和限制？
...
下列最佳作法可協助讓您的體驗如預期般運作。另外還有其他您在中使用視覺化體驗撰寫器(VEC)時應注意的提示和限制 [!DNL Adobe Target].
...
若要在帳戶層級啟用增強體驗撰寫器，請按一下 [!UICONTROL 管理>視覺化體驗撰寫器]，然後將開關切換至「開啟」位置。

在可視化體驗撰寫器中建立活動時，若要在活動層級上啟用增強體驗撰寫器，請按一下「[!UICONTROL 設定 > URL]」，然後將開關切換至「開啟」位置。

**如果不會在網站的安全頁面上載入「增強視覺化體驗撰寫器」，您可以允許列出某些IP位址。**

載入增強型視覺化體驗撰寫器的問題可藉由將下列IP位址列入允許清單來解決。 這些 IP 位址用於 Adobe 的伺服器，用於增強體驗撰寫器 Proxy。只有針對活動編輯才需要這些資訊。您網站的訪客不需要將這些IP位址列入允許清單。
