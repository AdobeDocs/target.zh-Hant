...
description: 瞭解在Adobe Target中使用視覺化體驗撰寫器(VEC)的最佳實務，讓您的體驗如預期般運作。
...
下列最佳作法可協助讓您的體驗如預期般運作。在[!DNL Adobe Target]中使用視覺化體驗撰寫器(VEC)時，您也應注意其他提示和限制。

只要遵循下列這些最佳作法，您設計的體驗就比較不會發生非預期的問題。

## 最佳實務 {#section_86CF28C99CFF40329E4CBAFE4DD78BB4}

**將at.js參考放在頁面`<head>`區段的頂端。**
...
**新增多個頁面時，體驗邊欄和頁面邊欄會同時開啟。結果會縮短可視化體驗撰寫器以最佳方式顯示網站的寬度。因此，可重新整理的網站在減少的空間中，可能會開始出現與預期不同的外觀。**
