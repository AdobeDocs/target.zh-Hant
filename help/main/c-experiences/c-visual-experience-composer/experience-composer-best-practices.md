...
description: 在Adobe Target使用Visual Experience Composer(VEC)時，學習最佳實踐，使您的體驗按預期工作。
title: 什麼是Visual Experience Composer最佳做法和限制？
...
下列最佳作法可協助讓您的體驗如預期般運作。使用中的Visual Experience Composer(VEC)時，還應注意其他提示和限制 [!DNL Adobe Target]。

只要遵循下列這些最佳作法，您設計的體驗就比較不會發生非預期的問題。

## 最佳實務 {#section_86CF28C99CFF40329E4CBAFE4DD78BB4}

**將at.js引用置於 `<head>` 的子菜單。**

如果還使用訪問者API服務，請將訪問者API指令碼放在at.js上方。

**您可以在帳戶層級 (已針對在帳戶中建立的所有活動啟用) 或是在個別活動層級啟用增強體驗撰寫器。**

要在帳戶級別啟用增強的體驗作曲家，請按一下 [!UICONTROL 「管理」>「視覺體驗作曲家」]，然後將開關切換到「開啟」位置。

在可視化體驗撰寫器中建立活動時，若要在活動層級上啟用增強體驗撰寫器，請按一下「[!UICONTROL 設定 > URL]」，然後將開關切換至「開啟」位置。

**如果增強的視覺體驗作曲家不會在您的站點上的安全頁面上載入，則可以允許列出某些IP地址。**

通過允許列出以下IP地址，可以解決載入增強的Visual Experience Composer時出現的問題。 這些 IP 位址用於 Adobe 的伺服器，用於增強體驗撰寫器 Proxy。只有針對活動編輯才需要這些資訊。訪問您站點的訪問者不需要列出這些IP地址。
