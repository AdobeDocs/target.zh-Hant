...
[!UICONTROL 修改]頁面會顯示視覺化體驗撰寫器(VEC)中對您的頁面所做的所有變更，並可讓您按一下頁面上的每個元素並[選取動作](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#reference_3BD1BEEAFA584A749ED2D08F14732E81)以進行其他變更。 您所做的每項變更都顯示為[!UICONTROL 修改]清單中的個別動作或元素。 您也可以新增修改，包括以下修改類型: CSS 選取器、Mbox。 和自訂程式碼。
...
* 沿著 Target UI 的側邊垂直固定「修改」面板，或水平固定在底部。 按一下[!UICONTROL 「固定」]圖示即可切換兩種設定。
  ...
  ![codeditor_dock影像](assets/codeditor_dock.png)
...
1. 若要新增修改:
   ...
   * 如果之前對體驗已經過修改，請按一下右側[!UICONTROL 「修改」]面板頂部的 + 圖示。
   ...
   ![codeeditor_page_mods_add影像](assets/codeeditor_page_mods_add.png)
...
* 在 DOM 就緒時交換，使用 jQuery
  ...
  下列使用JQuery的範例假設當[!DNL Target]執行選件時，客戶的網站在頁面上有jQuery可用。
...
***我在指令碼中使用`document.write`建立的元素沒有出現在我預期的位置。***
