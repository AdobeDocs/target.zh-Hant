...
description: 瞭解如何疑難排解[!UICONTROL 視覺化體驗撰寫器] (VEC)中的問題。
...
## 當我在[!UICONTROL 視覺化體驗撰寫器]中開啟我的網站時，[!DNL Target]資料庫未載入。 (僅限 VEC) {#section_8A7D3F4AD2CC4C3B823EE9432B97E06F}

+++詳細資料
在[!UICONTROL 視覺化體驗撰寫器]中開啟網站時，[!DNL Target]新增兩個引數（`mboxEdit=1`和`mboxDisable=1`）。
...
如果[!UICONTROL 增強體驗撰寫器]發生問題，請嘗試關閉[!UICONTROL 增強體驗撰寫器]，並改用[!UICONTROL 視覺化體驗撰寫器]。

若要停用[!UICONTROL 增強體驗撰寫器]，請前往&#x200B;**[!UICONTROL 管理]** > **[!UICONTROL 視覺化體驗撰寫器]**，並關閉&#x200B;**[!UICONTROL 啟用增強體驗撰寫器]**&#x200B;選項。
...
如果[!UICONTROL 視覺化體驗撰寫器]和[!UICONTROL 增強體驗撰寫器]都無法運作，請使用類似[!DNL Requestly] （[!DNL Chrome]或[!DNL Firefox]）的瀏覽器延伸模組或修改回應標頭(Firefox)，此模組會覆寫網站的X-Frames標頭選項，並允許這些選項載入iFrames中，進而啟用VEC。 如果您無法使用瀏覽器延伸模組，請使用[表單式體驗撰寫器](/help/main/c-experiences/form-experience-composer.md)。

>[!NOTE]
>
>除了下列資訊外，您還可以使用[!DNL Google Chrome]的[[!DNL Adobe Target] [!UICONTROL Visual Editing Helper]擴充功能](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)。
...
>
>對於[!UICONTROL Modify Response Header]延伸模組([!DNL Firefox])，因為您無法新增URL規則，所以您必須執行下列動作：
>
>* 在 VEC 中編輯時啟用規則，並在不使用 VEC 時停用規則。

**若要在[!DNL Chrome]或[!DNL Firefox]上使用[!DNL Requestly]延伸模組：**

1. 關閉[!UICONTROL 增強體驗撰寫器]。
...
1. 按一下&#x200B;**[!UICONTROL 「儲存」]**。
   ...
   您現在應該可以使用[!UICONTROL 視覺化體驗撰寫器]快速載入頁面。

**若要在[!UICONTROL Firefox]上使用[!DNL Modify Response Headers]延伸模組：**

1. 在[!DNL Firefox]上安裝[!UICONTROL Modify回應標頭]，並重新啟動瀏覽器。
1. 從您的[!DNL Firefox]擴充功能中，選取Modify Response Headers擴充功能。
1. 按一下&#x200B;**[!UICONTROL 「偏好設定」]**。
1. 從[!UICONTROL 動作]下拉式清單中選取&#x200B;**[!UICONTROL 篩選器]**。
1. 在[!UICONTROL 標頭名稱]欄位中，輸入： **[!UICONTROL X-Frame-Options]**。
1. 重複步驟 4 和 5 來使用 **[!UICONTROL x-frame-options]** 新增篩選。
1. 按一下「**[!UICONTROL 新增]**」。
1. 按一下&#x200B;**[!UICONTROL 「開始」]**。

![Firefox擴充功能](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/assets/firefox_extension.png)

設定擴充功能後，請開啟[!DNL Target]。 您的頁面現在應在[!UICONTROL 視覺化體驗撰寫器]中載入，即使[!UICONTROL 增強體驗撰寫器]已停用。
...
+++詳細資料
* 最新版本的擴充功能可確保與VEC的最佳相容性： [[!DNL Adobe Experience Cloud] [!UICONTROL Visual Editing Helper擴充功能]](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)。

  若要確認您是否使用最新版本，請前往[!UICONTROL 擴充功能] > [!UICONTROL 管理擴充功能]，然後按一下[!UICONTROL 詳細資料]。

* [!UICONTROL 視覺化體驗撰寫器]需要編寫程式庫，才能在網頁上執行修改。 這些程式庫內嵌於at.js程式庫中，並在每次使用VEC時由擴充功能從[!DNL Adobe]伺服器下載。

  無論at.js或[!DNL Adobe Experience Platform Web SDK]是否已包含在頁面中，擴充功能都會下載at.js資料庫。

  確保未將無效的變更新增到[!UICONTROL 管理] > [!UICONTROL 實作]區段中設定的at.js標頭。
...
## [!UICONTROL 視覺化體驗撰寫器]中的CSS衝突所造成的問題
