...
description: 探索為什麼有些網站可能無法可靠地在可視化體驗撰寫器 (VEC) 中開啟。 VEC Helper瀏覽器擴充功能可讓您可靠地在VEC內載入網站。
title: 如何使用視覺化體驗撰寫器(VEC) Helper擴充功能？
...
此 [!DNL Adobe Target] [!UICONTROL 視覺化體驗撰寫器] (VEC) Google Chrome的Helper瀏覽器擴充功能可讓您在VEC內以可靠的方式載入網站，來迅速撰寫體驗和保證體驗品質。

VEC Helper瀏覽器是Chrome擴充功能。 使用Mozilla Firefox時不需要此擴充功能。

>[!IMPORTANT]
>
>目前 [!DNL Target] 本文記錄的VEC Helper擴充功能是使用Manifest v2建立的。 Google最近宣佈不再允許使用Manifest v2建立的新擴充功能。
>
>現有的擴充功能仍可在Google Chrome中運作。 未來， [!DNL Adobe] 將淘汰本主題中說明的helper擴充功能，並要求客戶改用較新的版本 [Visual Editing Helper擴充功能](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md). 當此擴充功能停止運作時，您將透過發行說明和本文中的文字收到通知。 不過，由於Manifest v3中的安全性增強功能， [!DNL Adobe] 建議下載新的擴充功能，以繼續在中以視覺方式創作您的網站 [!DNL Target].
...
* 嘗試使用VEC開啟使用中的網站時，目前有一些限制 [服務背景工作](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API){target=_blank} （軟體）。
...
很可惜，攔截網路請求的Chrome擴充功能API不會收到已由SW攔截和處理的請求。 因此，如果網頁請求是由SW從快取中提供服務，則擴充功能無法修正標頭和Cookie，因為同時快取了X-Frame-Options或CSP標頭，網頁將不會載入VEC內。
...
適用於Chrome的VEC Helper瀏覽器擴充功能解決客戶現依賴 [!DNL Target] [增強體驗撰寫器](/help/main/administrating-target/visual-experience-composer-set-up.md#eec) 或協力廠商擴充功能，例如Requestly。
...
1. （視條件而定）滑動 **[!UICONTROL 插入Target資料庫]** 如果網頁尚未包含 [!DNL Target] at.js JavaScript資料庫。
...
1. （視條件而定）滑動 **[!UICONTROL Cookie]** 切換至「開啟」位置以自動新增 `SameSite=None` 屬性瀏覽器修正。
...
* 此擴充功能會載入最新版本的at.js （由以下網址提供）： [!DNL Target UI] 在 [!UICONTROL 管理>實作].
