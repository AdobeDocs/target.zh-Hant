...
description: 探索為什麼有些網站可能無法可靠地在[!UICONTROL 視覺化體驗撰寫器] (VEC)中開啟。 VEC Helper瀏覽器擴充功能可讓您可靠地在VEC內載入網站。
...
1. 按一下&#x200B;**[!UICONTROL 新增至Chrome >新增擴充功能]**。
...
1. （視條件而定）如果網頁尚未包含[!DNL Target] at.js JavaScript資料庫，請將「**[!UICONTROL 插入Target資料庫]**」切換滑至「開啟」位置。

   下圖顯示已啟用[!UICONTROL 插入Target資料庫]設定的VEC Helper：
...
1. （條件式）將&#x200B;**[!UICONTROL Cookie]**&#x200B;切換滑至「開啟」位置，以自動新增`SameSite=None`屬性瀏覽器修正。
...
* 擴充功能中的[!UICONTROL 插入Target資料庫]旗標預設為「關閉」。 如果您要在尚未針對 [!DNL Target] 實作的網站上使用 VEC，可以啟用此標記。
...
* 此擴充功能會載入最新版本的at.js，此版本可從[!UICONTROL 管理>實作]中的[!DNL Target UI]取得。
