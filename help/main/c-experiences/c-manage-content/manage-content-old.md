...
1. （選擇性）將游標停留在[!UICONTROL 代碼選件]索引標籤上所需的選件或資料夾上，然後按一下所需的圖示，以執行其他工作。
   ...
   * 編輯
   * 複製
   * 移動（例如，若要將一或多個專案移至資料夾，請按一下所需專案的&#x200B;**[!UICONTROL 移動]**&#x200B;圖示，按一下所需的資料夾，然後按一下&#x200B;**[!UICONTROL 放置]**。）
   * 刪除
...
1. （選擇性）將游標停留在[!UICONTROL 影像選件]索引標籤上所需的影像選件或資料夾上，然後按一下所需的圖示，以執行其他工作。
...
您可以在[!UICONTROL 選件]資料庫中的快顯示卡片上檢視選件定義詳細資料，而不需要開啟選件。
...
按一下[!UICONTROL 檢視完整詳細資料]連結以檢視選件內容以及參考代碼選件的活動。 這樣可讓您在編輯產品建議時，避免影響其他活動。 資訊包括[!UICONTROL 已上線的活動]和[!UICONTROL 非使用中的活動]。
