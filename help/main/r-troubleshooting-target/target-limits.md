...
* **上限**：256 個字元。
  ...
  使用at.js 1.*x*&#x200B;時，超過256個字元的值會遭截斷。 使用at.js 2.*x*&#x200B;或[!DNL Adobe Experience Platform Web SDK]時，在傳送超過256個字元的值時，您會收到錯誤訊息。 值不會自動截斷。
...
* **上限**：250 個字元。
  ...
  對於[!DNL Delivery API] (at.js 2.*x*)、批次mbox V2和[!DNL Adobe Experience Platform Web SDK] (alloy.js)整合，mbox名稱&#x200B;*可以*&#x200B;包含英數字元(A-Z、a-z、0-9)和下列任一字元：

  ```
  - , . _ / = ` : ; & ! @ # $ % ^ & * ( ) _ + | ? ~ [ ] { }
  ```

  對於at.js 1.*x*&#x200B;整合，mbox名稱&#x200B;*不可*&#x200B;包含下列任何字元：
