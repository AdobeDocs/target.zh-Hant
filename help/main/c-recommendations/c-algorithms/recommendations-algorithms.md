...
* [!UICONTROL 依設定檔屬性檢視次數最多]和[!UICONTROL 依設定檔屬性最暢銷商品]也依彙總檢視或購買計數來排名專案，但群組索引鍵是訪客設定檔屬性，例如國家/地區、或會員層級，而不是專案屬性。 每個工作階段的計數會歸因於訪客在互動時持有的任何設定檔值，而不是歸因於專案的任何屬性。 結果是一組以該設定檔屬性的不同值作為關鍵字的建議清單。
