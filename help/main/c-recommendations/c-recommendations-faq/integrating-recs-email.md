...
大多數電子郵件使用者端不允許POST要求。 因此，不建議將此API用於開放時間使用案例。 部分電子郵件用戶端 (例如 Gmail 或 Outlook) 可能會快取內容或封鎖影像，並要求收件者主動允許影像呈現。
...
| `mbox` | *mboxName* | 用於鎖定目標的 mbox 名稱。 | 與所有mbox呼叫相同的驗證。<br>250個字元限制。<br>不能包含下列任一個字元：`', ", %22, %27, <, >, %3C, %3E` |
...
| `mboxDefault` | *`https://www.default.com`* | 如果 `mboxNoRedirect` 參數不存在，則在沒有可用的建議時，`mboxDefault` 應該是將傳回預設內容的絕對 URL。 此 URL 可以是影像或其他靜態內容。<br>如果`mboxNoRedirect`引數存在，`mboxDefault`可以是任何文字，指出沒有任何建議，例如`no_content`。<br>電子郵件提供者必須處理傳回此值的情況，並將預設HTML插入電子郵件中。<br> **安全性最佳做法**：如果不允許列出 `mboxDefault` URL 中使用的網域，則可能會面臨「開啟重新導向弱點」的風險。 為避免第三方未授權使用重導程式連結或 `mboxDefault`，Adobe 建議您使用「已授權的主機」來允許列出預設的重新導向 URL 網域。 Target 會使用主機來允許列出您想要允許重新導向的網域。 如需詳細資訊，請參閱「[建立允許清單，指定在 *Hosts* 中授權傳送 mbox 呼叫至  [!DNL Target]](/help/main/administrating-target/hosts.md#allowlist) 的主機」。 |  |
| `mboxHost` | *mbox_host* | 這是呼叫觸發時新增至預設環境 (主機群組) 的網域。 |  |
| `mboxPC` | 空白 | （使用訪客設定檔的建議為必要。）<br>如果未提供&quot;thirdPartyId&quot;，則會產生新的tntId，並隨著回應傳回。 否則會保持空白。<br>**注意**：請務必為每個電子郵件收件者（亦即每個API呼叫）提供唯一值`mboxSession`和`mboxPC`。 如果您沒有為這些欄位提供唯一值，API 回應可能會因為在單一輪廓中產生的大量事件而變得緩慢或失敗。 | 1 &lt; 長度 &lt; 128<br>不能包含超過一個「.」 （點）。<br>只允許使用點作為設定檔位置字尾。 |

### 選用參數

| 參數 | 值 | 說明 | 驗證 |
|--- |--- |--- |--- |
| `mboxPC`<br>(可選) | *mboxPCId* | Target 訪客 ID。 如果您想要追蹤使用者在多次造訪間完全回到您的網站，或使用使用者設定檔參數時，請使用此值。<br>此值必須是使用者的實際 [!DNL Adobe Target] PCID，它會從網站匯出至您的 CRM。 電子郵件提供者會從您的 CRM 或 Data Warehouse 擷取此 ID，並使用它做為此參數的值。<br>當建議屬於A/B活動時，`mboxPC`值對於追蹤多次造訪間的度量追蹤的訪客網站行為也很實用。<br>**注意**：請務必為每個電子郵件收件者（亦即每個API呼叫）提供唯一的`mboxSession`和`mboxPC`值。 如果您沒有為這些欄位提供唯一值，API 回應可能會因為在單一輪廓中產生的大量事件而變得緩慢或失敗。 | 1 &lt; 長度 &lt; 128<br>不能包含超過一個「.」 （點）。<br>只允許使用點作為設定檔位置字尾。 |
