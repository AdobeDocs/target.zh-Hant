...
本頁記錄在 [!DNL Adobe Target]. 提供轉換率、轉換率的信賴區間、提升度、提升度的信賴區間和信賴度的定義。

![顯示轉換率、平均提升度和信賴區間以及A/B測試活動信賴度的Target報表。](/help/main/c-reports/statistical-methodology/img/target_report.png)
...
* 單位總和 *i* 取決於計數方法的選擇。
   ...
   * 若 *訪客* 作為計數方法使用，每個單位都是在活動期間定義為活動中唯一參與者的獨特訪客。
   * 若 *瀏覽* 作為計數方法，每個單位都是獨特造訪，定義為在 [!DNL Target] 工作階段(具有唯一 `sessionId`)。 當 `sessionId` 變更，或訪客達到轉換步驟，則會計入新瀏覽。
   * 若 *活動曝光數* 作為計數方法，每個單位都是獨特的曝光，定義為訪客每次載入活動的任何頁面時。

## 平均/轉換率的信賴區間

轉換率的信賴區間被直觀地定義為與基礎資料一致的可能轉換率範圍。

執行實驗時，我們針對特定體驗所觀察的轉換率為 *估計* 的值。 為了量化此估計中的不確定性，我們可以使用置信區間。 [!DNL Target] 一律會報告95%信賴區間，這表示從長遠來看，95%計算的信賴區間包含體驗的真正轉換率。
...
下圖顯示提升度和信賴區間，位於 [!DNL Target] 報告。 數字代表提升度界限的平均範圍，而如果提升度為正或負，箭頭會反映。 箭頭會以灰色顯示，直到信賴度超過95%為止。 信賴度超過臨界值後，箭頭會根據正或負提升度而變成綠色或紅色。
...
中的盒形圖 [!UICONTROL 平均提升度和信賴區間] 欄代表平均值和95%提升度信賴區間。 當給定非控制體驗與控制體驗的信賴區間有任何重疊時，盒形圖會呈灰色；當給定體驗的信賴區間範圍高於或低於控制體驗的信賴區間時，盒形圖會呈綠色或紅色。
...
最後一欄顯示 [!DNL Target] 報表。 體驗的信賴度是取得結果的機率（以百分比表示），比實際觀察到的結果（假設為Null假設為true）還小。 就p值而言，顯示的信賴度為 *1 - p值*. 直覺上，較高的信賴度表示控制與非控制體驗有相同轉換率的可能性較低。
