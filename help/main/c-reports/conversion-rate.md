---
keywords: 目標定位
description: 瞭解如何Adobe [!DNL Target] 顯示並計算每個體驗的轉換率、提升度、信賴度和信賴區間。
...
![轉換率 — 詳細影像](assets/conversion-rate-detail.jpg)
...
如果您插入「下訂單」(`orderConfirmPage`) mbox並選取它作為轉換mbox。
...
如果空值假設為真（實質上，如果該體驗或選件與控制體驗/選件之間的轉換率沒有差異），則顯示的體驗或選件的信賴度是取得比實際觀察到的結果更少極端值的機率（以百分比表示）。 就p值而言，顯示的信賴度為1 - p值。 更簡單地說，較高的信賴度表示資料不太符合控制和非控制選件/體驗具有相等轉換率的假設。

可信度大於或等於 99.995% 時會四捨五入為 100.00%。

![conf_report影像](assets/conf_report.png)  ![conf_report_detail圖片](assets/conf_report_detail.png)
...
此 *信賴區間* 是可以在指定信賴等級找到量度真實值的預估範圍。 Target一律會顯示95%信賴區間。 信賴區間在「轉換率」欄中以淺灰色 +/- 百分比出現。在下列範例中，體驗 B 提升度的信賴區間會加或減 15.65%。

![conversion_rate影像](assets/conversion_rate.png)

**範例：** 觀察到的體驗的RPV為$10，而其95% **信賴區間** 5到15美元。 我們不知道，其真正的RPV為$12。 然後，如果我們執行此測試多次，則我們計算的信賴區間的95%時間將包含 _true_ RPV值$12。
...
[!DNL Target] 建議使用 [韋爾奇的t檢定](https://en.wikipedia.org/wiki/Welch%27s_t-test)，因為這些測試需要的假設比z測試之類的替代測試少，且是適當的統計測試，可用於在控制體驗和替代體驗之間進行（量化）業務量度的成對比較。

### 詳細資訊

執行線上A/B測試時，每個使用者/訪客都會隨機指派給單一變體。 接著，我們測量感興趣的業務量度（例如轉換、訂購、收入等） 適用於每個變體中的訪客。 我們使用的統計測試接著會測試平均商業量度（例如轉換率、每位使用者的訂單、每位使用者的收入等）的假設， 與控制項和指定的替代變體相同。

雖然商業量度本身可能會根據某些任意的分佈進行分佈，但此量度平均值（在每個變體內）的分佈應透過 [中心極限定理](https://en.wikipedia.org/wiki/Central_limit_theorem). 請注意，雖然無法保證此平均值的抽樣分佈會以多快的速度收斂至正常，但一般會根據線上測試中的訪客規模來達成此條件。

假設平均值為常態，測試統計值可能會顯示成遵循t分佈，因為這是常態分佈值（商業量度之平均值差異）與根據資料估計值之縮放字詞的比率（平均值差異的標準誤差）。 此 **t檢定** 則為適當的假設測試，假設測試統計量遵循t分佈。

### 為什麼沒有使用其他測試

A **z檢定** 技術上不合適，因為在典型的A/B測試案例中，測試統計資料的分母不是衍生自已知變數，而是必須從資料中估計。 然而，對於足夠大的樣本量，z檢定和t檢定是相同的。

**卡方檢驗** 不使用，因為這些引數適用於判斷兩個變體之間是否有質化關係（亦即變體之間沒有差異的null假設）。 T檢視更適合用於的情況 _量化_ 比較量度。

此 **Mann-Whitney U測試** 是一種非引數測試，適用於平均商業量度（適用於每個變體）的取樣分佈不是常態分佈的情況。 不過，如先前所述，考慮到線上測試中涉及的流量大小，通常適用中央限制定理，因此可以安全地套用t測試。

更複雜的方法，例如 **ANOVA** （將t測試泛化成兩個以上的變體）可在測試具有兩個以上的體驗（「A/Bn測試」）時套用。 然而，ANOVA回答了「是否所有變體具有相同的平均值」的問題，而在典型的A/Bn測試中，我們更感興趣 _哪個特定變體_ 是最好的。 在 [!DNL Target]因此，我們應用常規t測試來比較每個變體與控制項，並以Bonferroni校正來說明多個比較。