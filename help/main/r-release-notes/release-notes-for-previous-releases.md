...
* **[!DNL A/B Test (Manual)]活動的Bayesian統計資料** [!DNL A/B Test (Manual)]活動現在支援Bayesian統計資料作為Welch的t檢定的替代方法。 在[!UICONTROL 目標與設定]中選擇統計方法：貝葉斯活動的報告會顯示以機率為基礎的決策量度，例如[!UICONTROL 成功控制的機會]，以及可信的區間，而不是頻率信賴區間。

* **[!DNL Customer Journey Analytics]報告[!DNL Auto-Allocate]個活動。** 除了[!DNL Target]和[!DNL Adobe Analytics] (A4T)之外，[!DNL Auto-Allocate]活動現在還可以使用[!DNL Customer Journey Analytics]作為其報告來源。
...
* 允許的URL的&#x200B;**Regex驗證。** 修正在&#x200B;**[!UICONTROL 允許的URL]**&#x200B;欄位中輸入無效規則運算式而沒有正確驗證的問題。 (TGT-55095)
...
* 在[!UICONTROL 編輯對象]對話方塊中的&#x200B;**按鈕。** 修正&#x200B;**[!UICONTROL 編輯對象]**&#x200B;對話方塊中的按鈕不再正確呈現的問題。 (TGT-54638)
...
* **[!UICONTROL 圖表檢視]與資料表及下載不同步。** 修正某些日期範圍的活動報表在&#x200B;**[!UICONTROL 圖表檢視]**&#x200B;中可能顯示缺少或零量度的問題，即使&#x200B;**[!UICONTROL 表格檢視]**&#x200B;和下載的報表仍顯示正確的值。 (TGT-54998)
...
* **對象使用清單未完全轉譯。** 修正對象詳細資料中的&#x200B;**[!UICONTROL 使用狀況]**&#x200B;區段可能只顯示對應活動子集的問題，即使其他活動與該對象相關聯。 (TGT-55094)
...
* **確認最後一個八位元IP模糊化的效果更清楚。** 當您在&#x200B;**[!UICONTROL 管理]** > **[!UICONTROL 實作]**&#x200B;上將&#x200B;**[!UICONTROL 模糊化訪客IP位址]**&#x200B;變更為&#x200B;**[!UICONTROL 最後一個八位元]**&#x200B;時，確認對話方塊現在會說明[!DNL Target]會隱藏訪客IP位址的最後一個八位元。 (TGT-44821)
...
* **使用增強體驗撰寫器(EEC)的空白或不完整頁面。** 修正啟用&#x200B;**[!UICONTROL 增強體驗撰寫器]**&#x200B;時，[!UICONTROL 視覺化體驗撰寫器]無法在編輯器中載入網站的問題。 (TGT-54576)
...
* **報表中的對象篩選器發生錯誤。** 修正在&#x200B;**[!UICONTROL 目標與設定]**&#x200B;中變更對象篩選條件，導致[!DNL Target]使用者介面的「報表」區段發生錯誤的問題。 (TGT-55006)

* **依優先順序排序活動。** 已使用&#x200B;**[!UICONTROL 優先順序]**&#x200B;欄標題在活動清單上新增依優先順序排序，其遞增和遞減順序與其他可排序的欄一致。 (TGT-54948)

* **其他活動屬性在儲存後未保留。** 修正儲存及重新開啟活動後，某些&#x200B;**[!UICONTROL 屬性]**&#x200B;選取範圍未持續存在的問題。 (TGT-53889)
...
* [!UICONTROL 頁面傳送]規則運運算元的&#x200B;**日文標籤。** 修正日文UI中頁面傳送規則運運算元標籤無法讀取或損毀的字串。 (TGT-53097)
...
* **編輯器中錯誤體驗上顯示的修改。** 修正在[!UICONTROL 視覺化體驗撰寫器]中的體驗之間切換後，刪除或其他修改可能出現在錯誤體驗上的問題。 (TGT-54955)

* **刪除插入HTML時移除的修改。** 修正刪除額外的&#x200B;**[!UICONTROL HTML]**&#x200B;區塊（使用&#x200B;**[!UICONTROL 插入在]**&#x200B;之前」或&#x200B;**[!UICONTROL 插入在]**&#x200B;之後）時，也移除沒有CSS選取器的連結修改的問題。 (TGT-54530)
...
* **某些活動中遺漏位置。** 修正某些活動中缺少&#x200B;**[!UICONTROL 位置]**&#x200B;的問題。 (TGT-54951)
...
* **套用至其他檢視時保留自訂程式碼。** 修正在相同&#x200B;**[!UICONTROL 活動]**&#x200B;中為另一個&#x200B;**[!UICONTROL 檢視]**&#x200B;新增或儲存自訂程式碼時，套用至一個&#x200B;**[!UICONTROL 檢視]**&#x200B;的自訂程式碼可能會被移除的問題。 (TGT-53933)
+++
...
* **在「活動」檢視中顯示的對象屬性。** 修正從&#x200B;**[!UICONTROL 活動]**&#x200B;檢視的對象規則詳細資料，未顯示從&#x200B;**[!UICONTROL 對象]**&#x200B;區段開啟相同對象時顯示的特定屬性的問題。 (TGT-54742)

* **匯出[活動]清單頁面上的CSV。** 已新增&#x200B;**[!UICONTROL 匯出CSV]**&#x200B;動作，以便您可從使用者介面匯出活動清單（包括套用篩選器時），而不只是依賴API進行例行匯出。 (TGT-51466)

* 找不到選取器時標籤的&#x200B;**體驗修改。** 體驗修改現在會執行選擇器存在性檢查；在頁面上找不到選擇器時，修改會標籤為無效。 (TGT-54815)

* **[!UICONTROL 自動個人化]活動。** 修正介面和活動載入問題，此問題會讓使用者無法可靠地建立、編輯或管理Automated Personalization活動，進而封鎖行銷活動設定，並延遲個人化使用案例。 (TGT-54421)
...
* **從活動建立對象時，可以看到對象名稱和說明。** 修正從活動流程建立或編輯對象時，對象&#x200B;**[!UICONTROL 名稱]**&#x200B;和&#x200B;**[!UICONTROL 描述]**&#x200B;欄位與直接在&#x200B;**[!UICONTROL 對象]**&#x200B;下建立對象相比，未明確顯示的問題。 (TGT-54837)
...
* **[!UICONTROL 已上線的活動]計入深入分析。** 修正見解儀表板上的&#x200B;**[!UICONTROL 已上線活動]**&#x200B;量度報告的總數可能高於在&#x200B;**[!UICONTROL 所有活動]**&#x200B;中顯示為已上線的活動數的問題。 (TGT-54788)
...
* [!UICONTROL 全域排除]中的&#x200B;**長ID清單。** 修正在&#x200B;**[!UICONTROL 全域排除專案]**&#x200B;中貼上或輸入長清單ID時，在更新介面中與舊版相比可能會遭截斷，導致排除清單不完整的問題。 (TGT-54422)
...
* [!UICONTROL 視覺化體驗撰寫器]中的&#x200B;**增強體驗撰寫器(EEC)狀態指標。** EEC指標代表是否啟用增強體驗撰寫器。 其顯示方式已經過修訂，因此不再類似互動式切換，因為它僅作為非互動式狀態顯示。 (TGT-54828)

* [!UICONTROL 視覺化體驗撰寫器]中的&#x200B;**可收合左側邊欄** 現在當活動開啟進行編輯時，可以收合左側邊欄。 這可改善包含多個對象和頁面的活動（包括在較小的顯示器上）對&#x200B;**[!UICONTROL 元件]**&#x200B;和&#x200B;**[!UICONTROL 屬性]**&#x200B;的存取權。 (TGT-54269)
...
* 視覺化體驗撰寫器(Chrome)中的&#x200B;**私人網路URL。** 除非允許本機網路存取，否則Chrome可以封鎖私人網路上的頁面。 **[!UICONTROL 視覺化體驗撰寫器]**&#x200B;現在會要求框架化Experience Cloud中使用的許可權，因此Chrome可以顯示允許提示而不是無訊息地失敗。 在您允許存取後，當您的環境支援內部URL時，VEC中可以載入該URL。 (TGT-54346)
...
* **在[實作]畫面中新增忽隱忽現的管理切換。** 已新增切換至[!UICONTROL 實作]畫面，以控制啟用忽隱忽現管理設定。 此切換可讓管理員直接從「實施」畫面設定忽隱忽現管理。 (TGT-52247)
...
* **在概觀頁面中顯示對象和體驗的全名。** 此增強功能會更新[!UICONTROL 總覽]頁面，以顯示對象和體驗的完整名稱。 以前，長名稱會被截斷，而且無法完全顯示，因此使用者必須按三下才能選取所有文字來檢視完整名稱。 此更新可確保看到完整的對象和體驗名稱，讓使用者更容易識別和檢閱活動設定。 (TGT-53323)
...
* **已加入允許清單的URL管理功能。** 已在[!UICONTROL 管理]區段中新增一個「已加入允許清單的URL」管理功能，以管理允許的遠端選件URL模式。 此功能包括在表格中顯示允許列出的URL、搜尋和篩選URL、透過模組對話方塊新增新的URL以及在確認後刪除URL的功能。 此功能可協助客戶控制哪些URL可用於遠端選件，進而改善安全性和合規性。 (TGT-54486)
...
* **啟用Web編輯器選件的Workspace欄。** 此更新解決來自[!UICONTROL 預設Workspace]的選件出現在網頁編輯器中的其他工作區所造成的客戶混淆。 雖然此行為如預期般運作，但[!UICONTROL 預設Workspace]選件會在所有工作區中刻意顯示，客戶回報UI未明確工作區來源，尤其是在非預設工作區（例如「核准者」）中建立活動時更是如此。 為了提高清晰度，[!UICONTROL Workspace]欄現在已在網頁編輯器的選件清單中啟用，可讓使用者輕鬆區分每個選件屬於哪個工作區，並防止誤解顯示的其他選件。 (TGT-54138)
* 在新標籤中開啟具有target=&quot;_blank&quot;的&#x200B;**連結。** 此修正解決包含具有~target=&quot;_blank&quot;~之連結的已編寫網站在按一下[!UICONTROL 瀏覽]模式時，會在新的瀏覽器標籤中開啟，中斷編輯器內預覽體驗的問題。 發生此行為是因為編寫頁面的原生連結屬性沒有受到擴充功能插入的JavaScript攔截，不像在舊版UI中，錨點元素會進行轉換，且其目標會覆寫，以保留在編輯器內的導覽。 更新可確保使用~target=&quot;_blank&quot;~的連結現在可在網頁編輯器中正確處理，因此在編寫期間不再開啟外部索引標籤。 (TGT-54134)
...
* **[!UICONTROL 工作區]下拉式清單在[!UICONTROL 使用者]區段中限製為20個。** 此修正解決了[!UICONTROL 管理] > [!UICONTROL 使用者]區段中的[!UICONTROL 工作區]下拉式清單僅顯示20個工作區的問題，即使使用者擁有更多存取權。 `licenseGroups`的基礎GraphQL呼叫也限製為20個結果，導致使用者擁有組織中更多工作區的存取權，UI仍會顯示不完整的清單。 更新會移除此硬性限制，因此現在會傳回完整的可用工作區集，並正確顯示。 (TGT-53820)
* **修正選件強制回應視窗未顯示Workspace欄的問題。** 修正優惠模式在更新的UI中未顯示Workspace欄的問題。 這會導致客戶感到困惑，因為來自[!UICONTROL 預設Workspace]的選件會與來自所選工作區的選件一起出現，而不會指出其來源。 工作區欄現已啟用，客戶可清楚識別每個選件所屬的工作區。 (TGT-52320)
...
* **如果已移除，Activity Edit不應該新增自動偵測的屬性。** 此修正解決編輯活動會自動重新引入使用者先前已移除的自動偵測屬性的問題。 重新開啟活動以進行編輯時，系統錯誤地還原了移除的屬性，導致[!UICONTROL 屬性清單]中的行為不一致和混淆。 此更新可確保在移除自動偵測到的屬性後，該屬性會在所有後續編輯期間保持被移除狀態，且除非使用者明確將其新增回，否則其不會重新出現。 (TGT-54182)
* **如果已移除，則不新增自動偵測的屬性。** 此修正可確保一旦使用者手動從活動中移除自動偵測到的屬性後，系統就不會在活動編輯器內的後續導覽期間重新引入該屬性。 先前，如果使用者取消選取自動偵測的屬性，並移至[!UICONTROL 鎖定目標]步驟，然後返回[!UICONTROL 體驗]，則編輯器會根據活動編輯器狀態片段中儲存的自動偵測清單重新填入移除的屬性。 更新後的邏輯現在會將自動偵測到的屬性與~ActivityState~磁碟片段中的目前屬性進行比較，並防止重新加入使用者已移除的任何自動偵測到的屬性。 這會產生跨步驟的一致行為並尊重使用者意圖。 (TGT-54181)
* **將自動偵測的文字加入屬性清單。** 此增強功能會更新[!UICONTROL 屬性清單]，以清楚標示系統自動偵測到的任何屬性。 當自動偵測的屬性也出現在使用者可見的[!UICONTROL 屬性清單]中時，它現在會使用~ActivityEditorSlice~狀態中儲存的值，在其名稱旁邊顯示「（自動偵測）」文字。 這反映了舊版UI的行為，並幫助使用者輕鬆區分手動選取的屬性和自動識別的屬性。 (TGT-54120)
* **將自動偵測到的[!UICONTROL 屬性]新增至狀態。** 此更新可確保~ActivityEditorSlice.ExperienceEditor~狀態一致地維護所有從Web編輯器傳遞到Activity [!UICONTROL Experience]索引標籤的自動偵測屬性ID的最新清單。 每次使用者導覽至[!UICONTROL 體驗]索引標籤時，都會使用任何新偵測到的屬性重新整理狀態，同時避免重複專案，確保準確追蹤且可靠的下遊行為。 (TGT-54119)
...
* **[!UICONTROL 環境]下拉式清單只顯示100個結果。** 此修正解決具有超過100個環境的客戶在[!UICONTROL Recommendations]內的[!UICONTROL 環境]下拉式清單中只能看到前100個專案的限制。 基礎GraphQL查詢(~getEnvironmentsV2~)已使用100的硬式編碼頁面大小進行分頁，導致UI即使有其他頁面可用，也只顯示部分清單。 對於擁有超過100個環境的客戶，此問題會導致缺少選項和不完整的選擇體驗。 更新會增加上限，因此所有環境都會傳回和顯示，確保完整可見性（無論環境計數為何）。 (TGT-53903)
...
* **修正[!UICONTROL 報告]箭頭未明確表示可擴充欄的問題。** 修正報表表格未清楚顯示其他欄可在更新的UI中展開的問題。 在欄標題附近的[!UICONTROL 報表]箭頭新增了消失的工具提示，以幫助客戶瞭解有更多欄可用。 (TGT-53479)
...
* **[!UICONTROL 訊息]欄間歇性地無法在更新的UI中顯示產品資料。** 已解決在更新的[!UICONTROL Recommendations] UI中，[!UICONTROL 目錄搜尋]中的[!UICONTROL 訊息]欄間歇性地無法顯示產品資料的問題，即使摘要中存在值亦然。 欄現在會一致地顯示所有產品的正確訊息值，確保可靠的可見性，而不需要手動重新設定欄。 (TGT-52777)
* 在更新的UI中儲存活動後，**[!UICONTROL 下載Recommendations資料]按鈕不可見。** 解決在更新的UI中，即使重新儲存後，[!UICONTROL 下載建議資料]按鈕未針對某些已儲存活動顯示的問題。 現在，所有活動中都會一致地顯示按鈕，以確保使用者可以可靠地匯出建議資料，而不需要因應措施。 (TGT-53802)
...
* 在更新的UI中使用[!DNL Adobe Analytics]作為報告來源時，出現&#x200B;**[!UICONTROL 目標與設定]錯誤訊息。** 解決更新的[!UICONTROL 總覽] UI中，目標區段顯示「發生錯誤」錯誤的問題。 我們無法完成您的要求。 當選取[!DNL Adobe Analytics] (A4T)作為報表來源時，如果問題仍然存在，請聯絡[!DNL Adobe Client Care]。 目標現在可正確顯示為[!UICONTROL Adobe Analytics]量度，以確保跨報表來源的一致可見性。 (TGT-54021)
...
* **無法在更新的UI中編輯或取代決策選件。** 解決在更新的UI中，無法透過[!UICONTROL 修改]面板編輯或取代決策優惠的問題，且優惠名稱顯示為空白。 決策優惠現在可完全存取及編輯，恢復與舊版UI的同等功能，並確保客戶可直接在活動中管理優惠。 (TGT-53884)
...
* **[!UICONTROL 多變數測試] (MVT)位置和圖表報告問題阻止產生報告。** 解決MVT活動無法在Target UI中產生[!UICONTROL 位置貢獻]和圖表報表的問題，顯示「發生錯誤」錯誤。 我們無法完成您的要求。」 現在，報表可在UI中正確載入，確保完整可見性。 (TGT-53654)
* **MVT報告未載入，因為[!UICONTROL 元素]貢獻報告錯誤。** 修正MVT活動報表無法在Target UI中載入，並顯示「無法擷取元素貢獻報表」錯誤的問題。 現在，報表可正確顯示，以確保元素貢獻的完整可見性。 (TGT-53691)
* **匯出訂單詳細資料至[!UICONTROL 體驗鎖定目標] (XT)活動的CSV問題。** 修正XT活動的[!UICONTROL 匯出訂單詳細資料至CSV]選項未正確顯示並傳回空白檔案的問題。 現在僅針對AP活動顯示選項，以確保精確的匯出功能並防止混淆。 (TGT-53798)
...
* **[!UICONTROL 刪除修改]按鈕問題導致無法移除活動修改。** 解決[!DNL Target] UI中的[!UICONTROL 刪除修改]按鈕無法運作，導致使用者無法移除活動內的修改的問題。 按鈕現在可如預期運作，允許可靠刪除修改，而不會延遲。 (TGT-53728)
...
* **[!UICONTROL 編輯]和[!UICONTROL 概觀]頁面之間的活動位置對齊方式不符。** 解決[!UICONTROL 總覽]頁面中的活動位置編號與[!UICONTROL 編輯體驗]頁面中的更新不符的問題。 現在，兩個檢視中的位置會保持一致，以確保精確的對齊並防止位置遺失或編號錯誤。 (TGT-53960 和 TGT-53954)
* **無法在更新的VEC中切換回[!UICONTROL 設計]模式。** 解決更新的VEC UI中，使用者在[!UICONTROL 瀏覽]模式下導覽至新頁面後，無法切換回[!UICONTROL 設計]模式的問題。 [!UICONTROL 設計]切換功能現在可以正常運作，讓修改可以順暢地套用至各頁面。 (TGT-53988 和 TGT-53993)
* **查詢引數未顯示在活動概觀中。** 已解決更新的UI中，查詢引數未顯示在活動的[!UICONTROL 概覽]頁面中，而導致[!UICONTROL 概覽]和頁面傳送URL之間不一致的問題。 現在，查詢引數可正確顯示，以確保活動位置可完全呈現，且在檢視間保持一致。 (TGT-53701)
...
+++ 檢視詳細資料
* **已解決更新的UI中的可用性問題**。 [!UICONTROL 觀察者]現在可以使用[!UICONTROL 檢視活動]選項預覽活動，就像在舊版UI中一樣。 (TGT-51741)
* **[!UICONTROL 觀察者]使用者現在可以在更新的UI中檢視活動內容。** 已還原觀察者角色使用者在更新後的活動UI中的可見性。 以前，觀察者無法檢視舊版UI中可用的修改、選件和內容變更。 (TGT-53785)
* **[!UICONTROL 核准者]使用者現在可以編輯活動目標，而不會發生編輯器許可權錯誤。** 解決「活動建立」UI中阻止核准者層級使用者儲存進階目標設定變更的許可權問題。 受影響的使用者收到`403 Forbidden.Resource`錯誤，需要編輯器許可權，儘管具有足夠的存取許可權。 (TGT-53819)
...
* **已在「僅限此活動」報表中還原多個對象選擇。** 解決活動建立UI中造成使用者無法在[!UICONTROL 目標與設定]的[!UICONTROL 此活動僅限]區段下選取多個對象的問題。 (TGT-53283)
* **以對象為基礎的報表圖表現在可正確顯示轉換資料。** 解決[!UICONTROL 報表]標籤中造成圖表在選取非預設對象時失敗的問題。 雖然可以使用資料和信賴度量度，但視覺化圖表只顯示實線，導致分析困難。 (TGT-53769)
* **[!UICONTROL 鎖定目標] UI現在會清楚指出排除的對象規則。** 解決在活動建立UI的[!UICONTROL 鎖定目標]區段中，設定為[!UICONTROL 排除]的對象規則未明確顯示的問題。 這會導致檢閱目標定位邏輯時發生混淆，尤其是排除特定URL的受眾。 (TGT-53809)
* **對象定義值現在可以在[!UICONTROL 鎖定目標]索引標籤中選取及複製。** 解決在「活動建立」介面中，使用者無法在[!UICONTROL 鎖定目標]索引標籤中選取和複製對象規則值的問題。 舊版UI提供了此功能，但更新版UI中缺少此功能。 (TGT-53856)
...
* **頁面載入事件選擇現在會在建議修改中持續存在。** 解決在活動建立UI中，將建議的事件型別從[!UICONTROL 檢視]切換為[!UICONTROL 頁面載入]時，使用者無法儲存變更的問題。 雖然選取範圍看起來成功，但在導覽離開後恢復，封鎖活動發佈。 (TGT-53957)
...
* 「**」[!UICONTROL 匯出訂單詳細資料至CSV]」現已下載完整資料。** 解決在更新的[!UICONTROL 總覽] UI中導致「[!UICONTROL 將訂單詳細資料匯出為CSV]」選項下載空白檔案的問題，即使存在有效的報表資料亦然。 (TGT-53787)
...
* **VEC不再在&quot;[!UICONTROL 套用初始修改]&quot;**&#x200B;上間歇性停頓 解決VEC中體驗無法載入並停留在「[!UICONTROL 套用初始修改]」上的間歇性問題。 此行為不一致，有時會觸發重新導向回圈或需要手動清除快取。 (TGT-53916)
* **無法重新產生VEC載入問題。** 我們調查了在編輯現有活動時，VEC停留在&quot;[!UICONTROL 套用初始修改]&quot;上的回報問題。 此行為疑似與缺少上層元素的HTML內容有關。 我們將繼續監視復發，並建議對HTML選件使用結構化容器來確保穩定性。 (TGT-53972)
* VEC中的&#x200B;**[!UICONTROL 設計]模式不再像[!UICONTROL 間歇瀏覽]模式那樣運作。** 解決在UI中，[!UICONTROL 設計]模式間歇性地表現為[!UICONTROL 瀏覽]模式，允許可點按的`<a>`連結並阻止元素選取的問題。 這會導致暫留方塊消失並封鎖修改工作流程。 (TGT-53136)
* **在[!UICONTROL Composer]模式中的按鈕點選不再觸發頁面重新導向。** 解決在更新的VEC UI中按一下[!UICONTROL 撰寫器]模式中的按鈕時，導致未預期的重新導向至按鈕的目標URL的問題。 這會防止使用者編輯call-to-action (CTA)元素，並中斷編寫工作流程。 (TGT-53137)
* **自動個人化活動中的選件代碼更新現在可順利儲存。** 解決在[!UICONTROL Automated Personalization]活動中更新優惠方案代碼時，「新建立」UI中導致「[!UICONTROL 無效的使用者輸入]」錯誤的問題。 即使輸入有效，該錯誤也會阻止使用者儲存變更。 (TGT-53586)
* VEC中的&#x200B;**[!UICONTROL 設計]模式現在會封鎖可編輯元件的連結導覽。** 解決在更新的VEC中，可點按的元素（例如按鈕和連結）即使在[!UICONTROL 設計]模式中也會觸發頁面重新導向的問題。 此行為模擬[!UICONTROL 瀏覽]模式，並阻止使用者修改關鍵元件。 (TGT-53696)
* **「[!UICONTROL 已按一下元素]」量度現在無重新導向或錯誤地運作。** 解決在選取「[!UICONTROL 按一下元素]」轉換量度時，新建立UI中導致非預期重新導向和空白畫面的問題。 安裝程式期間按一下按鈕會觸發導覽，而非註冊元素，導致「[!UICONTROL 找不到元素]」錯誤。 (TGT-53817)
...
* **[!UICONTROL 鎖定目標]區段未指出對象規則已設定為排除。** 使用排除邏輯設定的對象錯誤地顯示為包含在活動建立UI的[!UICONTROL 鎖定目標]區段中。 雖然後端正確套用了排除規則，但UI無法以視覺化方式呈現它，省略「排除」標籤並在行銷活動設定期間造成混淆。 [!UICONTROL 鎖定目標]區段現在可清楚顯示排除規則，以確保對象設定和目標定位視覺效果之間的一致性。 (TGT-53809)
...
* **修改現有活動時無法編輯條件內容。** 編輯活動時，[!UICONTROL 條件]內容區段顯示為停用，按鈕呈現灰色且無回應。 此問題已透過確保[!UICONTROL 條件]設定在活動更新期間完全可編輯而解決。 客戶現在可以修改[!UICONTROL 條件]內容，而不需要切換選擇或使用因應措施，在更新的活動建立程式中提高彈性和可用性。 (TGT-53812)
* **無法在活動內編輯條件。** 從活動記憶體取條件時，[!UICONTROL 編輯條件]和[!UICONTROL 移除條件]選項已停用。 不過，可以透過[!UICONTROL Recommendations]索引標籤成功編輯相同的條件。 現在，活動編輯工作流程和[!UICONTROL Recommendations]標籤中的條件都可以完全編輯，以確保一致且有效的編輯體驗。 (TGT-53814)
...
* **修正在[!UICONTROL 管理內容]強制回應視窗的[!UICONTROL Automated Personalization]活動中，僅活動對象出現無效使用者輸入錯誤的問題。** 客戶在AP活動的[!UICONTROL 管理內容]強制回應視窗中設定僅限活動的對象時，發生無效的使用者輸入錯誤。 儘管先前已成功使用對象，仍發生此問題。 合併的對象設定現在可正確儲存，而不會觸發驗證錯誤。 (TGT-53749)
...
* **優惠決定選項在初始活動建立期間現在會一致顯示。** 解決在更新的UI中，A/B活動首次建立流程期間，尤其是以無痕模式存取已啟用優惠決定的租使用者時，無法顯示[!UICONTROL 優惠決定]選項的問題。 只有在導覽至[!UICONTROL 鎖定目標]步驟並返回[!UICONTROL 體驗]後，才會顯示選項。 此修正可確保[!UICONTROL 優惠決定]選項在初始設定期間立即可用，從而提高可用性並減少混淆。 (TGT-51888)
...
* 已針對在更新的UI中建立的[!UICONTROL Recommendations]活動還原&#x200B;**點選追蹤。** 解決在更新的UI中建立的[!UICONTROL Recommendations]活動無法註冊點選追蹤，導致報告轉換數為零的問題。 舊版UI中內建的活動可正確追蹤點按次數，並如預期回報轉換。 此修正可確保在更新UI中建立的Recommendations活動現在包含正確的追蹤屬性、還原轉換報告並符合A4T量度。 (TGT-53287)
* **已針對建議活動還原點選追蹤。** 解決在更新的UI中建立的[!UICONTROL Recommendations]活動無法註冊點選追蹤，導致報告轉換數為零的問題。 舊版UI已正確將追蹤ID (`at-track-click`)套用至[!UICONTROL Recommendations]內容，而更新版UI誤插入預留位置(`__recsClickTrackIdPlaceholder__`)，導致後端追蹤無法進行。 此修正可確保[!DNL Recommendations]內容現在包含正確的追蹤ID、還原轉換報告並與A4T量度一致。 (TGT-53496)
* 在更新的UI中解決&#x200B;**集合編輯器當機。** 修正更新的[!UICONTROL 視覺化體驗撰寫器] (VEC) UI中，從編輯器面板開啟集合導致頁面當機並出現TypeError的問題：無法讀取未定義的屬性（讀取&#39;customLocale&#39;）。 此錯誤發生在多個活動型別，包括[!UICONTROL 建議]和A/B測試。 (TGT-53703)
...
* **修正第一次開啟[!UICONTROL Products]頁面時，集合未顯示產品的問題。** 存取[!UICONTROL Recommendations]區段中的集合的客戶在首次開啟[!UICONTROL 產品]頁面時，遇到空的產品結果。 此問題是由於GraphQL查詢中的後端錯誤所導致，該錯誤無法載入具有自訂條件之集合的產品資料。 問題已解決，產品現在無需環境切換即可正確顯示。 (TGT-53694)
* **修正無法在表單式[!UICONTROL Recommendations]活動中移除集合的問題。** 使用[!UICONTROL 表單式體驗撰寫器]建立[!UICONTROL Recommendations]活動的客戶無法取消選取先前選擇的集合。 UI要求在儲存前選取集合，以防止使用者恢復為「所有集合」。 此行為已更新，以符合VEC功能，允許客戶在儲存時無需選取集合，並按預期預設為「所有集合」。 (TGT-53708)
...
* **修正將促銷活動套用至即時[!UICONTROL Recommendations]活動觸發錯誤的問題。** 客戶在嘗試將前端促銷活動套用至即時[!UICONTROL Recommendations]活動時，發生「促銷活動缺少集合ID或篩選規則值」錯誤，即使在提供有效的設定詳細資料後也是如此。 促銷活動現在可以成功套用至已上線的活動，而不會觸發驗證錯誤，以確保在更新的活動建立UI中更順暢的體驗。 (TGT-53738)
* **修正[!UICONTROL Recommendations] UI中的集合未顯示產品的問題。** 來自單一租使用者的客戶回報，在新UI的[!UICONTROL Recommendations]區段中檢視某些集合時，產品清單無法載入。 問題透過切換環境而暫時解決，但此因應措施導致使用者體驗不佳。 產品實體現在會一致載入，不需要環境切換。 (TGT-53783)
...
* **[!UICONTROL 總收入]量度現已包含在來自活動報表的CSV匯出中。** 解決更新的[!UICONTROL 概覽] UI中，活動報告檢視中正確顯示總收入，但CSV匯出中遺失的問題，顯示為$0。 這種差異導致使用者無法依賴匯出的資料進行離線分析和報告。 (TGT-53325)
* **[!UICONTROL 總銷售]量度現已包含在活動報表的CSV匯出中。** 已解決更新UI中，[!UICONTROL 總銷售]量度在活動報表檢視中正確顯示，但CSV匯出中遺失的問題。 這種差異導致使用者無法存取下載報表中的完整效能資料。 此修正可確保CSV匯出中現在準確包含[!UICONTROL 總銷售額]值，以恢復應用程式內報表與離線分析之間的一致性。 (TGT-53330)
* **改善未啟用度量時[!UICONTROL 圖表檢視]的錯誤訊息。** 修正VEC中，相關[!DNL Analytics]報告套裝中未啟用所請求的量度時，[!UICONTROL 圖表檢視]顯示一般「發生錯誤」訊息的問題。 此問題是由後端GraphQL回應中的`not_enabled_metric`錯誤所觸發。 此修正以較具資訊性的訊息取代模糊的錯誤，協助使用者識別[!DNL Analytics]中的設定問題，減少混淆和不必要的支援升級。 (TGT-53577)
* **修正報表期間超過支援90天限制的問題。** 使用[!UICONTROL 報告]區段中的「[!UICONTROL 最近X天]」篩選器的客戶可選取超過90天的持續時間，這可能會導致效能問題和資料不完整。 已更新篩選器以強制實施最多90天的範圍，確保一致且可靠的報表。 (TGT-53795)
...
* **修正Analytics for Target (A4T)報告無法在[!UICONTROL 圖表檢視]中顯示資料的問題。** 透過A4T整合使用[!DNL Target]的客戶在A/B測試活動的「報表」索引標籤中切換至「圖表檢視」時發生「發生錯誤」錯誤。 雖然[!UICONTROL 資料表檢視]已正確載入，[!UICONTROL 圖表檢視]無法呈現所選時間範圍內的量度趨勢。 [!UICONTROL 圖表檢視]現在會依預期顯示所有支援量度的效能資料，以改進更新UI中的可見度和報告準確性。 (TGT-53573)
...
* **階層連結暫留現在會在VEC中反白顯示對應的元素。** 改善[!UICONTROL 視覺化體驗撰寫器]，以在階層連結功能表中暫留專案時，在編輯器中醒目提示對應的元素。 也會顯示元素型別，例如容器、粗體文字或按鈕。 此行為會套用至同層級元素，並根據驗證清單排除不支援的型別，例如SVG。 (TGT-53411)
...
* **修正[!UICONTROL Automated Personalization]活動中的[!UICONTROL 管理內容]邊欄顯示不一致位置標籤的問題。** 客戶回報[!UICONTROL 管理內容]邊欄在[!UICONTROL 體驗]和[!UICONTROL 選件]索引標籤中顯示不相符的位置編號。 （例如，體驗中的位置2和位置4，以及選件中的位置1和位置2），即使僅設定了兩個位置亦然。 位置標籤現在是一致的，並準確地對應到修改，提高了活動建立過程中的清晰度和可用性。 (TGT-52934)
...
* **已改善[!UICONTROL AEM體驗片段]選件的使用者歸因。** 解決VEC中[!UICONTROL AEM體驗片段] (XF)選件的「[!UICONTROL 上次更新]」欄位未正確顯示程式碼ID而非使用者名稱的問題。 在更新UI中選擇優惠期間發生這種差異，導致與舊版UI和HTML優惠不一致，後者正確顯示上次編輯者的名稱。 此修正可確保跨選件型別的使用者歸因一致，提高透明度並與預期行為一致。 (TGT-52880 和 TGT-52898)
...
* **已還原更新報表總覽UI之報表區段中的下載選項。** 解決新概觀UI中「報表」區段遺失[!UICONTROL 下載]按鈕，導致使用者無法匯出屬性重要性分數的問題。 此更新會還原完整的匯出功能，簡化可下載資料的存取，以供分析和報告之用。 (TGT-53222)
...
* **已針對更新的UI中的引數存在條件更正表單驗證。** 解決在更新的UI中選取「[!UICONTROL 自訂mbox引數值存在]」或「[!UICONTROL 引數存在]」時，不正確地要求客戶輸入值的問題。 此行為與預期邏輯衝突，預期邏輯只會檢查引數是否存在，無論其值為何。 此修正使表單驗證與預期行為一致，可讓您更順暢地設定活動，並支援完全採用更新的UI。 (TGT-53638)
...
* **已在更新的UI中修正MVT和AP活動的位置名稱持續性。** 已解決在更新的UI中，[!UICONTROL 多變數測試] (MVT)和[!UICONTROL Automated Personalization] (AP)活動中編輯的位置名稱未正確儲存的問題。 重新命名位置後，在標籤（例如[!UICONTROL 鎖定目標]和[!UICONTROL 體驗]）之間導覽，導致名稱恢復為先前的狀態。 此修正可確保各索引標籤的位置命名一致，並改善活動設定期間的可靠性。 (TGT-52927)
* **已在MVT活動的「管理體驗」面板中修正位置標籤。** 解決在更新的UI中，[!UICONTROL 多變數測試] (MVT)活動中的[!UICONTROL 管理體驗]面板顯示不一致的位置編號的問題。 此修正可確保位置標籤順序且正確與使用者定義的修改一致，進而在體驗設定期間提高清晰度和可用性。 (TGT-52929)
...
* **客戶無法從[!UICONTROL 活動概觀]**&#x200B;複製活動或檔名稱：以前，客戶無法直接在更新的活動建立程式中，從[!UICONTROL 活動概觀]複製活動或相關優惠方案/檔案的名稱。 此限制會影響可用性，尤其是在較小的熒幕上。 客戶現在可以輕鬆複製活動和檔名稱，無需因應措施。 (TGT-51850)
...
* **AP活動在[!UICONTROL 報表]區段**&#x200B;中未擷取模型就緒的資料：在[!UICONTROL 報表]區段檢視Automated Personalization (AP)活動的客戶，無法在報表群組和選件層級看到模型就緒指標。 發生此問題是因為無法從後端正確擷取模型就緒的資料。 功能已還原，且模型就緒的資料現在如預期般顯示。 (TGT-53600 和 TGT-53601)
* **排程為未來的活動在[!UICONTROL 活動]總覽**&#x200B;中錯誤顯示「[!UICONTROL 即時]」狀態：客戶觀察到，排程為未來開始的活動在[!UICONTROL 活動]總覽中錯誤標示為「[!UICONTROL 即時]」。 已解決此狀態不符問題，排程的活動現在可正確顯示為「[!UICONTROL 已排程]」，不需要重新整理頁面。 (TGT-52835)
...
* **在[!UICONTROL 檢視集合]對話方塊中看不到產品清單：**&#x200B;之前，客戶在[!UICONTROL Recommendations]索引標籤中檢視集合時，無法看到產品清單。 [!UICONTROL 檢視集合]對話方塊現在可以正確顯示相關產品，提高更新的[!UICONTROL Recommendations] UI的透明度和可用性。 (TGT-50531)
* **修正[!UICONTROL 產品目錄搜尋]進階搜尋**&#x200B;中區分大小寫篩選的問題： [!UICONTROL 產品目錄搜尋]頁面中的進階搜尋篩選現在會正確忽略區分大小寫功能，並符合後端和GraphQL服務的行為。 此更新可確保為客戶提供一致且準確的建議結果，無論文字大小寫為何。 (TGT-53585)
...
* **在「報表」頁面上切換對象時，導致Target UI發生錯誤**：客戶在[!UICONTROL 報表]區段中選取某些對象時遇到錯誤。 此問題是因為後端GraphQL呼叫中的對象處理無效，導致未預期的錯誤和遺失資料所導致。 問題已解決，現在即使沒有可用資料，案頭受眾仍會載入且不會發生錯誤。 (TGT-53370)
* [!UICONTROL 報表]區段中的&#x200B;**[!UICONTROL 圖形檢視]未顯示來自[!DNL Analytics]**&#x200B;的值：在Re連線埠區段中存取[!UICONTROL 圖形檢視]的客戶遇到遺漏資料，所有值都顯示為零。 此問題是因為從[!UICONTROL Analytics]擷取的資料不正確所造成。 [!UICONTROL 圖表檢視]現在會如預期顯示正確的值。 (TGT-52792)
+++
...
* **使用[!UICONTROL 增強體驗撰寫器] (EEC)按一下「接受Cookie」失敗，因為遺失函式**：客戶回報嘗試透過EEC接受Cookie導致主控台錯誤： `handleclickAcceptAllButton is not defined`。 Cookie接受功能現在可如預期運作，確保在更新UI中建立活動期間提供更順暢的體驗。 (TGT-52794)
...
* **使用Backspace清除所有日期值會導致頁面當機**：排程[!UICONTROL 目標與設定]區段中的活動的客戶在使用Backspace清除「[!UICONTROL 指定的日期與時間]」欄位中的所有值時發生當機情況。 此問題是由日期處理邏輯中的Null參考錯誤所導致。 頁面現在能順利處理空白的日期輸入，而不會當機。 (TGT-53624)
* **由於裝載無效，[!UICONTROL 產品目錄搜尋]中未出現任何產品**：存取[!UICONTROL 產品目錄搜尋]中[!UICONTROL Recommendations]區段的客戶遇到由於GraphQL裝載無效而造成的空白結果。 此後端錯誤導致產品資料無法正確載入。 產品現在會在更新後的UI中如預期般顯示。 (TGT-53630)
* **[!DNL Scene7]個影像已在更新的VEC中以較低解析度儲存**：在更新的VEC中編輯體驗的客戶注意到，[!UICONTROL Scene7]個影像URL在沒有解析度引數的情況下儲存，導致傳送的影像品質較低（400×400而不是預期的800×800）。 影像URL現在會保留正確的引數，以確保正確解析度。 (TGT-52631)
* **仍可在VEC中編輯已上線活動**：客戶可存取更新後VEC中已上線活動的編輯選項，這可能會導致意外的變更。 此問題已藉由停用已上線活動的編輯功能而解決。 編輯按鈕現在會隱藏在活動清單和編輯者的概覽中，而核准者和其他角色則不受影響。 (TGT-53055)
* **已解除委任VEC**&#x200B;中更新的[!UICONTROL 失敗]及[!UICONTROL 草稿]活動區段： [!UICONTROL 失敗]及[!UICONTROL 草稿]活動選項已從更新的VEC中移除。 新的UI不再支援草稿狀態，且失敗的行銷活動不會儲存在後端。 這些選項已不再相關。 相關篩選器和後端欄位（例如，`uiSyncFailed`、`errorMessage`）也已移除，以簡化活動管理。 (TGT-53150)
* **無法登入VEC進行活動**：嘗試透過VEC登入其網站的客戶被重複重新導向至登入頁面，導致無法存取活動編輯。 此問題無法在內部複製，且可能與網站端工作階段處理有關。 登入流程已穩定，客戶現在可以存取VEC而不會發生重新導向錯誤。 (TGT-53524)
* **在[!UICONTROL 瀏覽]模式中按兩次返回按鈕會導致VEC當機**：在VEC中瀏覽[!UICONTROL 瀏覽]模式的客戶在按兩次瀏覽器的返回按鈕時遭遇當機情況。 此問題會導致編輯器凍結，並需要重新整理頁面。 編輯器現在能以可靠的方式處理背面導覽，而不會當機。 (GT-53568)
...
* **修正排程活動在頁面重新整理前無法正確顯示[!UICONTROL 即時]狀態的問題**：客戶觀察到，當排程活動在未來日期和時間上線時，狀態會立即顯示為[!UICONTROL 即時]，而非[!UICONTROL 已排程]。 這會導致混淆，即使確認訊息正確指出活動已排程。 重新整理頁面已更正狀態顯示。 此問題現在已解決，排程活動可正確顯示排程狀態，不需要重新整理。 (TGT-52937)
...
* **修正客戶在活動建立程式期間無法輸入報表套裝名稱的問題**：在活動建立程式期間使用[!DNL Adobe Analytics]作為報表來源的客戶無法輸入[!UICONTROL 報表套裝]下拉式清單來搜尋特定報表套裝。 這會影響擁有大量報表套裝之組織的工作流程，而手動捲動會大幅延遲設定。 下拉式清單並未依字母順序排序，且未一致回應輸入的內容，因此很難找到「Office + Store - Web - Prod」等報表套裝。 此問題已解決，客戶現在可以透過輸入報告套裝名稱來有效搜尋。 (TGT-53345)
...
* **修正客戶無法在UI中使用[!UICONTROL 插入在前]或[!UICONTROL 插入在後]插入體驗片段的問題**&#x200B;客戶嘗試在更新的UI中使用「插入在前」或「插入在後」選項將[!UICONTROL 體驗片段]插入活動時，發生錯誤。 顯示的錯誤訊息為：「選件內容必須包含正好一個HTML元素」。 此問題僅發生在更新的UI中，不會發生在先前的版本中。 此問題現在已解決，客戶可以成功插入[!UICONTROL 體驗片段]，而不會遇到此錯誤。 (TGT-53442)
...
* **修正[!UICONTROL 優惠決定]選取器儲存後無法修改和意外變更的問題**：在更新的活動建立程式中，客戶無法如預期修改[!UICONTROL 優惠決定]選取器。 嘗試變更選擇器失敗，選擇器在儲存後恢復到不正確的值。 此行為導致決定選件從視覺化體驗撰寫器(VEC)中消失，並封鎖進一步編輯。 選擇器變更現在會正確保留，並且決定選件儲存後在VEC中保持可見和可編輯狀態。(TGT-53433)
* **修正儲存後從活動中[!UICONTROL 優惠決定]消失的問題**：在活動建立程式中新增的[!UICONTROL 優惠決定]在儲存並重新開啟活動後未保留。 編輯動態內容並使用特定選取器和屬性插入[!UICONTROL 優惠決定]時，會發生此問題。 [!UICONTROL 優惠決定]在儲存後現在會正確保留，而選取器會維持不變，以確保目標定位和編輯行為的一致性。 (TGT-53434)
...
* **修正[!UICONTROL 目錄搜尋]**&#x200B;中載入的影像不一致：修正[!UICONTROL 目錄搜尋]中的縮圖與影像在活動建立程式中無法一致載入的問題。 除非顯示「縮圖URL」欄，且在導覽或搜尋動作後已載入部分或完全未載入某些產品影像，否則影像無法顯示。 影像載入行為已穩定，現在無論欄可見度或導覽動作為何，縮圖都會以可靠的方式顯示。 (TGT-52778)
...
* **修正[!UICONTROL 建議目錄]間歇性地無法顯示完整產品屬性資料的問題**：在更新的[!DNL Recommendations] UI中，客戶遇到即使資料存在於摘要中，[!UICONTROL 目錄搜尋]結果中仍無法一致顯示某些產品屬性（例如訊息）的問題。 此問題需要客戶手動重新設定欄可見性，以擷取缺少的值。 [!UICONTROL 目錄搜尋]現在會可靠地顯示所有已設定的屬性，不需要手動重設欄。 (TGT-52769)
* **修正無法在已上線活動中停用[!UICONTROL 前端促銷活動]的問題**：未儲存嘗試在已上線活動中停用[!UICONTROL 前端促銷活動]。 選取[!UICONTROL 變更促銷活動]並停用它之後，促銷活動會在重新編輯活動時保持作用中，以防止建議設定的更新。 促銷活動設定現在已正確儲存，可讓客戶如預期停用或修改上線活動中的促銷活動。 (TGT-53231)
...
* **修正[!UICONTROL 自動化區段]報表顯示null對象值的問題**：客戶回報活動報表中的[!UICONTROL 自動化區段]顯示的null對象資料，導致無法正確分析區段效能。 存取[!UICONTROL 報表]區段並選取[!UICONTROL 自動化區段]時，即使需要有效的對象資料，也會發生此問題。 [!UICONTROL 自動化區段]現在可以正確顯示對象值，確保可靠的報表和分段深入分析。 (TGT-52793)
...
* **修正更新UI中在[!UICONTROL 瀏覽]和[!UICONTROL 設計]模式之間切換會重設SPA狀態的問題**：客戶報告在更新UI中在[!UICONTROL 瀏覽]和[!UICONTROL 設計]模式之間切換會導致網頁編輯器重新載入，重設SPA的狀態。 此問題會中斷工作流程，並需要客戶重新輸入資訊。 問題已解決。 現在，在模式之間切換時，會保留SPA狀態，以確保在活動建立期間獲得更順暢且一致的體驗。 (TGT-53074)
...
* **修正在AP活動中，活動建立程式封鎖推進至[!UICONTROL 鎖定目標]步驟的問題**：修正活動建立程式中，客戶無法繼續進行[!UICONTROL Automated Personalization] (AP)活動中的[!UICONTROL 鎖定目標]步驟的問題，除非新增兩個位置。 此行為與先前體驗不同，先前體驗中，具有多個選件的單一位置已足夠。 已更正此要求，讓客戶能夠繼續使用單一位置設定作為其AP工作流程的一部分。 (TGT-53426)
...
* **修正導致客戶無法在更新的UI中搜尋報表套裝的問題**：在更新的UI中，[!UICONTROL 目標與設定]區段中的[!UICONTROL 報表套裝]下拉式清單不允許客戶輸入和搜尋，導致難以找到特定的報表套裝，尤其是具有大量專案的租使用者。 和之前的UI不同，清單沒有排序，缺少基於輸入的篩選。 此問題已解決。 客戶現在可以輸入來搜尋和篩選報表套裝，還原舊版UI中可用的功能。 (TGT-53514)
...
* **修正限制在單一工作區的客戶可以從其他工作區檢視活動的問題**：在活動建立程式中選取[!UICONTROL 所有工作區]時，存取限制在單一工作區的客戶意外地能夠檢視所有工作區的活動。 此可見度對其他工作區中的已上線活動造成非預期變更的風險，可能會影響網站效能。 Workspace存取控制已增強，以確保客戶只能檢視其指派的工作區中的活動並與之互動。 (TGT-53101)
* **修正客戶無法存取即可從[!UICONTROL 預設Workspace]檢視活動的問題：**&#x200B;存取許可權僅限於中繼工作區的客戶無法透過活動建立程式從[!UICONTROL 預設Workspace]檢視活動。 即使後端設定和存取許可權正確，還是會發生此行為。 Workspace存取控制已增強，以確保客戶只能在其指派的工作區內檢視活動。(TGT-53297)
...
* **修正在Click Track模式中追蹤的元素欄位行為，以防止未儲存的輸入並提高清晰度**：修正活動建立程式中，[!DNL Click Track]模式中的[!UICONTROL 追蹤的元素]欄位可編輯，但未保留輸入名稱，導致客戶混淆的問題。 欄位現在已停用，以防止未儲存的輸入，並且已澄清所選元素的命名，以改善目標設定和追蹤準確性。** (TGT-53458)
* **修正活動建立程式中，[!UICONTROL 按一下追蹤]模式中無法命名追蹤元件的問題**：修正活動建立程式中，客戶無法在[!UICONTROL 按一下追蹤]模式中命名追蹤元件的問題。 輸入名稱后，欄位似乎可編輯，但未保留輸入，在編輯模式中預設為一般標籤，例如「MY PRIMARY GOAL 0」。 追蹤的元素欄位現在已停用，並且已釐清命名行為以提高目標設定和追蹤準確性。 (TGT-51396)
...
* **修正目錄搜尋UI中進階搜尋無法提供建議的問題**：修正新[!UICONTROL 目錄搜尋] UI中，[!UICONTROL 進階搜尋]功能無法提供建議的問題。 使用者必須輸入包含精確拼字的確切值，導致搜尋體驗繁瑣且容易出錯。 透過此修正，[!UICONTROL 進階搜尋]現在會隨著使用者型別提供相關建議，改善使用性，並減少尋找產品時的摩擦。 (TGT-52008)
...
* **修正活動建立程式中，在建議檢視中無法顯示產品訊息的問題**：修正活動建立程式中，[!DNL Recommendations]檢視中的[!UICONTROL 訊息]資料行未顯示產品資料（即使有訊息）的問題。 客戶必須手動移除並重新新增欄以暫時顯示內容，當捲動或搜尋時，內容通常會再次消失。 此更新可恢復產品訊息的一致可見性，並改善目錄導覽和稽核工作流程。 (TGT-52777)
...
* **已修正依[!UICONTROL 上次更新]資料行排序不一致的問題：**&#x200B;已修正活動建立程式中，依[!UICONTROL 上次更新]資料行排序目錄會產生不一致結果的問題。 客戶無法根據更新時間戳記可靠地組織產品資料，導致目錄檢閱和管理更加困難。 排序現在可如預期運作，改善更新UI中的精確度和可用性。 (TGT-53116)
...
* **修正活動載入及[!UICONTROL 取消]活動建立程式中的按鈕問題**：修正活動建立程式中，某些活動無法載入，導致客戶無法存取修改的問題。 此外，[!UICONTROL 取消]按鈕無回應，導致客戶無法停止載入程式或退出編輯檢視。 此修正可確保活動現在能可靠地載入，且[!UICONTROL 取消]按鈕如預期般運作，進而改善視覺化體驗撰寫器的整體穩定性和使用者體驗。 (VEC)(TGT-53218)
* **修正更新VEC UI中封鎖編輯及停用[!UICONTROL 取消]按鈕**&#x200B;的體驗切換問題：修正更新VEC UI中在體驗鎖定目標(XT)活動中的體驗之間切換時無法載入修改的問題。 客戶無法編輯超過他們最初輸入的體驗，且[!UICONTROL 取消]按鈕遺失或無回應。 此修正可確保修改現在在所有體驗中正確載入，且[!UICONTROL 取消]按鈕即使沒有Helper擴充功能，也能可靠運作，進而改善編輯工作流程並減少挫敗感。 (TGT-53256)
...
* **修正VEC中自訂程式碼變更無法可靠地跨編輯工作階段儲存的問題**：修正視覺化體驗撰寫器(VEC)中所做的自訂程式碼修改無法可靠地在單一嘗試中儲存的問題。 客戶回報說，即使同時使用[!UICONTROL 編輯內容]和最終[!UICONTROL 儲存]按鈕，網站和QA URL中仍間歇性地遺漏樣式更新或HTML編輯等變更。 此回歸已解決，確保所有自訂程式碼變更在整個編輯工作階段中都會如預期般持續存在。** (TGT-53418)
...
* 修正更新的[!DNL Target] UI中，具有[!UICONTROL 編輯者]角色的使用者能夠存取及嘗試編輯已上線活動的問題，此問題應受到限制。 活動清單和概觀畫面錯誤地顯示已上線活動的編輯選項，導致潛在的意外更改。 (TGT-53055)
* 修正在[!UICONTROL 進階搜尋] UI中選取「值存在」或「值不存在」運運算元時，提示使用者輸入運算元的問題，這些條件不應該需要此運算元。 (TGT-51961)
...
* 修正體驗片段(XF)內容在[!UICONTROL 視覺化體驗撰寫器] (VEC)預覽中無法呈現的問題（儘管在內容傳送中正常運作）。 (TGT-53318)
...
* 修正更新[!DNL Target] UI中，在[!UICONTROL 視覺化體驗撰寫器] (VEC)中作出的HTML選件修改，在從[!UICONTROL 鎖定目標]標籤導覽回[!UICONTROL 體驗]後未持續存在的問題。 (TGT-52874)
* 修正更新的[!DNL Target] UI中，在同一選取器之前和之後插入一個HTML選件導致不正確位置產生的問題。 當客戶從[!UICONTROL 鎖定目標]標籤傳回至[!UICONTROL 體驗]標籤時，僅保留一個選取器，導致修改遺失及內容傳遞中斷。 此行為與舊版UI不同，後者使用不同的位置識別碼正確保留兩個修改。 (TGT-52891)
* 修正在更新的[!DNL Target] UI中，按一下[!UICONTROL 修改]邊欄內新增的選件時，導致右側[!UICONTROL 組態]面板間歇出現並消失，使其難以與選件設定互動的問題。 (TGT-53288)
...
* 已修正更新VEC中導致客戶無法對[!UICONTROL 單頁應用程式] (SPA)檢視套用修改的問題。 雖然在舊版UI中建立的活動可支援檢視特定目標定位，但新版UI無法偵測或允許對這些檢視進行編輯，且檢視切換控制項顯示為停用。 (TGT-52556)
...
* 修正在[!UICONTROL 視覺化體驗撰寫器]內對體驗所做的修改無法顯示或在UI中顯示不一致的問題。 (TGT-TGT-53381)
* 修正更新VEC中，[!UICONTROL 管理內容]區段無法顯示體驗縮圖替代文字的問題。 此問題使得使用者難以識別檔案，尤其是當檔案名稱太長或截斷時。 (TGT-52291)
* 修正客戶在VEC活動中設定[!UICONTROL 頁面傳送]規則時遇到錯誤驗證錯誤的錯誤。 具體而言，在輸入文字值時，類似「等於任何」和「不等於任何」的運運算元會觸發「運運算元型別的輸入無效」，即使應該支援文字亦然。 (TGT-52629)
* 修正當使用「選取選件」按鈕選取選件時，更新UI的[!UICONTROL 修改]面板中導致不一致行為的幾個問題。 UI未取代現有的選件，而是新增了一個重複專案，並嘗試與任一選件互動導致主控台錯誤，例如`selectorNotFound`。 此外，有些選件不會顯示「選取選件」按鈕，只會顯示可編輯的屬性。 (TGT-53321)
...
* 修正在更新的[!DNL Target] UI中工作區之間複製活動時的多個問題。 客戶遇到與受眾存取相關的錯誤，尤其是即時活動，以及不正確的許可權驗證，即使使用者在來源和目的地工作區中同時具有「[!UICONTROL 核准者]」角色。 (TGT-53002)
...
* 修正[!UICONTROL 建立條件]強制回應視窗中，在[!UICONTROL 選擇價格規則]下拉式清單中選取「介於下列值之間」選項時，[!UICONTROL 包含規則]區段中的字串「to」未當地語系化。 (TGT-49754)
* 修正摘要建立精靈的[!UICONTROL 環境]下拉式清單中的字串&quot;[!UICONTROL 所有主機群組]&quot;未正確本地化的問題。 (TGT-46737)
...
* 修正無法在QA環境中建立活動的問題。 處理程式已重新導向至[!UICONTROL 活動]頁面，而非順利完成。 (TGT-53328)
...
* 修正更新UI中[!UICONTROL 目錄搜尋]的篩選器建議不準確的問題。 (TGT-52007)
...
* 修正當使用者在編輯Automated Personalization (AP)活動時按一下[!UICONTROL 管理內容]然後按一下[!UICONTROL 完成]時，頁面變為空白且無回應的問題。 (TGT-53047 和 TGT-52993)
* 修正在[!UICONTROL 目標與設定]下選取[!UICONTROL 已檢視mbox]轉換量度而造成頁面當機的問題。 (TGT-53346、TGT-53343 和 TGT-53348)
* 修正[!UICONTROL 重新導向至URL]功能無法如預期運作的問題，即使使用有效的URL，也不會重新導向。 (TGT-53307)
...
* 修正當工作區設定為&quot;[!UICONTROL 所有工作區]&quot;時，複製預設工作區中已存在的活動會導致不正確錯誤的問題：
...
* 修正將修改從[!DNL Page Load]移至[!UICONTROL 檢視]時，造成修改重複的問題，此問題仍留在[!UICONTROL 頁面載入]中，同時出現在[!UICONTROL 檢視]中。 此外，從[!UICONTROL 檢視]中移除修改也會不正確地從[!UICONTROL 頁面載入]中將其移除。 (TGT-53270)
...
* 修正[!UICONTROL 表單式體驗撰寫器]中，在建立或編輯[!UICONTROL Automated Personalization] (AP)活動時，在按一下&#x200B;**[!UICONTROL 管理內容]**&#x200B;圖示（![管理內容圖示](/help/main/assets/icons/Experience.svg)）之後導致編輯器當機的問題。 (TGT-53047)
...
* 修正捲動至清單底部時，[!UICONTROL 目錄搜尋]無法載入其他結果的問題，復原與舊版UI一致的行為。 (TGT-53088)
* 修正封鎖從[!UICONTROL 條件詳細資料]對話方塊刪除專案的問題。 (TGT-53245)
* 修正無法開啟或無法與無名稱產品互動的問題。 選擇傳回未命名結果的環境時，會發生此問題，導致無法存取產品詳細資料。 (TGT-53007)
* 修正導致[!UICONTROL 目錄搜尋]頁面當機並在選取特定產品時顯示空白畫面的問題。 (TGT-53087)
* 修正使用者無法在[!DNL Target] UI中編輯site_cart_z1 [!DNL Recommendation]活動的問題。 嘗試開啟活動在[!UICONTROL 總覽]頁面上觸發錯誤，封鎖編輯器的存取權。 (TGT-53221)
...
* 修正在VEC中設定影像選件時，[!UICONTROL 插入在前]和[!UICONTROL 插入在後]選項的[!UICONTROL 復原]功能的問題。

  先前，在影像選件上復原[!UICONTROL 插入在前]或[!UICONTROL 插入在後]動作會導致不一致的行為或無法正確回覆修改，尤其是在舊版[!DNL Target] UI中建立的活動中。 此問題已解決，以確保復原動作現在能可靠地用於這些修改。 (TGT-52809)
...
* 已將[!UICONTROL 目標與設定]中的[!UICONTROL 優先順序]滑桿還原為舊版功能，讓客戶如舊版UI所支援，可直接輸入數值。 (TGT-53185 和 TGT-53219)
...
* 已修正當客戶在[!UICONTROL 目標與設定]頁面上檢視特定活動的報告時， Analytics中的[!UICONTROL 檢視]連結未正確地指向QA環境而非生產環境的問題。 (TGT-53163)

+++

**[!UICONTROL 體驗]和[!UICONTROL 選件]**

+++檢視詳細資料

* 修正透過自訂程式碼叫用`triggerView`時造成無限回圈的問題。 (TGT-52885)
* 修正為活動定義的`LocalIds`與體驗定義中使用的`LocalIds`之間不相符的問題。 (TGT-52669)
* 修正活動[!UICONTROL 概覽]頁面上遺失量度名稱的問題，僅顯示「選件」而非正確的量度名稱。 (TGT-53054)
...
* 修正[!UICONTROL 表單式體驗撰寫器]中，無法儲存活動並觸發錯誤訊息：「無法讀取未定義的屬性（讀取&#39;map&#39;）」的問題。 (TGT-53145)
...
* 修正從[!UICONTROL 目錄搜尋]按一下產品時，顯示「無法擷取產品詳細資料」錯誤，且未使用關閉選項開啟強制回應視窗的問題。 (TGT-53082)
* 修正變更集合或促銷活動時，[!UICONTROL A/B測試]活動中的[建議（作為選件](/help/main/c-recommendations/recommendations-as-an-offer.md)）未正確更新的問題。 (TGT-52884)
...
* 解決在[!UICONTROL 目標與設定]頁面上，用於多個體驗的選取器未一致地反白顯示為選取狀態的問題。 (TGT-53062)
...
* 修正[!UICONTROL 活動QA] URL包含不必要查詢引數的問題： `at_preview_evaluate_as_true_audience_ids`。 (TGT-52907)
* 修正預覽URL不正確地包含使用者明確輸入之對象以外的其他對象的問題。 已修正此行為，以確保在產生QA或預覽連結時僅套用指定的對象。 (TGT-52912)
* 修正了在流量分配設定期間先選取[!UICONTROL 自動分配] (AA)時，使用者無法建立[!UICONTROL 自動鎖定目標] (AT)活動的問題。 此問題會導致後端驗證錯誤，且無法儲存活動。 (TGT-53096)
...
* 修正使用者具有[!UICONTROL 核准者]角色無法新增或儲存僅限於此活動的對象細分的問題。 嘗試這麼做會導致403禁止錯誤，指出「[編輯者]」許可權是必要的，即使使用者擁有足夠的許可權可核准和管理活動。 (TGT-52984)
* 修正使用[!UICONTROL 移除對象細分]選項移除活動特定對象時，同一活動內不再有對象出現在對象清單中以供重新選取的問題。 除非從頭重新建立，否則此行為會防止使用者重新新增相同的對象。 (TGT-52979)
* 修正從位置移除後，甚至在儲存活動之前，僅限活動的對象細分立即從UI消失的問題。 此行為與預期功能和工具提示指引有所抵觸，指引指出：「一旦儲存活動，將會刪除此資料庫中所有未使用的對象」。 (TGT-52982)
* 修正嘗試將[!UICONTROL 所有訪客]以外的對象指派至活動時發生的問題。 儲存後，顯示下列錯誤訊息：「我們無法完成您的要求。 如果問題仍然存在，請聯絡[!UICONTROL Adobe客戶服務]。」 (TGT-53008)
* 修正在活動編輯器中建立和指派新對象後，無法儲存活動的問題。 顯示的錯誤訊息為：「我們無法完成您的要求。 如果問題仍然存在，請聯絡[!UICONTROL Adobe客戶服務]。」 (TGT-52977)
...
* 修正移除所有內容選項後，[!UICONTROL 管理內容]功能無法正確驗證位置狀態的問題。 嘗試儲存或繼續活動設定時，此問題可能會導致不一致的行為或錯誤。 (TGT-52801)
* 修正使用者新增頁面及刪除不同體驗中的特定元素時，發生「輸入無效」錯誤的問題。 在元素操作期間產生重複的`LocalIds`，尤其是切換體驗和修改共用頁面結構時，會觸發錯誤。 (TGT-52720)
* 修正使用[!UICONTROL 產生臨時選件]功能導致[!UICONTROL 管理內容]面板中出現未定義位置的問題。 (TGT-53076 和 TGT-53070)
* 釐清客戶從[!UICONTROL 鎖定目標]步驟導覽回[!UICONTROL 體驗]時，使用HTML選件進行的修改可能遺失的行為。 針對此客戶，受影響的網站會動態產生多個DOM選取器，並隨著每次頁面載入而變更。 因此，重新開啟編輯器時，找不到原來用於修改的選取器，導致修改遺失或無效。 此情境如預期般運作。 為了確保修改內容可在編輯器中持續顯示，建議使用者端使用穩定、一致的選取器，且這些選取器不會在頁面重新載入時變更。 (TGT-52874)
...
* 已新增新的[!DNL Recommendations]摘要[狀態](/help/main/c-recommendations/c-products/feeds.md#status)： [!UICONTROL 部分匯入失敗]。 (KB-2215)
* 修正當加入具有[!UICONTROL 促銷活動]的[!DNL Recommendations]時，影響活動建立工作流程的問題。 當使用者選取「[!UICONTROL 依屬性促銷]」並新增篩選規則（例如[!UICONTROL 引數比對]）時，儲存並重新編輯活動後未保留選取的規則型別和運算元值。 重新開啟時，篩選規則型別會意外變更，且遺失運算元值。 (TGT-53059)
* 修正在[!DNL Recommendations] UI中，無論規則的邏輯為何，使用單一規則建立的任何促銷活動都會被錯誤解譯並顯示為「專案清單」促銷活動型別的問題。 (TGT-53063)
* 修正使用更新的[!UICONTROL 總覽]UI時，包含[!DNL Recommendations]的[!UICONTROL 體驗鎖定目標] (XT)活動缺少「[!UICONTROL 下載建議資料]」按鈕的問題。 (TGT-52730 和 TGT-52756)
...
* 修正在具有建議的表單式A/B活動中設定&quot;[!UICONTROL 依屬性促銷]&quot;促銷活動時，客戶無法儲存篩選規則的問題。 儲存並重新開啟活動後，篩選規則遺失，且活動無法成功儲存。 (TGT-53057)
...
* 修正從[!UICONTROL 報表]頁面選取「將[!UICONTROL 訂單詳細資料匯出至CSV]」導致下載空白檔案的問題。 即使活動中存在有效的訂單資料，也會發生此問題。 (TGT-52225)
...
* 修正在活動建立的[!UICONTROL 修改]步驟中所做的變更無法一致儲存的問題。 在某些情況下，完成所有步驟並按一下「儲存」後，新增至「[!UICONTROL 修改」]區段中的自訂指令碼將不會反映在已上線的網站上，儘管在儲存過程中沒有可見的錯誤。 (TGT-52661)
* 修正自訂程式碼變更未正確儲存，且無意間映象至相同活動內多個體驗的問題。 此外，使用者在開啟或重新整理特定活動時遇到存取問題，導致空白熒幕。 這些問題現已解決，以確保穩定的活動編輯和精確的體驗隔離。 (TGT-52594)
* 修正使用者在[!UICONTROL 瀏覽模式]時無法瀏覽至其他URL的問題。 這可防止測試者和編輯者驗證或預覽相同活動工作階段內的替代頁面。 (TGT-53052)
* 修正在活動建立期間同時開啟多個[!UICONTROL 視覺化體驗撰寫器] (VEC)執行個體的問題。 當使用者停用[!UICONTROL 增強體驗撰寫器] (EEC)並在[!UICONTROL 頁面傳送]步驟中移除網站URL的結尾斜線時，就會發生此問題。 (TGT-52782)
* 修正[!UICONTROL 目標與設定]步驟中的[!UICONTROL 收入]量度下拉式清單錯誤地預設為[!UICONTROL 每次造訪收入] (RPVISIT) （即使使用者已選取其他量度）的問題。  收合併重新展開量度設定面板時，發生問題，導致重設先前選取的值。 (TGT-52811 和 TGT-52878)
* 修正[!UICONTROL Automated Personalization] (AP)和[!UICONTROL Multivariate Testing] (MVT)活動中與Offer Naming和內容翻譯相關的活動建立工作流程幾個問題：
...
* 已將[!UICONTROL 重新排列]選項新增至更新的[!UICONTROL 視覺化體驗撰寫器] (VEC) UI，以符合舊版VEC中可用的功能。 (TGT-46957 和 TGT-52876)
* 修正了[!UICONTROL A/B測試]活動中對變體體驗（例如體驗B）所做的修改未保留的問題。 在體驗之間切換後，對變數的變更將會消失。 此問題不會影響控制體驗。 (TGT-52664)
* 已修正某些客戶無法建立或儲存活動，而其他客戶則可順利執行相同動作的問題。 不同帳戶之間的問題不一致。(TGT-52842)
* 已修正更新VEC中，使用者無法移動舊版UI中已存在之[!UICONTROL 頁面載入事件]的修改功能的問題。 (TGT-52617)
* 修正更新UI中建立變更時，[!DNL Target]中未顯示[!UICONTROL 頁面載入]事件的問題；更新僅套用至檢視。 (TGT-52604)
* 已修正導致某些活動修改無法在更新的VEC中正確顯示的問題。 (TGT-52818)
* 修正擷取[!UICONTROL Automated Personalization] (AP)活動的報告資料時，所發生的Null指標例外狀況。 (TGT-52362)
* 已修正導致選件層級詳細資料無法在[!UICONTROL Automated Personalization] (AP)活動的.CSV檔案中顯示的問題。 (TGT-52675)
* 修正在更新的VEC中套用修改時，最初會正確顯示變更的問題，包括預期的[!UICONTROL 體驗片段]。 不過，在切換體驗或進行其他編輯時，由於選取器問題，某些修改無法套用。 (TGT-52679)
* 修正複製現有活動以建立新活動時，複製活動中的QA連結錯誤地保留原始活動的頁面URL的問題。 (TGT-52775)
* 已修正更新VEC中無意中導致[!UICONTROL 裝置上決策]無法使用的問題。 (TGT-52371)
* 修正無法編輯產品[!DNL Recommendations]活動的問題。 嘗試透過Target UI存取VEC時，[!UICONTROL 總覽]頁面上出現錯誤，導致無法進行任何編輯。 (TGT-52823)
...
* 修正具有[!UICONTROL 編輯者]角色的使用者無法編輯[!DNL Recommendations]活動的問題。 嘗試變更設計並儲存活動導致403禁止錯誤，指出「[編輯者]」許可權是必要的，即使使用者已在相關工作區中擁有該角色。 (TGT-52836)
...
* 修正未針對某些活動產生[!UICONTROL 自動化區段]和[!UICONTROL 重要屬性]報告的問題。 (TGT-52904)
...
* 修正[!UICONTROL 多變數測試] (MVT)活動中不允許從先前移除的體驗中重複使用`experienceLocalId`的問題。 (TGT-52672)
...
* 修正更新即時[!UICONTROL Recommendations]活動時，由於要求裝載中的屬性無法辨識而發生的錯誤。 系統現在可以正確處理「無效的JSON」。 無法辨識的屬性名稱」錯誤。 (TGT-52723)
* 修正無法建立[!DNL Recommendations]設計的問題。 按一下「[!UICONTROL 建立]」觸發訊息：「指令碼內應至少使用1個實體變數。」 (TGT-52395 和 TGT-52899)
* 修正無法在不修改的情況下重新儲存[!DNL Recommendations]設計的問題。 (TGT-52879)
* 修正儲存[!UICONTROL Recommendations]活動時，造成「400錯誤請求」錯誤的後端驗證錯誤。 (TGT-52716)
* 修正[!UICONTROL 表單式體驗撰寫器]中，將游標暫留在[!UICONTROL 位置]下拉式清單中具有特殊字元的mbox上，導致編輯器變為空白並觸發「無法在&#39;Element&#39;上執行&#39;querySelector&#39;的問題。」 」錯誤。 (TGT-52717)
...
* 已新增[新常見問題集文章](/help/main/c-intro/updated-ui-faq.md)，以解決有關已更新[!DNL Target] UI和[!UICONTROL 視覺化體驗撰寫器] (VEC)的常見問題。
* 修正「[!UICONTROL 頁面傳送]」中的「[!UICONTROL URL — 不包含]」規則無法運作，導致內容無法顯示的問題，即使應封鎖內容亦然。 (TGT-52754)
* 修正[!UICONTROL 頁面傳送]錯誤顯示錯誤訊息「不允許重複的頁面URL」的問題。 (TGT-52765)
* 修正為包含體驗片段的[!UICONTROL 頁面傳送] URL建立對象，且錯誤地附加#的問題。 (TGT-52786)
* 修正在[!UICONTROL 目標與設定]頁面上複製活動及編輯設定導致[!DNL Target] UI無回應的問題。 (TGT-52797)
* 已修正更新的[!UICONTROL 視覺化體驗撰寫器] (VEC)中，不正確地允許將[!UICONTROL A/B測試]活動中的其他頁面重新導向至相同URL的問題。 (TGT-51838)
* 修正編輯活動時，[!UICONTROL 目標與設定]頁面上的量度變更未儲存的問題。 (TGT-52799)
...
* 修正更新[!UICONTROL 視覺化體驗撰寫器] (VEC)中未偵測到`triggerView()`個呼叫的問題。 (TGT-52575)
* 已修正更新的[!UICONTROL 視覺化體驗撰寫器] (VEC)中，使用者無法對[!UICONTROL 單頁應用程式] (SPA)檢視新增修改的問題。 (TGT-52556)
* 已修正更新[!DNL Target] UI中導致客戶無法檢視優惠方案詳細資料的問題。 (TGT-52607)
* 已修正對[!UICONTROL 選件資料庫]中選件所做的更新未反映在更新的[!UICONTROL 視覺化體驗撰寫器] (VEC)中的問題。 (TGT-52637)
...
* 修正無[!UICONTROL 活動QA]按鈕的活動顯示空白選項選取器的問題。 (TGT-52733)
* 修正QA連結無法正確呈現內容的問題。 (TGT-52718)
* 修正使用體驗片段取代元素時，未正確反映QA環境變更的問題。 (TGT-52762)
* 已修正更新的[!UICONTROL 視覺化體驗撰寫器] (VEC)中，當使用者嘗試新增體驗片段時，造成「無效的輸入」錯誤的問題。 (TGT-52701)
* 修正在更新的[!UICONTROL 視覺化體驗撰寫器] (VEC)中編輯對象鎖定目標時，「編輯對象」強制回應視窗顯示為空白的問題。 (TGT-52749)
* 新增訊息，以在選取的工作區中無法存取實體時通知使用者。 (TGT-52767)
* 修正UI無法允許手動指派環境ID至條件的問題。 而是預設為[!UICONTROL 產品目錄搜尋]主機群組的ID。 此修正可確保條件變更現在會套用至所有環境，而不僅僅是預設環境。 (TGT-52817)
* 修正具有建議的[!UICONTROL 體驗鎖定目標] (XT)活動缺少「[!UICONTROL 下載建議資料]」選項的問題。 (TGT-52730 和 TGT-52756)
...
* 修正當頁面無法載入內部網頁時，[!UICONTROL 視覺化體驗撰寫器] (VEC)中無法存取修改的問題。 (TGT-52488 &amp;TGT-52470)
* 修正VEC中較小熒幕大小上看不到[!UICONTROL 修改]面板的問題。 (TGT-52470)
* 已修正更新的VEC中，從[!UICONTROL 瀏覽]模式切換回[!UICONTROL 設計]模式時，造成主控台錯誤及無法進一步互動的問題。 (TGT-52532)
...
* 已修正導致活動的[!UICONTROL 總覽]頁面顯示[!UICONTROL 活動位置]的無效URL的問題。 (TGT-52695)
* 修正更新的[!DNL Target] UI中，造成活動位置出現重複專案的問題。 (TGT-52693)
* 修正觸發`getAudiencesV3`錯誤，導致客戶無法編輯或複製活動的問題。 (TGT-52709)
* 修正在將[!UICONTROL 體驗片段]或HTML選件新增至活動時，造成無效裝載錯誤的問題。 (TGT-52779 和 TGT-52773)
* 修正更新的[!DNL Target] UI中，由於無效輸入錯誤，導致E[!UICONTROL 體驗片段]無法正確顯示的問題。 (TGT-52701)
* 修正由於使用者錯誤無效，導致客戶無法在[!UICONTROL 表單式體驗撰寫器]中編輯活動的問題。 (TGT-52470)
...
* 已在[!UICONTROL 報表] > [!UICONTROL 報表設定] （ ![報表設定圖示](/help/main/assets/icons/Setting.svg)）下新增QA模式流量設定，以符合舊版[!DNL Target] UI的功能。 (TGT-52228 和 TGT-52329)
...
* 修正更新的[!UICONTROL 視覺化體驗撰寫器] (VEC)無法自動偵測`at_property`值的問題。 (TGT-52347)
* 修正在與表單元素互動時，在VEC中的[!UICONTROL 瀏覽]和[!UICONTROL 設計]模式之間切換後，若預期只有一個修改，則會記錄兩個修改的問題。 (TGT-52455)
* 修正由於錯誤指出選取器無效、已使用或未顯示，而無法選取更新VEC中[!UICONTROL 已按一下元素]設定的問題。 (TGT-52467)
* 修正在更新的VEC中新增[!UICONTROL 建議選件]方塊導致顯示重複(Ghost)方塊的問題。 在體驗A和B之間切換時，會重複新增更多GhostBox。 (TGT-52505 和 TGT-52519)
* 修正更新[!DNL Target] UI中，透過[!UICONTROL 選件]功能表對HTML選件進行的變更未反映在相關活動中（反之亦然）的問題。 此行為現在與舊版UI相符，其中更新在[!UICONTROL 選件]功能表與活動之間正確同步。 (TGT-52540 和 TGT-52541)
* 修正嘗試在活動中使用[!UICONTROL 選件資料庫]中的[!UICONTROL 體驗片段]最近更新時，未反映的問題。 (TGT-52659)
...
* 修正[!UICONTROL 活動]清單中的依名稱搜尋功能無法正確搭配多字查詢運作的問題。 (TGT-52529)
* 修正無法從[!UICONTROL Automated Personalization] (AP)活動中排除體驗的問題。 (TGT-52383)
* 修正管理AP活動中的內容時，「[!UICONTROL 篩選規則]」中缺少「[!UICONTROL 包含]」選項的問題。 (TGT-52384)
* 修正[!UICONTROL Automated Personalization] (AP)活動中報表不一致的問題，特別是有關如何使用[!DNL Target]內部系統的`optionLocalId`值追蹤及報告預設優惠的問題。
* 修正QA連結無法傳遞預期活動體驗的問題。 (TGT-52163)
* 修正具有[!UICONTROL 核准者]許可權的使用者無法正確編輯已上線活動的問題，收到「存取遭拒」錯誤訊息。 (TGT-52416)
...
* 修正順序條件無法在[!UICONTROL 視覺化體驗撰寫器] (VEC)中正確顯示[!DNL Recommendations]活動的問題。 (TGT-52435)
...
* 具有[!UICONTROL 產品管理員]和[!UICONTROL 系統管理員]許可權的[!DNL Target]使用者現在可以編輯[!UICONTROL 管理]頁面上的所有設定，無論他們在[!DNL Target]中的角色為何。 沒有這些許可權的使用者擁有這些設定的唯讀存取權。 此更新可確保對[系統管理設定](/help/main/administrating-target/administrating-target.md)進行更嚴格的存取控制。 (TGT-48179)
* 修正無法儲存活動[網站偏好設定](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings)的快取問題。 (TGT-52213)
* 修正在VEC中載入網站後，客戶無法在[!UICONTROL 網站偏好設定]區段中依ID和類別啟用選取的問題。 [!UICONTROL 網站偏好設定]設定會在啟用後自動回覆為停用。 (TGT-52207)
* 修正[頁面傳送](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings)URL以正斜線(/)結尾時，[!UICONTROL 視覺化體驗撰寫器] (VEC)無法顯示正確頁面的問題。 (TGT-52237)
...
* 已修正封鎖[!DNL Customer Journey Analytics]在活動[!UICONTROL 概覽]頁面上正確顯示的問題。 (TGT-52359)
* 修正報表群組無法在[!UICONTROL Automated Personalization] (AP)活動中持續存在的問題。 (TGT-52368)
* 修正無法儲存包含Offer Decisioning之活動的問題。 (TGT-52390)
* 修正已選取預設選件，但在[!UICONTROL Automated Personalization] (AP)和[!UICONTROL 多變數測試] (MVT)活動中顯示其他選件內容的問題。 (TGT-52372)
* 修正GET許可權邏輯，以在完整組織存取權與特定組織+使用者存取權之間檢查或。 (TGT-52374)
* 修正為[!UICONTROL 受管理的內容]和[!UICONTROL 報表對象]選取對象後未顯示對象名稱的問題，即使已啟用[!UICONTROL 僅顯示選取的專案]。 (TGT-52393)
...
* 修正[!UICONTROL 活動]設定頁面與[!UICONTROL 報表]總覽頁面之間對象清單不一致的問題。 (TGT-52203)
...
* 已修正導致活動[!UICONTROL 概覽]頁面上的位置不正確的問題。 (TGT-52182)
...
* 修正在新增其他頁面然後導覽回[!UICONTROL 體驗]索引標籤後，修改消失的問題。 (TGT-52264)
* 修正阻止客戶在[!UICONTROL 體驗鎖定目標] (XT)活動中變更對象的問題。 (TGT-52191)
* 修正因不支援的UI規則而無法編輯XT活動的錯誤。 (TGT-52273)
* 已修正更新[!UICONTROL 視覺化體驗撰寫器] (VEC)中，在編輯器底部不一定會顯示階層連結，而造成精確選取元素困難的問題。 (TGT-52169)
* 修正[!UICONTROL 對象]下拉式清單因分頁而無法顯示所有對象的問題。 (TGT-52204)
* 修正在[!UICONTROL Automated Personalization] (AP)活動中新增優惠方案時，造成使用者輸入訊息無效的問題。 (TGT-52210)
* 修正客戶無法存取A4T時，[!UICONTROL Analytics for Target] (A4T)未正確選取為報表來源的問題。 (TGT-52226)
* 修正無法以[!UICONTROL 檢視頁面] URL量度儲存活動的問題。 (TGT-52260)
...
自2025年4月22日起，只有[!UICONTROL 產品]和[!UICONTROL 解決方案]管理員能夠更新[!UICONTROL 管理]區段中的設定，無論他們在[!DNL Target]工作區中的角色為何。 沒有此許可權的使用者將擁有[!UICONTROL 管理]區段的唯讀存取權。
...
* 修正具有[!UICONTROL 編輯者]許可權的客戶無法儲存活動的問題。 (TGT-52227)
...
* 修正導致客戶無法開啟特定[!UICONTROL 體驗鎖定目標] (XT)活動的對象資訊快顯視窗的錯誤。 (TGT-52049)
* 修正在[!UICONTROL 視覺化體驗撰寫器] (VEC)中完成變更後，對象設定恢復為&quot;[!UICONTROL 所有訪客]&quot;的問題。 (TGT-52132)
* 修正未針對特定活動顯示對象細分的問題(TGT-52057)
* 修正客戶無法在預設工作區中插入[!UICONTROL 體驗片段]的問題。 (TGT-52073)
* 針對[!UICONTROL Automated Personalization] (AP)活動，修正選件顯示為「找不到內容」且未顯示在[!UICONTROL 選件]頁面的問題。 (TGT-52150)
* 新增在活動中允許重複受眾的功能。 (TGT-51200)
* 修正編輯後XT活動的[!UICONTROL 目標與設定]頁面上顯示錯誤mbox名稱的問題。 (TGT-52026)
* 修正`defaultContent`不在`experiences/optionLocations`中卻顯示在選項中的問題。 (TGT-52036)
* 修正確保空白字串未轉換為Null值的問題。 (TGT-52037)
* 已修正要求客戶在編輯後重新設定[!UICONTROL 目標與設定]頁面上[!UICONTROL 報告設定]中的[!UICONTROL 最佳化目標]的問題。 (TGT-52071)
* 修正無頁面傳送規則的活動在[!UICONTROL 總覽]頁面上顯示多個規則的問題。 (TGT-52084)
* 新增錯誤訊息，說明使用者嘗試以基本多語言平面以外的字元（例如表情符號）儲存優惠方案。 (TGT-52105)
* 修正開啟活動觸發錯誤訊息的問題：「此活動使用一或多個在來源中刪除的對象。」 (TGT-52120)
* 修正在編輯期間，ClickTrack量度未顯示在更新的[!UICONTROL 視覺化體驗撰寫器] (VEC)中的問題。 (TGT-52152)
* 修正將查詢引數作為活動位置的URL未在活動的[!UICONTROL 概覽]頁面上顯示查詢引數的問題。 (TGT-51635)
* 修正無法在[!UICONTROL 視覺化體驗撰寫器] (VEC)的[!UICONTROL 瀏覽模式]中顯示整個體驗URL的問題。 (TGT-52101)
* 修正編輯活動導致頁面傳送在URL結尾新增「/」使其無效的問題。 (TGT-52114)
* 修正[!UICONTROL 表單式體驗撰寫器]中的[!UICONTROL 活動QA]連結不正確地重新導向至[!DNL Adobe Experience Cloud]首頁的問題。 (TGT-52055)
* 修正儲存及重新開啟後，新增至[!UICONTROL A/B測試]活動的其他頁面未保留的問題。 (TGT-51994)
* 已修正導致客戶無法刪除內嵌樣式區段中的樣式的問題。 (TGT-52070)
* 已還原存取[!UICONTROL 活動QA]對話方塊中的[對象定義卡](/help/main/c-target/c-audiences/audiences.md#section_11B9C4A777E14D36BA1E925021945780)，類似於舊版UI。 (TGT-52056)
...
* 修正客戶無法在Ove[!UICONTROL r]檢視頁面的鎖定目標圖形中檢視對象，以及在活動編輯期間檢視對象的問題。 (TGT-52050)
* 已修正導致客戶無法在[!UICONTROL 體驗鎖定目標] (XT)活動中依優先順序重新排序體驗的問題。 (TGT-52054)
* 修正復原文字樣式變更時，造成轉譯錯誤的問題。 (TGT-51876)
* 修正修改重新導向選件時，[!DNL Target]也會移除與該選件相關聯的所有[!UICONTROL ClickTrack]選取器的問題。 (TGT-51936)
* 修正取消[!UICONTROL ClickTrack]時，[!DNL Target]無法正確儲存選取器的問題。 (TGT-51937)
* 修正在[!UICONTROL 目標與設定]頁面上開啟和關閉mbox選擇器，但未進行任何變更後，觸發無效名稱錯誤的問題。 (TGT-51983)
...
* 修正[!UICONTROL 「插入在前」]選項無法用於導覽列前插入內容的問題。 (TGT-52031)
...
* 解決造成[!UICONTROL 活動]頁面載入緩慢的問題。 (TGT-51151)
...
* 解決在活動建立或編輯期間，將對象取代為「[!UICONTROL 所有訪客]」所導致「不允許重複的對象」錯誤的問題。 (TGT-51978)
* 解決儲存[!UICONTROL A/B測試]活動時造成「使用者輸入無效」錯誤的問題。 (TGT-51976)
* 解決計算量度無法在[!UICONTROL 目標與設定]頁面上正確顯示的問題。 (TGT-51975)
* 已解決無法在`pageviews`量度的[!DNL Analytics]設定中比對`companyName`與`reportSuite`的問題。 (TGT-51965)
* 解決在活動中切換體驗時移除修改的問題。 (TGT-51945)
* 解決移除頁面對象時也移除了[!UICONTROL ClickTrack]選取器的問題。 (TGT-51935)
* 解決在開啟活動的[!UICONTROL 總覽]頁面後無法編輯活動的問題。 (TGT-51931)
* 已解決在建立活動期間造成`[Unused optionLocalIds: 0]]`錯誤的問題。 (TGT-51920)
* 解決移除文字樣式變更後，部分變更未正確轉譯的問題。 (TGT-51876)
* 已解決導致目標對象無法在[!UICONTROL 表單式體驗撰寫器]中正確更新的問題。 (TGT-51845)
* 解決[!UICONTROL 視覺化體驗撰寫器]中的URL在活動導覽期間未正確更新的問題。 (TGT-51832)
* 解決在建立活動和新增選件時，無法正確顯示選件至[!UICONTROL 選件] UI的問題。 (TGT-51805)
* 解決當無法傳遞個人化或目標內容時，部分活動缺乏後援畫面以顯示預設內容的問題。 (TGT-51638)
* 已解決導致即時優惠方案和某些資料夾無法在[!UICONTROL 優惠方案] UI中正確顯示的問題。 (TGT-51628)
* 解決部分URL字串和goURL無法正確當地語系化的問題。 (TGT-35741)
* 修正了無法在[!DNL Target] UI中正確本地化角色（[!UICONTROL 核准者]、[!UICONTROL 編輯者]和[!UICONTROL 觀察者]）的問題。 (TGT-29925)
...
* 已解決阻止使用者變更[!UICONTROL 修改]面板中選件的問題。 (TGT-51800)
* 解決動作在體驗和對象（包括[!UICONTROL ClickTrack]模式）的左側面板中無法正確顯示的問題。 (TGT-51895)
...
* 解決在[!UICONTROL 對象]面板中看不到僅限於此活動的對象的問題，使其無法編輯或重複使用。 (TGT-51860)
* 已修正封鎖[!DNL Target Standard]客戶使用[!UICONTROL Analytics for Target] (A4T)報告來建立活動的問題。 (TGT-51854)
...
* 修正無法編輯僅限於此活動受眾的[!UICONTROL 體驗鎖定目標] (XT)活動的問題。 (TGT-51846)
* 修正[!UICONTROL 視覺化體驗撰寫器] (VEC)在第一次編輯時無法正確套用修改至體驗的問題。 (TGT-51843)
* 修正按一下VEC內特定元素時觸發「ID」錯誤的問題。 (TGT-51814)
* 在活動建立期間更新VEC中的錯誤處理。 (TGT-51759)
* 修正[!UICONTROL 修改]面板中缺少檢視在儲存活動時導致「無效的使用者輸入」錯誤的問題。 (TGT-51827)
...
* 合併的對象可包含子群組，每個子群組都包含多個對象。 此版本修正子群組對象無法在[!UICONTROL 規則]對話方塊中顯示的問題。 (TGT-51813)
* 解決在開啟舊版活動時，某些體驗對象已更換為[!UICONTROL 所有訪客]的問題。 (TGT-51812)
* 已解決無法編輯僅限於此活動的受眾之活動的問題。 (TGT-51807)
* 解決無法在更新的[!DNL Target] UI中編輯頁面標題修改的問題。 (TGT-51797)
* 解決複製體驗、刪除其他體驗，然後嘗試儲存活動時發生的Null錯誤。 (TGT-51796)
* 解決在建立活動的[!UICONTROL 鎖定目標]步驟期間，對象排除規則無法顯示在對象資訊面板中的問題。 (TGT-51579)
...
* 解決取消活動建立程式時移除所有狀態中對象變更的問題（選取「[!UICONTROL 取消]」而非「[!UICONTROL 新增對象]」）。 (TGT-51769 和 TGT-51770)
* 解決某些活動（尤其是使用自訂程式碼時）無法載入[!UICONTROL 視覺化體驗撰寫器] (VEC)的問題。  問題導致VEC顯示空白畫面或[!DNL Target] UI恢復為舊版。 (TGT-51758)
* 解決編輯對象的頁面傳送後，捨棄修改的問題。 (TGT-51756)
* 解決在[!UICONTROL 目標與設定]頁面上變更量度型別時，從活動中移除所有非量度對象（頁面和體驗對象）的問題。 (TGT-51753)
* 解決在編輯活動時按一下[!UICONTROL 取消]將Target UI導覽至[!UICONTROL 活動清單]，而非[!UICONTROL 活動詳細資料]頁面的問題。 (TGT-51731)
* 解決客戶無法透過[!UICONTROL 將報表匯出至CSV]選項下載報表的問題。 (TGT-51708)
* 解決表單式體驗撰寫器中顯示[!DNL Target Standard]客戶誤用[!UICONTROL 屬性] （一種[!DNL Target Premium]功能）的問題。 (TGT-51678)
* 修正建立新優惠方案時無法顯示[!DNL Adobe Experience Platform]屬性的問題。 (TGT-51665)
* 已將[!DNL Recommendations]詳細目錄的所有作用中篩選器移至快速搜尋，將UI與[!UICONTROL 目錄搜尋]對齊，而非[!UICONTROL 篩選器]邊欄。 (TGT-50723)
...
* [!UICONTROL 活動]使用者介面更新
* [!DNL Recommendations]使用者介面更新

#### [!UICONTROL 活動]使用者介面更新

隨著[!DNL Adobe Target] UI現代化工作的繼續，我們很高興宣佈已正式發行更新的[!UICONTROL 活動]使用者介面。

>[!NOTE]
>
>自2月17日起，客戶將可逐步存取新的[!UICONTROL 活動] UI。 為確保所有客戶都能順暢地推出，此版本將在受控階段中部署。 第一個階段會將最初的[!DNL Target]客戶群組升級至新的[!UICONTROL 活動] UI。 後續階段將升級其餘客戶。
...
* [[!UICONTROL 已更新變更記錄]](/help/main/c-activities/change-log.md)頁面，現在從[[!DNL Audit Query API]](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/landing/governance-privacy-security/audit-logs/audit-api/overview){target=_blank}取得即時深入分析的資訊。
...
>[!NOTE]
>
>自2月17日起，客戶將可逐步存取新的[!UICONTROL Recommendations] UI。 為確保所有客戶都能順暢地推出，此版本將在受控階段中部署。 第一個階段會將最初的[!DNL Target]客戶群組升級至新的[!UICONTROL 活動] UI。 後續階段將升級其餘客戶。

更新會根據最新[!DNL Adobe Spectrum]設計系統，將之前不一致的設計模式標準化，同時加入新的增強功能，例如：

* [產品目錄搜尋](/help/main/c-recommendations/c-products/catalog-search.md)現在提供更新的資料庫，允許產品即時同步。
* 透過API建立的[!UICONTROL Recommendations]物件（[!UICONTROL 條件]、[!UICONTROL 設計]、[!UICONTROL 集合]和[!UICONTROL 排除專案]） [現在可在UI](/help/main/c-recommendations/c-recommendations-faq/recommendations-faq.md)中使用。
* [Recommendations設定](/help/main/administrating-target/recommendations-settings.md)已整合至[!UICONTROL 管理]區段下。
...
#### [!UICONTROL 優惠資料庫]使用者介面更新

為了增強[!DNL Adobe Target]使用者的使用者體驗，此版本會更新[!UICONTROL 優惠資料庫]使用者介面。
...
* **[!UICONTROL 程式碼編輯器]升級**：使用語法醒目提示和行編號重新整理HTML和JSON編輯器。
...
* 使用JSON選件
* 在[[!UICONTROL 表單式體驗撰寫器]](/help/main/c-experiences/form-experience-composer.md)中建立的活動中使用自訂決定範圍
* 擷取使用全域`__view__`範圍的[!UICONTROL 表單式體驗撰寫器]建立的活動時，未使用`renderDecisions: true`
...
### [!UICONTROL 視覺化體驗撰寫器] Helper擴充功能（2024年4月23日）

舊版[!DNL Target]視覺化體驗撰寫器Helper擴充功能是使用Manifest V2建立的。 [!DNL Google]宣佈從2024年6月起，將不再允許使用資訊清單V2建立的擴充功能。 如需詳細資訊，請參閱[[!UICONTROL 視覺化體驗撰寫器]協助程式擴充功能](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/vec-helper-browser-extension.md)。
...
### [!UICONTROL 瀏覽器]對象屬性中`Browser:iPad`和`Browser:iPhone`的更新（2024年4月30日）
...
* 修正在[!UICONTROL 視覺化體驗撰寫器] (VEC)中執行撰寫時，導致擴充功能重複Cookie的問題。
...
* 修正造成[!UICONTROL Multivariate] (MVT)活動(以[!UICONTROL Analytics for Target] (A4T)報告設定)無法正確顯示的問題。 (TGT-47490)
...
* 具有收入目標量度的[!UICONTROL Analytics for Target] (A4T)活動未顯示「收入」做為欄名稱，且收入量度未在報表中以($)格式顯示。 這是已經解決的外觀問題。 (TGT-46995)
* 修正導致報告日期間隔無法正常運作的問題。 (TGT-47396)
* 修正了客戶使用[!UICONTROL 更多動作]圖示啟動或停用活動後，[!UICONTROL 所有活動]頁面上顯示錯誤狀態的問題。 (TGT-47367)
* 修正造成[!UICONTROL 重要屬性]報告無法針對單一客戶顯示的問題。 (TGT-47272)
...
作為[!DNL Adobe Target]團隊持續努力改善[!DNL Target]個使用者的使用者體驗的一部分，此版本重新整理[!DNL Target] UI中的[!UICONTROL 活動]頁面。 本次更新統一並標準化先前不一致的設計模式，同時加入了新的增強功能。
...
* 已增強新的[!UICONTROL 活動] UI，以便在建立新活動時以`selectorCriteria`的預設設定開啟[!UICONTROL 視覺化體驗撰寫器] (VEC)。 (TGT-46586)
* 已修正導致某些客戶在使用VEC時無法在[!UICONTROL Composer]模式中編輯元素的問題。 (TGT-46470)
* 已新增使用自訂屬性時可指定通用偏好選擇器的功能。 (TGT-46545)
* 修正有時候使用[!UICONTROL Analytics for Target] (A4T)的[!UICONTROL 自動鎖定目標]報告無法在[!DNL Target] UI中顯示的問題，即使該報告在[!DNL Adobe Analysis Workspace]中正確顯示亦然。 (TGT-46494)
...
* 已新增動態架構的[!UICONTROL 視覺化體驗撰寫器] (VEC)增強功能。 (TGT-44064)
...
* 已修復使 VEC 操作以錯誤順序應用的問題。 在某些情況下，VEC以非同步方式套用了一些修改，而且如果元素在[!UICONTROL 插入]動作之後顯示，則對元素新增額外修改會導致錯誤。 也修復了 VEC URL，現在按一下錨點連結時就會更新。 (TGT-45983)
* 已修正VEC [!UICONTROL 覆蓋]功能的問題，此功能現在支援Shadow DOM中的元素。 (TGT-45202 和 TGT-45262)
* 修正在VEC中開啟單頁應用程式(SPA)頁面，然後移至[!UICONTROL 瀏覽]模式時，導致「上一頁」和「下一頁」箭頭無法正常運作的問題。 (TGT-45956)
...
* 已修正在[!UICONTROL 自動分配]活動(使用[!UICONTROL Analytics for Target] (A4T)作為報表來源)的[!DNL Target] UI和[!DNL Adobe Analytics] UI中造成不一致的報表資料的問題。 (TGT-46112)
...
* 如[!UICONTROL 活動]清單頁面的[!UICONTROL 狀態]欄中所示，修正有時無法正確同步活動的問題。 (TGT-46010 和 TGT-44831)
* 修正在使用[!UICONTROL Analytics for Target] (A4T)作為報告來源的活動的[!UICONTROL 報告]頁面上，有時無法顯示&quot;[!UICONTROL 在Analytics中檢視]&quot;連結的問題。 (TGT-45808)
* 調整表格中值的顯示方式，現在以百分比形式顯示，而不是帶小數點的數字。 例如，顯示 8%，而非顯示 .08。 (TGT-45548)
* 已修正導致客戶無法使用鍵盤焦點移至[!UICONTROL 體驗鎖定目標] (XT)活動的[!UICONTROL 目標與設定]頁面中的下一個元素的問題。 (TGT-44526)
* 修正在建立活動時，在開啟&quot;[!UICONTROL 新增對象]&quot;對話方塊後，導致鍵盤失去焦點的問題。 (TGT-44525)
...
| 功能 | 詳細資料 |
|--- |--- |
| [!UICONTROL Automated Personalization]活動的[!UICONTROL QA模式] | [!DNL Adobe Target] [!UICONTROL QA模式]現在可用於[!UICONTROL Automated Personalization]活動，取代[!UICONTROL 預覽連結]功能。<P>如需詳細資訊，請參閱[活動 QA](/help/main/c-activities/c-activity-qa/activity-qa.md)。 |

* 效能增強功能，在[!UICONTROL Automated Personalization]活動中[管理排除專案](/help/main/c-activities/t-automated-personalization/managing-exclusions.md#concept_4EF78013F80E48EFA024AE0274C9F037)時，不允許重複功能（包括減少載入時間）。
...
* 修正當影像名稱包含GB 18030字元時，[!UICONTROL 建立設計]面板無法顯示影像的問題。 (TGT-44614)
* 已修正體驗中某些 GB 18030 符號字元在文字/HTML 中錯誤逸出的問題。 (TGT-44600)
* 修正造成[!UICONTROL 自動Personalization]活動報告在分析期間凍結的問題。 (TGT-44820)
* 修正當活動名稱包含方括弧（ [或] ）時，無法在[!UICONTROL 活動]頁面上搜尋活動的問題。 (TGT-44777)
...
* 修正將查詢引數新增至URL時，頁面無法在[!UICONTROL 視覺化體驗撰寫器] (VEC)中載入的問題。 (TGT-44873)
* 對整個 [!DNL Target] UI 進行了各種本地化修正。

### 與[!DNL Target]共用的Real-Time CDP設定檔屬性[!UICONTROL Real-Time CDP設定檔屬性] （2023年6月13日）

此版本包含下列增強功能：

| 功能 | 詳細資料 |
|--- |--- |
| 與 [!DNL Target] 共用 Real-Time CDP 輪廓屬性 | [!UICONTROL Real-Time CDP設定檔屬性]可以與[!DNL Target]共用，以用於HTML和JSON選件。<P>如需詳細資訊，請參閱[與  [!DNL Target]](/help/main/c-integrating-target-with-mac/integrating-with-rtcdp.md#rtcdp-profile-attributes) 共用 Real-time CDP 輪廓屬性。 |
...
| 功能 | 詳細資料 |
|--- |--- |
| 適用於Headless個人化和實驗的AEM [!UICONTROL 內容片段] | 在[!DNL Target]個活動中使用[!DNL Adobe Experience Manager] (AEM) [!UICONTROL 內容片段]。 將 AEM 的易用性和強大功能與 [!DNL Target] 的強大人工智慧 (AI) 和機器學習 (ML) 功能相結合，以協助 Headless 個人化和實驗。<P>如需詳細資訊，請參閱[AEM [!UICONTROL 內容片段]](/help/main/c-integrating-target-with-mac/aem/content-fragments-aem.md)。 |
| [*Adobe Target 開發人員指南*](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hant){target=_blank} | *Adobe Target開發人員指南*&#x200B;已重新放置到&#x200B;*[!UICONTROL Adobe Experience League]*。 移至&#x200B;*[!UICONTROL Experience League]*&#x200B;可協助當地語系化其他語言的文字，在&#x200B;*Experience League*&#x200B;中統一搜尋範圍，並提供來自&#x200B;*[!UICONTROL Adobe Target商業從業者指南]*&#x200B;和&#x200B;*[!UICONTROL Adobe Target開發人員指南]*&#x200B;的搜尋結果，並提供額外的好處。<P>系統會自動將您從先前的位置重新導向至&#x200B;*[!UICONTROL Experience League]*。 請視需要更新您的書籤。 |
...
* 已停用[!UICONTROL Done]」選項，因為在建立[合併的對象](/help/main/c-target/combining-multiple-audiences.md)時，可在[!DNL Target] UI中看到載入迴轉器。 (TGT-44079)
* 修正[!UICONTROL 對象]頁面底部的[!UICONTROL 語言]連結，使其正確連結至「[!UICONTROL 帳戶通訊偏好設定]」頁面。 (TGT-43562)
* 解決在[!UICONTROL 管理] > [!UICONTROL 報告] > [!UICONTROL 報告Experience Cloud解決方案]下選取[!UICONTROL Adobe Analytics]選項後，有時會讓客戶無法建立[!UICONTROL A/B測試]活動的問題。 (TGT-44844)
* 已修正導致客戶無法從[!UICONTROL 視覺化體驗撰寫器] (VEC)中檢視具有多個體驗的[!UICONTROL 多變數測試]活動中最後一個體驗的問題。 VEC 底部的 [DOM 路徑](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)有時會造成客戶無法看到最後一個體驗。 (TGT-44578)
* 如果頁面要求授權或叫用重新導向，VEC 中的瀏覽 URL 無法反映目前頁面，而此頁面在正常瀏覽器工作階段是可見的，這個問題已經修正。 (TGT-44350)
* 已修正導致客戶無法變更「[!UICONTROL 建議]」>「[!UICONTROL 設定]」中的「[!UICONTROL 篩選不相容的條件]」設定的問題。 (TGT-44398)
* 修正使用名稱中包含點的[!UICONTROL Analytics分類]報表套裝時，造成POST要求建立[!DNL Recommendations]摘要失敗的問題。 (TGT-44598)
...
* 已修正在[!UICONTROL A/B測試]和包含建議的[!UICONTROL 體驗鎖定目標] (XT)活動中導致「500錯誤」的問題。 當 [!DNL Target] 無法從 [!DNL Target] UI 和 [!DNL Recommendations] 後端正確地刪除不再使用的標準物件時，即導致此問題。 (TGT-44383)
* 已從[!UICONTROL 選件層級]報告中移除[!UICONTROL Automated Personalization]活動顯示的選件名稱中移除位置。 此變更使報告更易讀取。 (TGT-44294)
* 已從AP和[!UICONTROL 自動鎖定目標] [!UICONTROL Personalization Insights]和[!UICONTROL 重要屬性]報告（在[!DNL Target] UI中）移除45天和90天行事曆選項。 由於使用模式並且為了改善效能，這些日期範圍已被取代。 UI 已更新以反映目前允許的範圍：15、30 和 60 天。 (TGT-39357)
* 已不允許在活動上線後變更[!UICONTROL 目標與設定]頁面上的[!UICONTROL 與最佳化目標相同]設定的功能。 (TGT-43923)
...
* 已變更「使用裝置上決策的實作方法」的[!UICONTROL 實作]頁面（[!UICONTROL 管理] > [!UICONTROL 實作]）上的連結，以指向說明如何使用裝置上決策來操作所有受支援SDK （Node.js、Java、.NET和Python）的頁面。 如需詳細資訊，請參閱[Adobe Target開發人員指南](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hant){target=_blank}中的[Target SDK快速入門](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/getting-started.html?lang=zh-Hant){target=_blank}。
...
新的[!DNL Adobe Target]模型API （也稱為Blocklist API）可讓使用者檢視和管理機器學習模型中用於[!UICONTROL Automated Personalization] (AP)和[!UICONTROL 自動鎖定目標] (AT)活動的功能清單。
...
* 在[!DNL Target] UI中新增工具提示，以幫助客戶在建立或編輯使用A4T的[!UICONTROL 自動鎖定目標]活動時選取最佳化條件。
...
| 功能 | 詳細資料 |
| --- | --- |
| [!DNL Adobe Experience Manager] (AEM) 體驗片段 | AEM 體驗片段功能的更新包括以下內容：<ul><li>已新增在[!UICONTROL 選件]清單中按型別（HTML或JSON）篩選AEM體驗片段的功能。 (TGT-43121)</li><li>已修正允許客戶在使用VEC時插入JSON [!UICONTROL 體驗片段]選件的問題，此狀況不受支援。 只有使用[!UICONTROL 表單式體驗]撰寫器時，才能插入JSON選件。 (TGT-43846)</li></ul>如需詳細資訊，請參閱 AEM [體驗片段](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)。 |
| 適用於Google Chrome的新[!UICONTROL Visual Experience Composer]擴充功能 | Chrome網站商店中提供適用於Chrome的新[!DNL Adobe Target] [!UICONTROL Visual Experience Composer] (VEC)擴充功能。<br>自2023年1月起，目前的[!DNL Target] VEC Helper擴充功能將停止在Google Chrome中運作，因為Google不允許使用Manifest V2的擴充功能。 下載新的擴充功能，從新的一年開始，繼續在 [!DNL Target] 中以視覺方式創作您的網站。<br>以下連結顯示 Chrome 網站商店中的兩個擴充功能：<ul><li>[新擴充功能](https://chrome.google.com/webstore/detail/adobe-experience-cloud-vi/kgmjjkfjacffaebgpkpcllakjifppnca){target=_blank}</li><li>[舊的擴充功能](https://chrome.google.com/webstore/detail/adobe-target-vec-helper/ggjpideecfnbipkacplkhhaflkdjagak){target=_blank}</li></ul>如需詳細資訊，請參閱「[Visual Editing Helper 擴充功能](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)」。 |
...
* 修正從[!UICONTROL 構成]切換到[!UICONTROL 瀏覽]模式後，在VEC中建立或編輯活動時，造成[!UICONTROL 體驗]頁面上的[!UICONTROL 修改]面板中無法正確顯示某些元件的問題。 (TGT-43300)
* 已修正導致部分客戶無法封存使用[!UICONTROL 自動鎖定目標]的[!UICONTROL A/B測試]活動的問題。 (TGT-40978)
...
* 新增在下載at.js 2.10.0 （和更新版本）以允許或停用設定第三方Cookie時的[!UICONTROL 跨網域]選項。 (TGT-43674)
* 更新 [!DNL Target] UI 中的通知以在匯入 [!DNL Recommendations] 摘要失敗時告知客戶。 (TGT-35811)
* 修正造成[!UICONTROL 決定選件]在[!UICONTROL 視覺化體驗撰寫器] (VEC)中無法正常運作的問題。 (TGT-43866)
* 修正在建立[!UICONTROL Multivariate Testing] (MVT)活動時，選取[!UICONTROL 按一下元素]轉換目標時導致顯示錯誤訊息的問題。 (TGT-43842)
* 修正無法針對[!UICONTROL Automated Personalization] (AP)活動在下載的CSV報告檔案中顯示[!UICONTROL 曝光數]欄的問題。 (TGT-43780)
* 修正當使用[!UICONTROL 表單式體驗撰寫器]時，客戶在複製體驗後無法編輯HTML/JSON優惠的問題。 (TGT-43633)
* 修正客戶無法從非預設工作區複製[!UICONTROL A/B測試]活動至另一個非預設工作區的問題。 (TGT-41910)
* 修正了一個問題，以確保客戶可以在[!UICONTROL A/B測試]和[!UICONTROL 體驗鎖定目標] (XT)活動中正確顯示包含建議的[!DNL Recommendations]物件（設計、條件、集合等）的使用情況，並從[!DNL Target] UI和[!DNL Recommendations]後端刪除不再使用的條件物件。 (TGT-42331)
...
* 具有[!UICONTROL 編輯者]角色的使用者無法再編輯已上線活動中的對象。 (TGT-43582)
...
* 已針對新的[!UICONTROL 對象]頁面提供增強功能，以防止過去儲存對象的舊資料庫與直接從後端擷取資訊的新架構之間出現不一致狀態。 (TGT-43552)
...
* 修正在[!UICONTROL 體驗鎖定目標] (XT)活動中呈現[!UICONTROL 優惠方案決策]時斷斷續續造成逾時的問題。 (TNT-44611)
...
* 已修正導致一位客戶無法編輯活動的體驗層級微調和受眾的問題（若該微調和受眾是在部署新的[!UICONTROL 受眾] UI之前建立的）。 (TGT-43433)
...
* 您可以使用此版本預先擷取[!UICONTROL 自動Personalization] (AP)和[!UICONTROL 自動鎖定目標] (AT)活動（先前未由[!DNL Target]傳回）的內容。 如果AP/AT活動在傳送路徑上，且優先順序高於使用相同位置進行內容傳送的其他AB/XT活動，這可能會變更一般使用者在預先擷取呼叫情況下看到的體驗（不會變更「執行」流程）。
...
* 新增在將[!UICONTROL 傳送API]與使用[!UICONTROL Analytics作為報告來源] (A4T)的活動搭配使用時，傳回`prefetch`檢視和`pageLoad`點選量度的[!DNL Analytics]裝載資料的功能。 (TNT-43198)
* 更新機器人篩選使用者代理清單，允許日本常用的瀏覽器型別。 (TNT-43867)

### Target Standard/Premium 22.2.1 (2022 年 2 月 1 日)

此維護版本包含以下針對Target Standard/Premium 22.1.2版本中宣佈的新[!UICONTROL 對象] UI的修復和增強功能，該版本將在未來六週內向所有地區的客戶推出。 此修正調整了在 [!DNL Adobe Target Standard/Premium] 中建立的客群功能。

* 修復了無法將從 [!DNL Adobe Experience Platform]、[!DNL Adobe Experience Cloud] 和 [!DNL Adobe Target Classic] 匯入的客群指派為報告客群的問題。 (TGT-43140)
* 針對從[!DNL Adobe Experience Platform]、[!DNL Adobe Experience Cloud]和[!DNL Adobe Target Classic]匯入的對象，在[!UICONTROL 對象]清單中新增[!UICONTROL 刪除]選項。 還新增了大量刪除功能。 (TGT-42914)

### at.js 2.8.1 版 (2022 年 1 月 28 日)

* 修正`pageLoad`在[!UICONTROL 裝置上決策] (ODD)混合執行模式中未對應到target-global-mbox的問題。
...
| [!UICONTROL 對象] UI重新整理 | 作為[!DNL Adobe Target]團隊持續努力改善[!DNL Target]使用者的使用者體驗的一部分，此版本重新整理[!DNL Target] UI中的[!UICONTROL 對象]和[!UICONTROL 個人資料指令碼]頁面。 本次更新統一並標準化先前不一致的設計模式，同時加入了新的增強功能，例如：<ul><li>能夠同時選擇和刪除多個客群</li><li>重新整理的[客群建立器設計](/help/main/c-target/c-audiences/create-audience.md)</li><li>[!UICONTROL 對象]資料庫規則產生器中的排除規則支援</li><li>全新的「對象Source」篩選器，能更快速地探索對象</li><li>工作階段持續搜尋和篩選器選項</li><li>為 [!DNL Target Premium] 客戶在工作區之間移動客群的功能。</li></ul>如需詳細資訊，請參閱[對象](/help/main/c-target/target.md).<br>**注意**：此功能將在未來八週內向不同地區的客戶推出。 |
| [!UICONTROL 設定檔指令碼] UI重新整理 | [!UICONTROL 設定檔指令碼]資料庫也進行了更新，包括一個重新整理的介面和幾項生產力更新：<ul><li>能夠同時選擇和刪除多個輪廓指令碼</li><li>輪廓指令碼的新代碼編輯器</li><li>代碼編輯器中的語法醒目提示和錯誤檢查</li><li>透過鍵盤快捷鍵自動完成權杖 (mbox 或輪廓) 參數</li></ul>如需詳細資訊，請參閱[訪客設定檔](/help/main/c-target/c-visitor-profile/visitor-profile.md)。<br>**注意**：此功能將在未來八週內向不同地區的客戶推出。 |
...
| 功能 | 詳細資料 |
| --- | --- |
| [!UICONTROL 視覺化體驗撰寫器] (VEC) | 新增 [Web 元件](https://developer.mozilla.org/en-US/docs/Web/Web_Components)的支援。 在自訂元素及其內部元素上，可建立和測試個人化體驗和產品建議。<br>如需詳細資訊，請參閱[可視化體驗撰寫器選項](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#custom)。 |
...
* 已修正導致客戶無法透過按一下[!DNL Target]活動報告中的[!UICONTROL 在Analytics中檢視]按鈕來開啟[!DNL Analysis Workspace]中的[!UICONTROL A4T]面板的問題。 (TGT-42099 和 TGT-42100)
* 修正在使用[!UICONTROL 表單式體驗撰寫器]編輯[!UICONTROL A/B測試]和[!UICONTROL 體驗鎖定目標] (XT)活動時，造成[!UICONTROL 編輯設計]按鈕不顯示的問題。 (TGT-41980)
...
透過[!DNL Adobe Experience Platform Web SDK]使用[!DNL Target] [!UICONTROL 對象]時，已新增下列增強功能：
...
| [!UICONTROL 設定檔指令碼] UI重新整理 | [!UICONTROL 設定檔指令碼]資料庫也進行了更新，包括一個重新整理的介面和幾項生產力更新：<ul><li>能夠同時選擇和刪除多個輪廓指令碼</li><li>輪廓指令碼的新代碼編輯器</li><li>代碼編輯器中的語法醒目提示和錯誤檢查</li><li>透過鍵盤快捷鍵自動完成權杖 (mbox 或輪廓) 參數</li></ul>如需詳細資訊，請參閱[訪客輪廓](/help/main/c-target/c-visitor-profile/visitor-profile.md)。 |
| [!BADGE Premium]{type=Positive url="/help/main/c-intro/intro.md#premium newtab=true" tooltip="檢視Target Premium包含的內容。"} Recommendations條件建立和編輯 | [!UICONTROL Recommendations條件]的建立與編輯工作流程已經過簡化，以精簡選擇正確的推薦演演算法和設定以達成目標的過程。<br>如需詳細資訊，請參閱[建立準則](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md)。 |
...
* 修正了阻止[編輯條件]按鈕在VEC中[!DNL Recommendations]活動的[!UICONTROL 體驗]頁面上正確顯示的問題。 (TGT-39512)
* 修正了在重複和複製到測試工作區時阻止活動同步的問題。 (TGT-40686)
* 修正了無法在VEC中使用「[!UICONTROL 插入在後]」時，修改具有[體驗片段](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)的選擇器的問題。 (TGT-41802)
* 修正了阻止將產品中的空 JSON 內容傳送到後端的問題。 [!DNL Target] 現在會傳送 JSON 物件，即使它是空的。 (TGT-41555)
* 修正了當客戶在檢視報告時點選「在Analytics中檢視」時，造成舊版[!DNL Analytics]報告開啟而非[!DNL Analysis Workspace]的問題。 (TGT-41867)
* 當客戶嘗試選取[!DNL Analytics]做為[!UICONTROL Automated Personalization]活動的報告來源(A4T)時，向顯示的UI訊息新增了額外說明。 訊息指出，「[!DNL Target]是[!UICONTROL Automated Personalization]活動唯一支援的來源。」 (TGT-41954)
* 當客戶嘗試使用「新行」而不是逗號分隔主機時，對錯誤訊息新增了額外的說明。 (TGT-40671)
* 修正了導致某些活動的「[!UICONTROL 上次更新]」日期在西班牙文和日文客戶的英文UI中不同的問題（以西班牙文和日文檢視UI時）。 (TGT-38980)
...
* 修正在[!UICONTROL 表單式體驗撰寫器]中建立之[!UICONTROL 自動Personalization]活動的報表，會參照報表中已刪除之優惠的問題。 此問題會導致顯示以下錯誤訊息：「擷取此報表的資料時遇到問題。 如果問題仍然存在，請聯絡 Adobe 客戶服務。」 (TGT-41028)
...
| 功能 | 詳細資料 |
| --- | --- |
| [!UICONTROL Analytics for Target] (A4T) | 從使用[!DNL Analytics]作為報告來源(A4T)的活動按一下[!UICONTROL 報告]頁面上的「[!UICONTROL 在Analytics中檢視]」連結，[!DNL Analysis Workspace]現在會開啟。 之前，連結會開啟 [!DNL Analytics] 報告。 (TGT-36959) |
...
* 修正當重新整理[!UICONTROL 對象]頁面時，導致預設工作區變更為其他工作區的問題。 (TGT-38871)
* 修正[!UICONTROL 管理] > [!UICONTROL 實作]中的問題，有時會導致出現錯誤訊息，指出「您的全域mbox可能不同步。 請嘗試重新儲存。」
...
* 修正將對象變更為[!UICONTROL 所有訪客]後，活動無法同步的問題。 (TGT-40259)
* 修正在[!UICONTROL Automated Personalization]活動中的不同位置使用選件時，導致無法複製選件的問題，即便已啟用「[!UICONTROL 不允許複製]」選項。 (TGT-39567)
* 已修正導致[!UICONTROL 管理] > [!UICONTROL Scene7設定]頁面無法正確載入的問題。 (TGT-39918)
...
* 現在每天都會產生[!UICONTROL 自動鎖定目標] (AT)和[!UICONTROL Automated Personalization] (AP)活動的[!UICONTROL Personalization Insights]報告。 您可以選擇提供過去15、30和60天[!UICONTROL 自動化區段]或[!UICONTROL 重要屬性]的報告。 45 天與 90 天選項已經移除，讓其他的回顧視窗設定可以每日執行。 (TGT-39472)
* 修正造成目前相依性在客戶在活動的[!UICONTROL 目標與設定]頁面上按一下[!UICONTROL 編輯相依性]時不顯示的問題。 (TGT-39340)
* 修正當重新整理工作區的[!UICONTROL 對象庫]時發生的問題。 在重新整理前，目前所選工作區的客群已顯示。 在重新整理後，[!UICONTROL 預設Workspace]及其對象已顯示。 目前工作區及其客群在重新整理後持續存在。 (TGT-38871)
...
* 在[!UICONTROL 自動鎖定目標]活動中使用[!UICONTROL Analytics做為報告來源] (A4T)時，在選取[!DNL Adobe Analytics]量度時新增警告。 [!UICONTROL 自動鎖定目標]模型已最佳化，以搭配二進位（轉換型）量度。 選取連續量度（例如收入）可能會有次優結果，而[!UICONTROL Personalization深入分析]報表可能不準確。 (TGT-38926)
* 已在[!UICONTROL 自動鎖定目標摘要]報告中，為使用A4T的[!UICONTROL 自動鎖定目標]活動新增狀態圖示。 在報告中，每個體驗旁的綠色勾號圖示表示已為該體驗產生的個人化機器學習模型。 時鐘圖示表示，提供的流量還不足以建立模型。 (TGT-38925)
* 已產生使用A4T和[!DNL Analytics]轉換量度的[!UICONTROL 自動鎖定目標]活動的[!UICONTROL 自動化區段]和[!UICONTROL 重要屬性]報告，且看起來與使用[!DNL Target]作為報告來源時相同。 (TGT-38931)
* 已在[!UICONTROL Recommendations] [!UICONTROL 集合]清單中新增環境篩選選項。 (TGT-38353)
* 修正造成[!UICONTROL Recommendations]集合中顯示錯誤產品計數的問題。 (TGT-39162)
* 已新增[!UICONTROL 上次更新時間]篩選器至[!UICONTROL Recommendations] [!UICONTROL 目錄搜尋]。 (TGT-38340)
* 修正[!UICONTROL Recommendations]中，在變更產業垂直後導致[!UICONTROL 建立順序]頁面擱置的問題。 (TGT-38160)
...
* 已修正導致某些客戶無法在[!UICONTROL 預設Workspace]中看到活動清單的問題。 (TGT-38526)
...
| 功能 | 詳細資料 |
| --- | --- |
| ![Premium badge](/help/main/assets/premium.png) 自動鎖定目標活動的 Analytics for Target (A4T) 支援 | [!UICONTROL 自動鎖定目標]活動現在支援[Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)。<br>這項整合可讓您使用[!UICONTROL 自動鎖定目標]整體機器學習演演算法，根據每位訪客的設定檔、行為和內容來選擇最佳體驗。<br>如果您已[實作A4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)以搭配A/B測試和體驗鎖定目標活動使用，則您已準備就緒！<br>如需詳細資訊，請參閱[自動分配和自動鎖定目標活動的A4T支援](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)。 |
...
* 修正在切換組織後載入新的[!UICONTROL 管理]頁面時造成錯誤顯示的問題。 (TGT-37730)
* 修正造成在[!UICONTROL 管理>實作]頁面上顯示錯誤使用者端代碼的問題。 (TGT-37849)
* 修正使用者在成功載入VEC後有時無法在[!UICONTROL 視覺化體驗撰寫器] (VEC)中使用編輯功能的問題。 (TGT-37162)
...
* 修正造成使用者在切換組織後有時無法從[!UICONTROL 管理>實作]頁面下載at.js的問題。 (TGT-37668)
* 現在載入時會停用 at.js 下載按鈕，以防止 [!DNL Target] 在使用者多次按一下下載按鈕時傳送多個請求。 (TGT-37633)
* 修正[!UICONTROL 體驗鎖定目標] (XT)活動中造成體驗在較長時間內顯示「擷取結果」的問題。 (TGT-37684)
...
* 修正阻止某些使用者無法顯示[!UICONTROL 活動]清單的問題。 以下的錯誤消息會顯示：「無法擷取 URLsuggestions。」 在 Adobe 後端系統中，使用者在 其 FirstName(FirstName/r/n) 中使用歸位的使用者發生錯誤。 (TGT-37330)
* 修正當工作區名稱（在Enterprise適用的[!UICONTROL Adobe Admin Console]中指定）包含撇號時，使用者無法顯示[!UICONTROL 活動]頁面的問題。 (TGT-37709)
* 修正在選取最佳化和轉換量度時， [!UICONTROL 自動分配]活動中即使已指定報表套裝，錯誤訊息仍錯誤地通知使用者選取報表套裝的問題。 (TGT-37689)
* 修正在導覽至[!UICONTROL 鎖定目標]頁面後，有時導致[!UICONTROL 目標與設定]頁面上的量度空白的問題。 (TGT-37691)
...
#### [!UICONTROL 管理]區段UI重新整理

我們使用新技術堆疊逐漸重寫整個 [!DNL Target] UI，以提供改善的效能、縮短發布新功能時所需的維護時間，並改善整個產品的使用者體驗。 重新整理的第一個區段是[!UICONTROL Setup]區段，已重新命名為[!UICONTROL 管理]。

在此重新整理中，您將能夠使用[!UICONTROL 管理]區段中的頁面輕鬆執行許多動作，例如：

* 從[!UICONTROL 實作]索引標籤下載最新的at.js檔案（**[!UICONTROL 管理]** > **[!UICONTROL 實作]**）。
* 自訂您的at.js設定，並可輕鬆檢閱變更（**[!UICONTROL 管理]** > **[!UICONTROL 實作]**）。
* 修改增強的報告設定，例如預設貨幣和時區、要從報告排除的IP等等。（**[!UICONTROL 管理]** > **[!UICONTROL 報告]**）
* 基於隱私權原因模糊化訪客IP位址（**[!UICONTROL 管理]** > **[!UICONTROL 實作]**）
* 在Adobe Admin Console中管理每個工作區及其角色之前，檢視現有的使用者清單（**[!UICONTROL 管理]** > **[!UICONTROL 使用者]**）。
* 搜尋並篩選[!UICONTROL 管理]區段中的所有資料表。
...
* 修正使用可縮放向量圖形(SVG)影像時導致[!UICONTROL 插入於] > [!UICONTROL 影像]無法正常運作的問題。 (TGT-37242)
* 修正具有[!UICONTROL 發佈者]角色的使用者無法刪除草稿活動的問題。 (TGT-37358)
* 修正當選取[!UICONTROL 我的所有工作區]時，使用者無法編輯活動的問題。 (TGT-37276)

### Target Standard/Premium 20.5.1 (2020 年 6 月 17 日)

| 功能/增強功能 | 說明 |
| --- | --- |
| [!UICONTROL 自動分配]活動的Analytics for Target (A4T)支援 | [!UICONTROL 自動分配]活動現在支援[Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)。<br>此整合可讓您使用[!UICONTROL 自動分配]多臂吃角子老虎機功能，將流量導向成功體驗，同時使用[!UICONTROL Adobe Analytics]目標量度和/或[!UICONTROL Adobe Analytics]報告與分析功能。<br>如果您已[實作A4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)以搭配A/B測試和體驗鎖定目標活動使用，則您已準備就緒！<br>如需詳細資訊，請參閱[自動分配和自動鎖定目標活動的A4T支援](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)。 |
...
| [!UICONTROL 發行者]角色 | 這個新角色類似於目前的[!UICONTROL 觀察者]角色（可以檢視活動，但不能建立或編輯活動）。 但是，[!UICONTROL 發佈者]角色具有啟用活動的額外許可權。<br>如需詳細資訊，請參閱： <ul><li>**Target Standard 使用者**：在&#x200B;*使用者*&#x200B;中[指定其角色和權限](/help/main/administrating-target/c-user-management/c-user-management/user-management.md#roles-permissions)。</li><li>**Target Premium 使用者**：步驟 6：在&#x200B;*「設定企業權限」*&#x200B;中[指定角色和權限](/help/main/administrating-target/c-user-management/property-channel/properties-overview.md#section_8C425E43E5DD4111BBFC734A2B7ABC80)。</li></ul> |
| [!DNL Analysis Workspace]<br>2020 年 6月 25 日的 A4T 支援 | [!DNL Analysis Workspace]現在支援[!UICONTROL Anaytics for Target] (A4T)。 [!UICONTROL Analytics for Target (A4T)面板]可讓您在[!DNL Analysis Workspace]中分析您的[!DNL Adobe Target]活動和體驗。<br>如需詳細資訊，請參閱&#x200B;*A4T報告*&#x200B;中的[Analytics的報告](/help/main/c-integrating-target-with-mac/a4t/reporting.md)以及&#x200B;*Analytics工具指南*&#x200B;中的[Analytics for Target (A4T)面板](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/panels/a4t-panel.html?lang=zh-Hant&?lang=zh-Hant)。 |

**增強功能、修正和變更**

* 修正造成「訪客」量度儲存在活動定義而非「獨特訪客」中的問題。 (TGT-37098)
* 修正[!DNL Target] UI中造成[!UICONTROL 對象]頁面上垂直卷軸無法正確運作的問題。 (TGT-36968)
...
| 功能/增強功能 | 說明 |
| --- | --- |
| 可視化體驗撰寫器 (VEC) | **新增 VEC 功能表選項**：當您在 VEC 中按一下頁面元素時，會出現一個功能表，顯示該元素類型的可用選項。<ul><li>您現在可以使用[!UICONTROL 樣式>背景]選項，變更背景影像和所選元素的顏色。 (TGT-15001)</li></ul>檢視[視覺化體驗選項](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)中的&#x200B;*樣式*。<br>**點選追蹤改善**：我們已改善VEC和單頁應用程式(SPA) VEC內設定點選追蹤的程式。<ul><li>選取要在點擊追蹤中使用的元素時，所有可用元素的名稱都會顯示在右側的修改面板中，讓您快速輕鬆地選取所需元素。</li><li>三步驟引導式活動工作流程的[!UICONTROL 目標與設定]頁面會顯示數字，代表針對點選追蹤所選取的元素數目。 您可以將游標移至此數字上，查看所有所選元素的名稱。 (TGT-33878)</li></ul>請參閱[點擊追蹤](/help/main/c-activities/r-success-metrics/click-tracking.md)。 |
...
* 從「集合」選擇器選取[!UICONTROL Recommendations]集合後，您必須現在按一下[!UICONTROL 儲存]按鈕。 此工作流程與 [!DNL Target] 內其他工作流程相同。 (TGT-33205)
...
| 在[!UICONTROL A/B測試]和[!UICONTROL 體驗鎖定目標]活動中![Premium badge](/help/main/assets/premium.png)<br>Recommendations | 您現在可以在[!UICONTROL A/B測試] （包括[!UICONTROL 自動分配]和[!UICONTROL 自動鎖定目標]）和[!UICONTROL 體驗鎖定目標] (XT)活動中包含建議。 屆時將開啟以下列舉的全新功能:<ul><li>在同一個活動內測試及鎖定建議與非建議內容。</li><li>輕鬆試驗建議在頁面中的放置方法，包括多個建議的排列順序。</li><li>使用[!UICONTROL 自動分配]自動將流量推送到績效最佳的建議體驗。</li><li>使用[!UICONTROL 自動鎖定目標]，根據訪客的個別設定檔，以動態方式為訪客指派自訂建議體驗。</li></ul>若要開始，請使用VEC建立[!UICONTROL A/B測試]或[!UICONTROL 體驗鎖定目標]活動，並使用[!UICONTROL 插入在前]、[!UICONTROL 插入在後]或[!UICONTROL 取代為]動作將建議新增到體驗中。 (RECS-6166)<br>如需詳細資訊，請參閱[產品建議形式的建議](/help/main/c-recommendations/recommendations-as-an-offer.md)。 |
...
| 功能/增強功能 | 說明 |
| --- | --- |
| [!DNL Target] API中的![Target Premium徽章](/help/main/assets/premium.png)<br/>[!UICONTROL 企業許可權]支援 | [Adobe Target Admin API](https://developers.adobetarget.com/api/#admin-apis) 現在能充分運用 Target UI 中相同企業權限功能的優勢。 從 **2019 年 2 月 21 日**&#x200B;開始，系統管理員將能利用程式設計方式存取報表資料，也能建立及管理任何工作區中的活動、產品建議與客群。 這些動作先前只限預設工作區使用。 未來版本將提供 Automated Personalization (AP) 活動支援。 |
| ![Target Premium徽章](/help/main/assets/premium.png)<br/>[!UICONTROL 建議]：根據環境（主機群組）篩選集合和排除專案 | 您現在可以針對選取的環境（主機群組）預覽[!UICONTROL 建議]集合和排除專案的內容。<br/>先前，當您檢視集合或排除專案時，顯示的包含專案是預設主機群組（在[!UICONTROL 建議>設定>預設主機群組]中指定）的結果。<br/>現在，當您建立或更新集合或排除專案時，可以使用[!UICONTROL 環境]選取器來選擇要預覽結果的環境。 新的[!UICONTROL 環境]篩選器可節省時間和精力，因為您不再需要導覽至[!UICONTROL 設定]頁面來選取適當的預設主機群組，才能建立或編輯集合和排除專案。<br/>**注意：**&#x200B;變更選取的環境後，您必須按一下[!UICONTROL 搜尋]來更新傳回的結果。<br/>新的[!UICONTROL 環境]篩選器可從[!DNL Target] UI中的下列位置取得：<ul><li>[!UICONTROL 目錄搜尋] （[!UICONTROL Recommendations >目錄搜尋]）</li><li>[!UICONTROL 建立集合]對話方塊（[!UICONTROL Recommendations >集合>新建]）</li><li>[!UICONTROL 更新集合]對話方塊（[!UICONTROL Recommendations >集合>編輯]）</li><li>[!UICONTROL 建立排除專案]對話方塊（[!UICONTROL 建議>排除專案>新建]）</li><li>[!UICONTROL 更新排除專案]對話方塊（[!UICONTROL 建議>排除專案>編輯]）</li></ul><br>如需詳細資訊，請參閱下列主題:<uL><li>[集合](/help/main/c-recommendations/c-products/collections.md)</li><li>[排除項目](/help/main/c-recommendations/c-products/exclusions.md)</li><li>[目錄搜尋](/help/main/c-recommendations/c-products/catalog-search.md)</li><li>[設定](https://experienceleague.adobe.com/docs/target-dev/developer/recommendations.html?lang=zh-Hant){target=_blank}</li><li>[建議：根據環境 (主機群組) 篩選集合和排除項目](/help/main/administrating-target/hosts.md)</li></ul>(TGT-20622)</ul> |
...
* 修正當[!UICONTROL 管理]頁面上選取Adobe Analytics作為報告來源時，在某些情況下使用者無法建立活動的問題。 使用者看到「請選取報表套裝」訊息，但無法選取報表套裝。 (TGT-31968)
...
* 改善在使用者的 [!DNL Target] 工作階段建立或編輯活動或產品建議時逾時的工作流程。 當使用者按一下[!UICONTROL 儲存]時，會顯示工作階段已到期訊息，但在重新登入之後，對話方塊會通知使用者已成功登入，而[!DNL Target]中的UI會維持在相同的頁面上，不會遺失資料。
...
>[!NOTE]
>
>具有FP-11577的[!DNL Adobe Experience Manager] 6.2 （或更新版本）現在支援[!DNL at.js]實作與其[!UICONTROL Adobe Target Cloud Services]整合。 如需詳細資訊，請參閱 *Adobe Experience Manager 6.2* 文件中的[功能套件](https://experienceleague.adobe.com/docs/?lang=zh-Hant)和[與 Adobe Target 整合](https://experienceleague.adobe.com/docs/?lang=zh-Hant)。
...
* 刪除客群時，[!DNL Target] 會顯示參考該客群之活動的清單。 使用者可按一下清單中的每個活動以顯示其[!UICONTROL 概觀]頁面。 (TGT-17997)
...
* 在所有使用[!UICONTROL Analytics for Target] (A4T)報表的活動的[!UICONTROL 目標與設定]頁面上新增工具提示，通知使用者如果活動的頁面已載入at.js （0.9.1版或更新版本），則不需要追蹤伺服器。 (TGT-22607)
...
* 修正未在下載的[!UICONTROL 訂單詳細資料]CSV報表中正確標示極端訂單的問題。 (TGT-22500)
...
* 如果您使用表單式體驗撰寫器搭配自動建立的全域 mbox 以外的 mbox (`target-global-mbox`)，然後選擇參與量度做為成功量度，則量度只會在活動中使用了該 mbox 的頁面上遞增。 例如，如果您的mbox為`homepage_mbox`，每次造訪的[!UICONTROL 頁數]量度為在該造訪期間對`homepage_mbox`的點選數。
