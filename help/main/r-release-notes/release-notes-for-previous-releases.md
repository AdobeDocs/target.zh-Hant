...
### [!DNL Target Standard/Premium] 26.3.1 （2026年3月3日）

**活動**

+++檢視詳細資料

* **套用至其他檢視時遺失自訂程式碼。**&#x200B;此修正解決新增自訂程式碼並套用至其他檢視時，套用至一個檢視的自訂程式碼遺失的問題。 先前，當使用者將自訂程式碼套用至活動中的多個檢視時，先前套用的自訂程式碼將會消失。 此修正可確保在套用修改至其他檢視時，所有檢視中都會正確保留自訂程式碼。 (TGT-54230)
...
* **設定檔引數「介於」規則未儲存在條件中。**&#x200B;此修正解決對設定檔引數使用&#x200B;**In Between**&#x200B;運運算元的條件規則無法正確儲存的問題。 先前，當使用者設定具有&#x200B;**介於**&#x200B;之間運運算元的規則並輸入範圍值時，條件似乎已成功儲存，但在重新開啟或編輯條件時，規則會消失。 此修正可確保正確儲存&#x200B;**介於**&#x200B;之間的規則，並在重新開啟條件時持續保留。 (TGT-54498)
...
* **變更所有活動後的「深入分析」頁面位置。** **深入分析頁面**&#x200B;位置已更新，顯示於導覽中的&#x200B;**所有活動**&#x200B;頁面之後。 「所有活動」頁面現在是Target應用程式的預設頁面。 (TGT-54672)
...
* **讓左側導覽可收合。**&#x200B;現在所有索引標籤都可以摺疊和展開左側導覽。 摺疊時，圖示會顯示工具提示。 展開時，圖示和文字都會顯示。 此增強功能可提供使用者在需要時更多熒幕空間，同時維持可快速存取導覽選項。 (TGT-54608)
...
* 當後端傳回null時，**預設experienceEditorType為「visual」。**&#x200B;此修正解決某些案例中將`experienceEditorType`值傳回為null，導致UI中編輯器型別未正確初始化的意外行為的問題。 (TGT-54690)

+++

### [!DNL Target Standard/Premium] 26.2.2 （2026年2月16日）

**[!UICONTROL Visual Experience Composer] (VEC)**

+++檢視詳細資料

* **無法將影像新增至活動 — 未知錯誤。**&#x200B;此修正解決使用者嘗試在活動中新增或取代影像時遇到「未知錯誤」的問題。 此問題與Scene7 URL同步和驗證失敗有關。 此修正可確保在Visual Experience Composer中正確新增和取代影像。 (TGT-54137)

+++

### [!DNL Target Standard/Premium] 26.2.1 （2026年2月8日）

**屬性**

+++檢視詳細資料

* **屬性代號在移除後未自動重新新增。**&#x200B;此修正解決在編輯活動時，使用者已取消選取的屬性Token會自動重新新增的問題。 先前，當使用者移除自動偵測到的屬性並在步驟之間導覽時，系統將重新引入已移除的屬性而不出現任何警報或提示，導致在意外的屬性上啟用活動。 此更新可確保一旦屬性代號被移除後，它會在所有後續的編輯和導覽期間保持被移除狀態。 (TGT-54040)
...
* **已加入允許清單的URL管理功能。**&#x200B;新的「允許列出的URL」管理功能已新增至「[!UICONTROL Administration]」區段，以管理允許的遠端選件URL模式。 此功能包括在表格中顯示允許列出的URL、搜尋和篩選URL、透過模組對話方塊新增新的URL以及在確認後刪除URL的功能。 此功能可協助客戶控制哪些URL可用於遠端選件，進而改善安全性和合規性。 (TGT-54486)
...
* **在GraphQL中支援createdAt和modifiedAt的篩選功能。已新增**&#x200B;對`createdAt`與`modifiedAt`日期欄位篩選活動的支援至GraphQL API。 此增強功能可啟用Target UI中的準確篩選、減少使用者端資料處理，且不需要多個API呼叫來擷取已篩選的活動資料。 (TGT-54499)
...
* **啟用Web編輯器提供的工作區列。**&#x200B;此更新解決來自[!UICONTROL Default Workspace]的優惠方案出現在網頁編輯器中的其他工作區所造成的客戶混淆。 儘管此行為按設計方式運行，但[!UICONTROL Default Workspace]優惠在所有工作區中都有意可見，客戶報告UI未清除工作區原點，尤其是在在非預設工作區（如「批准者」）中建立活動時。 為了提高清晰度，Web Editor的優惠清單中現在啟用了[!UICONTROL Workspace]列，使用戶能夠輕鬆區分每個優惠屬於哪個工作區，並防止誤解所顯示的附加優惠。 (TGT-54138)
* **在新頁籤中開啟了與target=&quot;_blank&quot;的連結。**&#x200B;此修正解決含有具有~target=&quot;_blank&quot;~之連結的已撰寫網站在[!UICONTROL Browse]模式中按一下時，會在新的瀏覽器標籤中開啟，中斷編輯器內預覽體驗的問題。 發生此行為是因為編寫頁面的原生連結屬性沒有受到擴充功能插入的JavaScript攔截，不像在舊版UI中，錨點元素會進行轉換，且其目標會覆寫，以保留在編輯器內的導覽。 更新可確保使用~target=&quot;_blank&quot;~的連結現在可在網頁編輯器中正確處理，因此在編寫期間不再開啟外部索引標籤。 (TGT-54134)
...
* **活動編輯不應添加自動檢測到的屬性（如果已刪除）。**&#x200B;此修復程式解決了編輯活動將自動重新引入用戶先前刪除的自動檢測屬性的問題。 重新開啟活動進行編輯時，系統錯誤地還原了已刪除的屬性，導致[!UICONTROL Properties List]中的行為不一致和混亂。 此更新可確保在移除自動偵測到的屬性後，該屬性會在所有後續編輯期間保持被移除狀態，且除非使用者明確將其新增回，否則其不會重新出現。 (TGT-54182)
* **如果已刪除，則不添加自動檢測的屬性。**&#x200B;此修復程式確保一旦用戶手動從活動中刪除自動檢測到的屬性，系統就不再在活動編輯器中後續導航期間重新引入該屬性。 以前，如果用戶取消選擇了自動檢測的屬性，將其移到[!UICONTROL Targeting]步驟，然後返回到[!UICONTROL Experiences]，則編輯器將根據儲存在活動編輯器狀態切片中的自動檢測清單重新填充刪除的屬性。 更新的邏輯現在將自動檢測的屬性與~ActivityState~切片中的當前屬性進行比較，並防止重新添加用戶已刪除的任何自動檢測的屬性。 這將導致跨步驟的一致行為，並尊重用戶意圖。 (TGT-54181)
* **將自動檢測到的文本添加到屬性清單中。**&#x200B;此增強更新[!UICONTROL Properties List]，以明確標籤系統自動檢測到的任何屬性。 當使用者可見的[!UICONTROL Properties List]中也存在自動偵測的屬性時，它現在會使用儲存在~ActivityEditorSlice~狀態的值，在其名稱旁邊顯示「（自動偵測）」文字。 這反映了舊版UI的行為，並幫助使用者輕鬆區分手動選取的屬性和自動識別的屬性。 (TGT-54120)
...
* **[!UICONTROL Environment]下拉式清單僅顯示100個結果。** This fix addresses a limitation where customers with more than 100 environments could only see the first 100 entries in the [!UICONTROL Environment] drop‑down within [!UICONTROL Recommendations]. 基礎GraphQL查詢(~getEnvironmentsV2~)已使用100的硬式編碼頁面大小進行分頁，導致UI即使有其他頁面可用，也只顯示部分清單。 對於擁有超過100個環境的客戶，此問題會導致缺少選項和不完整的選擇體驗。 更新會增加上限，因此所有環境都會傳回和顯示，確保完整可見性（無論環境計數為何）。 (TGT-53903)
...
* **修正[!UICONTROL Reports]箭頭未明確表示可展開欄的問題。**&#x200B;修正報表表格未清楚顯示其他欄可以在更新的UI中展開的問題。 在欄標題附近的[!UICONTROL Reports]箭頭新增了消失的工具提示，以幫助客戶瞭解有更多欄可用。 (TGT-53479)
...
* **無法將[!UICONTROL Advanced Settings]用於[!UICONTROL Revenue]量度。**&#x200B;此修復解決了在[!UICONTROL Advanced Settings]中為[!UICONTROL Revenue]度量配置[!UICONTROL Goals & Settings]時用戶遇到403「拒絕訪問」錯誤的問題。 添加與主目標相關的依賴項條件時出現問題；後端錯誤地要求編輯器權限，即使對於已具有足夠權限建立和編輯活動的用戶也是如此。 因此，儘管設定有效，儲存活動仍會失敗。 更新會更正許可權檢查，使具有適當存取許可權的使用者可以成功新增收入量度相依性，而不會觸發禁止的資源錯誤。 (TGT-54092)
...
+++檢視詳細資料
* **更正了韓文和日文UI中的幾個本地化錯誤。**（TGT-54003、TGT-54004、TGT-54006、TGT-54007和TGT-54018）
...
+++檢視詳細資料
* **[!UICONTROL Delete Modification]按鈕問題阻止刪除活動修改。**&#x200B;解決了[!UICONTROL Delete Modification] UI中的[!DNL Target]按鈕無法運行的問題，阻止用戶刪除活動中的修改。 按鈕現在按預期工作，允許可靠地刪除修改，而不會延遲。 (TGT-53728)
* **在更新的UI中無法識別首選選擇器。**&#x200B;解決了更新的UI中的問題，其中首選選擇器（如`data-target-component-id`）未出現在VEC中的CSS選擇器清單中。 使用者現在可以可靠地選取偏好的屬性，而不是動態產生的類別名稱，以確保在SPA頁面更新中穩定鎖定目標。 (TGT-53908)
...
* **編輯期間，現有活動不再陷入無限載入循環。**&#x200B;解決了「新建建立UI」中的問題，在該問題中，編輯VEC中的現有活動導致頁面在無限載入循環中停滯。 此問題不會影響新建立的活動，並且是由頁面上預先存在的修改觸發的。 (TGT-53913)
* **現有活動頁面的修改現在正確載入到VEC中。**&#x200B;解決了更新的VEC中的問題，該問題導致在編輯具有已保存修改的現有活動時，頁面在載入階段仍卡住。 載入的新活動無問題，但先前配置的活動無法呈現。 (TGT-53967)
...
* **對象排除規則在[!DNL Target] UI中顯示為包含不正確。在活動內編輯目標時，配置了排除規則的**&#x200B;訪問群體顯示為包括。 雖然在執行期間已正確套用排除邏輯，但UI無法正確反映規則，而省略「排除」標籤。 [!DNL Target] UI現在可以正確顯示受眾配置和目標工作流中的排除規則，確保市場活動設定的清晰性和一致性。 (TGT-53808)
...
+++檢視詳細資料
* **修正當`redirectOptions`時，重新導向選件未在承載中包含`includeContent=true`的問題。**&#x200B;使用`includeContent=true `擷取重新導向選件的客戶在回應承載中遺失`redirectOptions`欄位。 這種不一致影響了工作流，如服務複製和活動建立。 請求內容時，重定向服務現在正確包括`redirectOptions`。 (TGT-53737)
...
+++檢視詳細資料
* **Form validation corrected for parameter presence conditions in updated UI.**&#x200B;已解決更新UI中選取「[!UICONTROL Custom mbox parameter value is present]」或「[!UICONTROL Parameter is present]」錯誤地要求客戶輸入值的問題。 此行為與預期邏輯衝突，預期邏輯只會檢查引數是否存在，無論其值為何。 此修正使表單驗證與預期行為一致，可讓您更順暢地設定活動，並支援完全採用更新的UI。 (TGT-53638)
* **已針對頁面傳送中的引數顯示狀態規則修正表單邏輯。」**&#x200B;已解決在更新的UI中選取頁面傳送規則（例如&quot;[!UICONTROL Parameter is present]&quot;、&quot;[!UICONTROL Parameter is not present]&quot;、&quot;[!UICONTROL Parameter value is present]&quot;或&quot;[!UICONTROL Parameter value is not present]&quot;）時，不正確地要求使用者輸入其他引數值的問題。 此行為與舊版UI不一致，且與在不指定值的情況下偵測引數存在的預期邏輯相衝突。 此修復程式可恢復預期的規則配置行為，簡化活動設定並提高可用性。 (TGT-53640)
...
* **已為更新的UI中的MVT和AP活動修復位置名稱持久性。**&#x200B;解決了更新的UI中的問題，其中在[!UICONTROL Multivariate Test]&#x200B;(MVT)和[!UICONTROL Automated Personalization]&#x200B;(AP)活動中編輯的位置名稱未正確保存。 更名位置後，在頁籤（如[!UICONTROL Targeting]和[!UICONTROL Experiences]）之間導航，導致名稱恢復到其以前的狀態。 此修復程式確保跨標籤的位置命名一致，並提高活動設定期間的可靠性。 (TGT-52927)
* **在MVT活動的「管理體驗」面板中更正了位置標籤。**&#x200B;已解決更新UI中，[!UICONTROL Manage Experiences] (MVT)活動中的[!UICONTROL Multivariate Test]面板顯示不一致位置編號的問題。 此修正可確保位置標籤順序且正確與使用者定義的修改一致，進而在體驗設定期間提高清晰度和可用性。 (TGT-52929)
...
+++查看詳細資訊
* **客戶無法從[!UICONTROL Activity Overview]**&#x200B;複製活動或文檔名稱：以前，客戶無法直接從更新的活動建立流程中的[!UICONTROL Activity overview]複製活動或關聯的優惠/文檔名稱。 這一限制影響了可用性，特別是在較小的螢幕上。 客戶現在可以輕鬆複製活動和文檔名稱，而無需解決方法。 (TGT-51850)
* **在活動建立期間主動接收已建立的[!DNL Target]個客戶資料**：通過啟用主動收集來自[!DNL Target]個客戶的報告、內容和螢幕截圖，改進了活動建立過程。 此增強功能可解決現有使用情形中發現的資料缺口，並有助於確保在活動和實驗設定期間獲得更準確的洞察力。 (TGT-52415)
* **AP活動未在[!UICONTROL Reports]節**&#x200B;中獲取模型就緒資料：在[!UICONTROL Reports]節中查看Automated Personalization(AP)活動的客戶無法在報告組和提供級別上查看模型就緒指示器。 出現此問題是因為未從後端正確讀取模型就緒資料。 功能已恢復，現在模型就緒資料將按預期顯示。 (TGT-53600 和 TGT-53601)
* **在[!UICONTROL Live]概述[!UICONTROL Activity]中未正確顯示「**」狀態為將來計畫的活動：客戶發現在[!UICONTROL Live]概述中未正確將計畫在將來啟動的活動標籤為「[!UICONTROL Activity]」。 已解決此狀態不匹配問題，計畫的活動現在正確顯示為「[!UICONTROL Scheduled]」，而無需刷新頁面。 (TGT-52835)
...
+++查看詳細資訊
* **產品清單在[!UICONTROL View Collection]對話框中不可見：**&#x200B;以前，客戶在[!UICONTROL Recommendations]頁籤中查看集合時無法查看產品清單。 [!UICONTROL View Collection]對話框現在正確顯示關聯產品，提高了更新的[!UICONTROL Recommendations] UI中的透明度和可用性。 (TGT-50531)
* **修復了導致[!UICONTROL Product Catalog Search]高級搜索**&#x200B;中區分大小寫篩選的問題：[!UICONTROL Product Catalog Search]頁中的高級搜索篩選現在正確忽略區分大小寫，與後端和GraphQL服務的行為保持一致。 此更新可確保客戶的建議結果一致且準確，而不管文本大小寫如何。 (TGT-53585)
* **更新的[!UICONTROL Product Catalog Search] UI中的高級搜索未提供建議**：更新的[!UICONTROL Product Catalog Search] UI中使用高級搜索功能的客戶需要輸入拼寫正確的準確值，因為未顯示任何建議。 這一問題使產品難以高效地定位。 現在，在高級搜索輸入過程中，建議會按預期顯示。 (TGT-52008)
* **某些批准者無法查看[!UICONTROL Product Catalog Search]**&#x200B;中的產品：具有[!UICONTROL Approver]權限的客戶無法查看[!UICONTROL Product Catalog Search]中的任何產品，儘管具有相同角色的其他用戶具有訪問權限。 此問題是由影響目錄可見性的權限不一致引起的。 所有批准者現在都可以按預期查看[!UICONTROL Recommendations]節中的產品。 (TGT-53617)
...
* **使用Backspace清除所有日期值會導致頁面當機**：使用Backspace清除&quot;[!UICONTROL Goals & Settings]&quot;欄位中的所有值時，排程[!UICONTROL Specified Date & Time]區段中的客戶發生當機問題。 此問題是由日期處理邏輯中的Null參考錯誤所導致。 該頁現在可以正常處理空日期輸入，而不會崩潰。 (TGT-53624)
* **由於有效負載無效，[!UICONTROL Product Catalog Search]中未出現任何產品**：訪問[!UICONTROL Recommendations]中[!UICONTROL Product Catalog Search]節的客戶遇到由無效的GraphQL有效負載導致的空結果。 此後端錯誤阻止了正確載入產品資料。 產品現在按預期在更新的UI中顯示。 (TGT-53630)
* 在更新的VEC **[!DNL Scene7]中以較低解析度保存了**&#x200B;個影像：在更新的VEC中編輯經驗的客戶注意到，[!UICONTROL Scene7]個影像URL沒有解析度參數保存，導致傳送的影像質量較低（400×400，而不是預期的800×800）。 影像URL現在會保留正確的引數，以確保正確解析度。 (TGT-52631)
...
* **無法登入VEC進行活動**：嘗試透過VEC登入其網站的客戶被重複重新導向至登入頁面，導致無法存取活動編輯。 此問題在內部無法複製，可能與站點端會話處理有關。 登錄流已穩定，客戶現在可以訪問VEC而無重定向錯誤。 (TGT-53524)
* **在[!UICONTROL Browse]模式下按兩次後鍵導致VEC崩潰**：在VEC中瀏覽[!UICONTROL Browse]模式的客戶在按兩次瀏覽器的後鍵時遇到崩潰。 此問題導致編輯器凍結，需要刷新頁面。 編輯器現在可靠地處理後退導航，而不會崩潰。 (GT-53568)
* **由於未定義位置映射，無法編輯活動**：由於`LocationMapping.behaviorTargetedActivity`邏輯中未定義位置ID，客戶在嘗試編輯活動時遇到錯誤。 此問題導致400錯誤並阻止了活動更新。 現在可以編輯活動，而不出現與位置相關的驗證錯誤。 (TGT-53607)
* **保存活動觸發了無效的用戶輸入錯誤**：在更新的VEC中進行小修改後，客戶在嘗試保存活動時遇到無效的用戶輸入錯誤。 錯誤是由後端驗證邏輯中位置映射不匹配引起的。 現在可以成功儲存活動，而不觸發位置相關錯誤。 (TGT-53603)
...
+++檢視詳細資料
* **修復了保存活動觸發無效用戶輸入錯誤的問題**：客戶在嘗試保存現有活動時遇到嚴重錯誤。 錯誤消息指示無效的用戶輸入，特別是引用JSON負載中無法識別的屬性名稱內容。 特別要注意的是，使用相同屬性的新活動已成功保存，表明此問題已與舊活動資料隔離。 活動建立過程現在可以正確處理舊屬性配置，防止無效的輸入錯誤，並確保新活動和現有活動的保存行為一致。 (TGT-53507)
* **修正由於InvalidProperty.Json錯誤而導致客戶無法儲存活動的問題**：客戶嘗試在更新的UI中儲存活動時發生錯誤，即使未進行任何修改。 錯誤訊息指出無效的JSON結構：「無效的Json。 無法識別的屬性名稱「content」。 位置：行 — 1，欄 — 432。」 此問題是由請求承載中無法識別的屬性所導致，現已解決。 客戶可以成功儲存活動，而不會遇到此錯誤。 (TGT-53513)
...
+++查看詳細資訊
* **修復了過期的「時間框」受眾即使在刪除後也阻止活動保存的問題**：由於與過期的「時間框」受眾相關的錯誤，客戶無法在更新的UI中保存活動。 即使刪除受影響的受眾後，錯誤消息仍會持續：「活動包含具有無效時間框架的受眾。」 出現此問題是因為系統繼續驗證僅活動受眾，即使該受眾不再使用。 由於時區差異，這種行為更加複雜。 驗證邏輯已更正，客戶現在可以保存活動，而不會遇到此錯誤。 (TGT-53517)
...
* **已修復更新UI內活動建立進程中XF內容的間歇性預覽問題**：已修復活動建立進程中XF內容在更新的UI內間歇性無法以預覽模式呈現的問題。 儘管內容已正確交付，但預覽並不一致顯示，使客戶難以驗證服務設定。 XF預覽現在可靠載入，提高了活動配置期間的可信度和效率。 (TGT-53318)
...
+++查看詳細資訊
* **修復了活動建立過程中URL中前導空格導致QA連結斷開的問題**：修復了活動建立過程中的問題，在保存時未正確裁剪活動URL中前導空格。 這會導致後端的QA連結無效和格式不正確。 更新後，URL現在已完整儲存，避免連結損毀，並改善客戶QA工作流程的可靠性。 (TGT-53427)
...
+++查看詳細資訊
* **已修復目錄搜索UI中的問題，高級搜索無法提供建議**：已修復新[!UICONTROL Catalog Search] UI中的問題，其中[!UICONTROL Advanced Search]功能無法提供建議。 用戶需要輸入精確的拼寫值，使搜索體驗變得繁瑣且容易出錯。 通過此修復，[!UICONTROL Advanced Search]現在可在用戶類型時提供相關建議，從而提高可用性並減少產品定位時的摩擦。 (TGT-52008)
...
* **按[!UICONTROL Last Updated]列修復了不一致的排序：**&#x200B;修復了活動建立進程中的問題，在該進程中，按[!UICONTROL Last updated]列對目錄進行排序會產生不一致的結果。 客戶無法根據更新時間戳可靠地組織產品資料，使目錄審查和管理更加困難。 現在，按預期進行排序，提高了更新後的UI的準確性和可用性。 (TGT-53116)
...
+++檢視詳細資料
* **已修復活動載入和活動建立進程中的[!UICONTROL Cancel]按鈕問題**：修復了活動建立進程中某些活動載入失敗的問題，使客戶無法訪問修改。 此外，[!UICONTROL Cancel]按鈕沒有響應，阻止客戶停止載入進程或退出編輯視圖。 此修復程式可確保活動現在可靠載入，並且[!UICONTROL Cancel]按鈕按預期運行，從而改善Visual Experience Composer中的總體穩定性和用戶體驗。 (VEC)(TGT-53218)
* **已修復已更新的VEC UI中阻止編輯並禁用[!UICONTROL Cancel]按鈕的體驗切換問題**：已修復已更新的VEC UI中的問題，在「體驗目標」(XT)活動中，在切換體驗時無法載入修改。 客戶無法編輯超出他們最初輸入的體驗的體驗，[!UICONTROL Cancel]按鈕丟失或沒有響應。 此修復程式可確保修改現在在所有體驗中正確載入，並確保[!UICONTROL Cancel]按鈕可靠地運行，即使沒有幫助程式擴展，也可以改進編輯工作流並減少挫折感。 (TGT-53256)
* **在活動建立流程中在多個體驗之間切換時修復了白屏問題**：修復了在多個體驗之間切換導致白屏的問題。 也修正了活動建立程式中，客戶嘗試修改活動內的多個體驗時，遇到白熒幕的問題。 將變更套用至兩個體驗，然後選取第三個體驗（阻止進一步編輯）後，就會發生此問題。 此更新可確保體驗之間順利轉換，讓客戶能夠在不中斷的情況下進行修改。 (TGT-53266)
* **修正VEC中自訂程式碼變更無法可靠地跨編輯工作階段儲存的問題**：修正視覺化體驗撰寫器(VEC)中所做的自訂程式碼修改無法可靠地在單一嘗試中儲存的問題。 客戶回報網站和QA URL斷斷續續遺失樣式更新或HTML編輯等變更，即使同時使用[!UICONTROL Edit Content]和最終[!UICONTROL Save]按鈕亦然。 已解決此回歸，確保所有自定義代碼更改在編輯會話期間按預期保持不變。** (TGT-53418)
* **已修復在活動建立期間更新的UI中缺少`triggerViews`的問題**：已修復活動建立進程中`triggerViews`未出現在更新的UI中的問題，即使這些問題已在頁面上正確實現。 This impacted multiple customers and made it difficult to validate view-based triggers during activity setup. `TriggerViews` now display as expected, allowing customers to complete and test their configurations reliably. (TGT-53239)
...
* 修正從[!DNL Target]前端移轉至[!DNL Target]中央的部分活動因先前修正的同步錯誤而造成量度設定不一致的問題。 具體來說，原本使用轉換量度且後來更新為以Analytics為基礎的量度的活動，會在`primaryMetricType`和`successCriteria`欄位中保留過時的值。 (TGT-52643)
* 修正QA預覽頁面的整個內容因HTML修改中無意包含`contentEditable`屬性而變為可編輯的問題。 這可讓使用者按一下並編輯頁面上的任何文字，這可能會在QA期間導致版面問題和混淆。 (TGT-53247)
* 修正將修改從[!DNL Page Load]移動到[!UICONTROL View]導致修改重複的問題，修改留在[!UICONTROL Page Load]中，同時出現在[!UICONTROL View]中。 此外，從[!UICONTROL View]中移除修改也會錯誤地從[!UICONTROL Page Load]中移除它。 (TGT-53270)
...
+++查看詳細資訊
* 在客戶在[!UICONTROL Goals & Settings]頁上查看特定活動的報告時，修復了問題，[!UICONTROL View in Analytics]連結錯誤地指向QA環境而不是生產環境。 (TGT-53163)
...
+++查看詳細資訊
* 修復了通過自定義代碼調用`triggerView`時導致無限循環的問題。 (TGT-52885)
* 修復了導致為活動定義的`LocalIds`與在經驗定義中使用的`LocalIds`之間不匹配的問題。 (TGT-52669)
* 修復了活動[!UICONTROL Overview]頁上缺少度量名稱的問題，僅顯示「提供」，而不顯示正確的度量名稱。 (TGT-53054)
...
+++查看詳細資訊
* 修復了[!UICONTROL Form-Based Experience Composer]中阻止活動保存並觸發錯誤消息的問題：「無法讀取未定義的屬性（正在讀取&#39;map&#39;）」。 (TGT-53145)
...
+++查看詳細資訊
* 修復了按一下[!UICONTROL Catalog Search]中的產品時顯示錯誤「檢索產品詳細資訊失敗」，並在沒有關閉選項的情況下開啟了模式的問題。 (TGT-53082)
* 修復了[活動中](/help/main/c-recommendations/recommendations-as-an-offer.md)建議作為優惠[!UICONTROL A/B Test]在更改收集或促銷時未正確更新的問題。 (TGT-52884)
* 修復了生產環境中的問題，在此問題中，按一下更新的UI中的實體會顯示錯誤：「無法檢索產品詳細資訊。 如果此問題仍然存在，請與[!DNL Adobe Client Care]聯繫。&quot; (TGT-53071)
...
+++查看詳細資訊
* 修復了將訂單詳細資訊保存到CSV檔案導致檔案為空的問題。 (TGT-52225)
...
* 修復了阻止活動編輯並觸發錯誤消息的問題：「無法讀取未定義的屬性（正在讀取&#39;map&#39;）」。 (TGT-53161)
...
+++查看詳細資訊
* 在切換工作區時改進了對自組服務的處理。
   * 當從預設工作區切換到非預設工作區（或在非預設工作區之間）時，即席服務現在會被正確複製。 在初始化期間，將更新工作區上下文並為提供分配新ID以確保唯一性。
...
* Updated entity validation logic to respect the original workspace context of the activity. 諸如[!UICONTROL Experience Fragments] (XF)之類的實體現在會根據活動最初建立的工作區進行驗證。 For example, if an XF exists in the default workspace and the activity is copied from workspace X to workspace Y, validation still passes as long as the XF is valid in the original (default) workspace. (TGT-53196)
* 在活動複製期間對複製臨機受眾的支援增強。
...
* 修正預覽URL不正確地包含使用者明確輸入之對象以外的其他對象的問題。 已更正此行為，以確保在生成QA或預覽連結時僅應用指定的受眾。 (TGT-52912)
* 修正了在流量配置設定期間先選取[!UICONTROL Auto-Target] (AA)時，使用者無法建立[!UICONTROL Auto-Allocate] (AT)活動的問題。 此問題導致後端驗證錯誤並阻止保存活動。 (TGT-53096)
...
+++查看詳細資訊
* 修復了具有[!UICONTROL Approver]角色的用戶無法添加或保存僅活動訪問群體改進的問題。 嘗試這樣做會導致403禁止錯誤，指出需要「[編輯器]」權限，儘管用戶有足夠的權限批准和管理活動。 (TGT-52984)
* 使用[!UICONTROL Remove Audience Refinement]選項刪除特定於活動的受眾時，修復了問題，該受眾不再出現在受眾清單中，以便在同一活動中重新選擇。 除非從頭重新建立同一訪問群體，否則此行為阻止用戶重新添加該訪問群體。 (TGT-52979)
* 修正從位置移除後，甚至在儲存活動之前，僅限活動的對象細分立即從UI消失的問題。 此行為與預期功能和工具提示指導相矛盾，工具提示指導指出：「保存活動後，將刪除此庫中所有未使用的受眾。」 (TGT-52982)
* 修正嘗試將[!UICONTROL All Visitors]以外的對象指派至活動時發生的問題。 儲存後，顯示下列錯誤訊息：「我們無法完成您的要求。 如果問題仍然存在，請聯絡[!UICONTROL Adobe Client Care]。」 (TGT-53008)
* 修正在活動編輯器中建立和指派新對象後，無法儲存活動的問題。 顯示的錯誤消息是：「We cannot complete your request。 如果問題仍然存在，請與[!UICONTROL Adobe Client Care]聯繫。&quot; (TGT-52977)
...
+++檢視詳細資料
* 修復了複製現有活動並將報告源更改為[!DNL Adobe Analytics]&#x200B;(A4T)將導致「用戶輸入無效」錯誤的問題。 當某些與[!DNL Analytics]報告不相容的度量操作（如`restart_same_experience`、`restart_random_experience`和`restart_new_experience`）從原始活動中保留時觸發錯誤。 (TGT-52900)
...
* 修復了[!UICONTROL Form-Based Experience Composer]中的一個問題，在[!UICONTROL Location]下拉清單中將滑鼠懸停在具有特殊字元的框上，導致編輯器變為空白，並觸發了&quot;Failed to execute &#39;querySelector&#39; on &#39;Element&#39;&quot;。 」錯誤。(TGT-52717)
* 使用新的「PARTIALLY_IMPORTED」指標改善摘要狀態準確度。 先前，即使檔案中並非所有列都已匯入，摘要仍會標示為「success」，這是誤導性的。 (TGT-52892)
* 已修復錯誤，在遷移到AP V2後，某些API調用到`/admin/rest/ui/v1/campaigns`時返回了客戶端錯誤(HTTP 4xx)。 (TGT-52721)

### [!DNL Target Standard/Premium] 25.6.2（2025年6月12日）

此版本包括以下修復和更新：

* 已添加[新的常見問題解答文章](/help/main/c-intro/updated-ui-faq.md)，以解決有關已更新的[!DNL Target] UI和[!UICONTROL Visual Experience Composer]&#x200B;(VEC)的常見問題。
* 修復了[!UICONTROL URL - does not contain]中「[!UICONTROL Page Delivery]」規則不起作用的問題，允許即使在應該阻止內容時也顯示內容。 (TGT-52754)
...
* 移除在活動建立期間選取預設工作區時不需要的驗證。 強制屬性驗證不再適用於預設工作區，但仍保留非預設工作區。 (TGT-52449)
...
* 修復了沒有[!UICONTROL Activity QA]按鈕的活動顯示空選項選擇器的問題。 (TGT-52733)
* 修正QA連結無法正確呈現內容的問題。 (TGT-52718)
* 修復了以經驗片段替換元素未正確反映QA環境中更改的問題。 (TGT-52762)
* 已修復更新的[!UICONTROL Visual Experience Composer]&#x200B;(VEC)中的問題，該問題在用戶嘗試添加體驗片段時導致「無效輸入」錯誤。 (TGT-52701)
* 修復了在更新的[!UICONTROL Visual Experience Composer]&#x200B;(VEC)中編輯目標受眾時「編輯受眾」模式顯示為空的問題。 (TGT-52749)
* 添加一條消息，當所選工作區中無法訪問實體時通知用戶。 (TGT-52767)
* 修復了UI無法允許將環境ID手動分配給條件的問題。 相反，它預設為[!UICONTROL Product Catalog Search]主機組的ID。 此修復程式確保現在所有環境都應用了條件更改，而不只是預設環境。 (TGT-52817)
...
* 已修復在QA模式下無法添加或編輯URL的問題。 (TGT-51941)
* 已在[!UICONTROL Reports] > [!UICONTROL Report Settings]（![報告設定表徵圖](/help/main/assets/icons/Setting.svg)）下添加QA模式通信設定，以與舊版[!DNL Target] UI中的功能對齊。 (TGT-52228 和 TGT-52329)
...
* 修復了更新的[!UICONTROL Visual Experience Composer]&#x200B;(VEC)無法自動檢測`at_property`值的問題。 (TGT-52347)
* 修復了在與表單元素交互時在VEC中在[!UICONTROL Browse]和[!UICONTROL Design]模式之間切換後，當只需要記錄一個修改時導致記錄兩個修改的問題。 (TGT-52455)
...
* 已修復阻止提供更新並錯誤顯示「無效用戶」錯誤的問題。 (TGT-52361)
* 已修復阻止保存現有活動的問題，並觸發「無效用戶輸入」錯誤。 (TGT-52422)
* 修復了阻止編輯現有HTML提供的問題，在保存時觸發「無效用戶輸入」錯誤，即使沒有進行代碼更改也是如此。 (TGT-52351)
...
* 修復了[!UICONTROL Analytics for Target]&#x200B;(A4T)被錯誤選擇為報告源的問題，即使客戶沒有訪問A4T的權限。 (TGT-52226)
* 已修復阻止使用[!UICONTROL View a Page] URL度量保存活動的問題。 (TGT-52260)
* 修復了在活動中建立聘用時阻止客戶選擇工作區的問題。 (TGT-52289)
* 修正阻止客戶在所有工作區中建立活動的問題。 (Tgt-52218)
* 修復了切換到另一個體驗時錯誤顯示對一個體驗的修改的問題。 (TGT-52184)
* 修復了開啟活動時預設優惠在[!DNL Target] UI中錯誤顯示的問題。 (TGT-52198)
...
* 已修復錯誤允許具有重複名稱的度量的問題。 量度現在需要唯一名稱。 (TGT-52201)
* 修復了在[!UICONTROL Automated Personalization]&#x200B;(AP)活動中編輯服務級別目標時無法看到受眾定義的問題。 (TGT-52148)
* 修正具有[!UICONTROL Editor]許可權的客戶無法儲存活動的問題。 (TGT-52227)
* 當選項保持不變時，`OptionLocalIDs`不再錯誤地增量。 (TGT-52139)
* 已修復在嘗試建立活動時導致「無效`optionLocalIds`」消息的問題。 (TGT-52154)
* 已修復為活動定義的`OptionLocalIDs`與用於定義經驗的之間的差異。 (TGT-52215)
* 修復了在嘗試建立A/B活動時導致驗證失敗的問題。 (TGT-51923)
...
* 修復了一個問題，在沒有頁面傳遞規則的活動在[!UICONTROL Overview]頁上顯示了多個規則。 (TGT-52084)
...
如需此版本和舊版at.js的相關資訊，請參閱[at.js版本詳細資料](https://experienceleague.adobe.com/zh-hant/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions){target=_blank}。
...
* [[!UICONTROL Updated Change Log]](/help/main/c-activities/change-log.md)頁面，目前正在從[[!DNL Audit Query API]](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/landing/governance-privacy-security/audit-logs/audit-api/overview){target=_blank}取得即時深入分析的資訊。
...
* 可定製的清單視圖，可跨不同團隊需求提供更好的靈活性。
* 使用[語法醒目提示和行編號](/help/main/c-experiences/c-manage-content/create-json-offer.md)重新整理HTML和JSON程式碼編輯器。
* 增強的快速資訊和詳細資訊螢幕，以便更方便地訪問資訊。
* 會話持久性搜索和篩選器選項。

![RecommendationsUI刷新](/help/main/r-release-notes/assets/recs-ui-refresh.png)

### Target Standard/Premium 25.1.1 （2025年1月9日） {#ui-update-1}

此版本包括以下更新：

#### [!UICONTROL Offers Library]用戶介面更新

為了增強[!DNL Adobe Target]用戶的用戶體驗，此版本更新了[!UICONTROL Offers Library]用戶介面。

>[!NOTE]
>
>為確保對所有客戶進行無縫部署，此版本將部署在受控階段。 第一階段將初始目標客戶組升級為新的優惠UI。 後續階段將升級其餘客戶。

>[!IMPORTANT]
>
>有關[!DNL Target] UI版本切換終止計畫的重要資訊，請參閱[[!DNL Target]  UI版本切換棄用](/help/main/r-release-notes/release-notes.md#toggle)。

此更新使用最新的[!DNL Adobe Spectrum]設計系統，標準化了不一致的設計模式並引入了新的增強功能，包括：

* **批量優惠管理**：同時選擇和刪除或移動多個優惠。

* **[!UICONTROL Code Editor]升級**：已刷新HTML和JSON編輯器，其語法突出顯示和行編號。

* **改進的優惠卡**：增強的快速資訊和詳細資訊卡，便於資訊訪問。

* **持續搜索和篩選器**：添加會話持續搜索和篩選器選項。

有關詳細資訊，請參閱[優惠](/help/main/c-experiences/c-manage-content/manage-content.md)和本節中的子項目。 此部分中的所有優惠項目都已更新以反映這些UI更改。

以下是一段短視頻，突出介紹此版本中的更改：

![提供UI刷新視頻](/help/main/r-release-notes/assets/offers-video-v2.gif)

## 發行說明 - 2024

### [!DNL Adobe Experience Platform Web SDK] `__view__`範圍優化（2024年10月22日）

2024年7月22日至2024年8月15日，[!DNL Target]團隊優化了`__view__`範圍，提高了活動印象、訪問和訪問者報告的準確性。 此優化旨在自動捕獲自動呈現的主張的報告資料，並且對大多數帳戶應是透明的。

所有新[!DNL Adobe Experience Platform Web SDK]客戶都將啟用此優化。 但是，從at.js遷移且未遵循以下實施步驟的客戶禁用了優化。 我們敦促這些客戶在2025年2月3日前審查其實施情況。 在此日期之後，我們將為所有客戶啟用優化。 如下所述，如果未能在此之前審查和調整實施情況，可能會影響報告。 如果您需要確認您的實施是否受到影響，或需要更多時間來調整實施，請與[!DNL Adobe Customer Care]聯繫。

>[!IMPORTANT]
>
>如果您無法在2025年2月3日之前完成實施審查並解決任何問題，則可以請求一次性延長六個月。 確保在2025年1月31日之前提交您的請求。 Adobe會審查並決定你的請求。
...
* 提取使用全局`renderDecisions: true`作用域的[!UICONTROL Form-Based Experience Composer]建立的活動時不使用`__view__`

如果通知未按[資料收集](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/web-sdk/personalization/rendering-personalization-content){target=_blank}指南的&#x200B;*呈現個性化內容*&#x200B;中的說明實現，則[!DNL Target]和[目標報告分析](/help/main/c-integrating-target-with-mac/a4t/a4t.md)(A4T)中可能缺少報告資料。 在某些情況下，您可能會注意到通信量拆分不正確，因為未捕獲報告資料。 或者，在其他情況下，重複報告同一事件。

根據您的實施情況，檢查[!DNL Analytics]和A4T報告影響。

[!DNL Platform Web SDK]支援兩種實現類型來呈現體驗和個性化：
...
* **分割呼叫（也稱為頁面事件的頂端和底部）。**
  ...
  此實作型別是[建議的新](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/web-sdk/use-cases/top-bottom-page-events){target=_blank}分割呼叫實作方法[!DNL Adobe]。 使用此方法時，新的最佳化不會影響[!DNL Analytics]或A4T報表。
...
有關at.js版本的詳細資訊，請參閱[Adobe Target開發人員指南](https://experienceleague.adobe.com/zh-hant/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions){target=_blank}中的&#x200B;*at.js版本詳細資訊*。

### [!DNL Target]中的[!DNL Adobe Customer Journey Analytics]報告（2024年5月8日）

[Adobe Customer Journey Analytics](https://experienceleague.adobe.com/zh-hant/docs/customer-journey-analytics){target=_blank}和[!DNL Target]之間的整合為您的優化程式提供了強大的分析和節省時間的工具。
...
[!DNL Adobe]建議客戶盡快轉到更新的[Visual Editing Helper擴展](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)。

### `Browser:iPad`對象屬性（2024年4月30日）中`Browser:iPhone`和[!UICONTROL Browser]的更新

| 更新 | 詳細資料 |
|--- |--- |
| [!UICONTROL Browser:iPad]和[!UICONTROL Browser:iPhone]已在建立對象時所使用的[瀏覽器屬性](/help/main/c-target/c-audiences/c-target-rules/browser.md)中更新。 | [!DNL Adobe Target]可讓您[鎖定數個類別屬性中的任何一個](/help/main/c-target/c-audiences/c-target-rules/target-rules.md)，包括瀏覽您的頁面時使用特定[瀏覽器或瀏覽器選項](/help/main/c-target/c-audiences/c-target-rules/browser.md)的訪客。<P>從[!DNL Target]標準/高級版24.3.1（2024年3月4日至6日）開始，將更新使用目標UI（如`Browser:iPad`和`Browser:iPhone`）建立的內置訪問群體，以使用[!DNL iPad]、[!DNL iPhone]和`profile.mobile.deviceVendor`對`profile.mobile.isMobilePhone`和`profile.mobile.isTablet`執行正確的目標。<P>此更新不需要客戶方面的任何操作。<p><B>重要資訊</b>：要使客戶在配置檔案指令碼（和JavaScript段）中對[!DNL iPad]和[!DNL iPhone]執行正確的目標設定，客戶必須在2024年4月30日&#x200B;**之前進行手動更改。** 有關必須手動更改的備用設定的示例，請參閱[受眾屬性 [!DNL iPad] 中 [!DNL iPhone] 和[!UICONTROL Browser]的更新。](/help/main/c-target/c-audiences/c-target-rules/browser.md#updates) |
...
* 已修復計算活動中唯一選擇器數量的邏輯。 (TGT-47878)
* 已修復導致配置了[!UICONTROL Multivariate]&#x200B;(A4T)報告的[!UICONTROL Analytics for Target]&#x200B;(MVT)活動無法正確顯示的問題。 (TGT-47490)
* 改進報告中顯示的警告消息，當沒有通信的體驗用作控制體驗時。 (TGT-47537)
* 新增許多後端和本地化修正。
...
如需詳細資訊，請參閱[Adobe Target開發人員指南](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/allowlist-edges.html?lang=zh-Hant){target=_blank}中的&#x200B;*允許列出Target邊緣節點*。
...
* 已修復導致[!UICONTROL Auto Personalization]活動的報告在分析期間凍結的問題。 (TGT-44820)
* 修復了如果活動名稱包含方括弧（[!UICONTROL Activity]或[），則無法在]頁面上搜索活動的問題。 (TGT-44777)
...
| 功能 | 詳細資料 |
|--- |--- |
| AEM[!UICONTROL Content Fragments]用於無頭個性化和實驗 | 在[!DNL Adobe Experience Manager]個活動中使用[!UICONTROL Content Fragments] (AEM) [!DNL Target]。 將 AEM 的易用性和強大功能與 [!DNL Target] 的強大人工智慧 (AI) 和機器學習 (ML) 功能相結合，以協助 Headless 個人化和實驗。<P>如需詳細資訊，請參閱[AEM [!UICONTROL Content Fragments]](/help/main/c-integrating-target-with-mac/aem/content-fragments-aem.md)。 |
| [*Adobe Target 開發人員指南*](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hant){target=_blank} | *Adobe Target開發人員指南*&#x200B;已重新放置到&#x200B;*[!UICONTROL Adobe Experience League]*。 移至&#x200B;*[!UICONTROL Experience League]*&#x200B;有助於當地語系化其他語言的文字，在&#x200B;*Experience League*&#x200B;中統一搜尋範圍，並提供來自&#x200B;*[!UICONTROL Adobe Target Business Practitioner Guide]*&#x200B;和&#x200B;*[!UICONTROL Adobe Target Developer Guide]*&#x200B;的搜尋結果，並提供額外的好處。<P>將自動將您從上一個位置重定向到&#x200B;*[!UICONTROL Experience League]*。 請視需要更新您的書籤。 |
...
| 功能 | 詳細資料 |
|--- |--- |
| 已針對[!UICONTROL Auto-Allocate]和[!UICONTROL Auto-Target]最佳化A4T量度<p> (發行日期：2023 年 3 月 30 日) | 針對[!DNL Target]和[!UICONTROL A4T]活動使用[!UICONTROL Auto-Allocate]時，[!UICONTROL Auto-Target]可讓您根據二項式事件選擇量度，或根據連續事件選擇量度。<P>請注意受支援量度的以下變更：<ul><li>[!DNL Target] 已保留現有活動的先前行為，直到 2023 年 9 月 9 日。在此日期之後，將停止使用非支援之量度的活動，以強制現有活動遷移至新行為。</li></ul>如需詳細資訊，請參閱[和[!UICONTROL Auto-Allocate]活動的[!UICONTROL Auto-Target]A4T支援](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md#supported)中的「支援的目標量度」。<br>因為此功能，已更新以下教學課程：<ul><li>[在 [!DNL Analysis Workspace] 中為[!UICONTROL Auto-Allocate]個活動設定A4T報告](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-allocate-activities.html?lang=zh-Hant){target=_blank}</li><li>[在 [!DNL Analysis Workspace] 中為[!UICONTROL Auto-Target]個活動設定A4T報告](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-target-activities.html?lang=zh-Hant){target=_blank}</li></ul> |

* 增強客群和活動同步，以便在 [!DNL Adobe Experience Platform] 和 [!DNL Adobe Audience Manager] 中建立的項目可更快地在 [!DNL Target] UI 中使用。(TGT-44568)
* 增強UI，可讓使用者移除[!UICONTROL Default URL] > [!UICONTROL Administration] > [!UICONTROL Visual Experience Composer]下的[!UICONTROL Default URL]。 此變更可讓客戶將預設 URL 變更回空字串，這在以前於初始設定後是不可能的。(TGT-44577)
* 刪除禁止客戶編輯或刪除現成可用客群 (具有保留名稱的客群) 的限制。(TGT-44655)
* 已停用[!UICONTROL Done]選項，但載入旋轉器時，建立[!DNL Target]合併的對象[時，可在](/help/main/c-target/combining-multiple-audiences.md) UI中看到。 (TGT-44079)
* 修正[!UICONTROL Language]頁面底部的[!UICONTROL Audiences]連結，使其正確連結至&quot;[!UICONTROL Account communication preferences]&quot;頁面。 (TGT-43562)
* 解決在[!UICONTROL A/B Test] > [!UICONTROL Adobe Analytics] > [!UICONTROL Administration]下選取[!UICONTROL Reporting]選項後，有時無法讓客戶建立[!UICONTROL Reporting Experience Cloud Solution]活動的問題。 (TGT-44844)
* 已修正導致客戶無法從[!UICONTROL Multivariate Test] (VEC)內檢視具有多個體驗的[!UICONTROL Visual Experience Composer]活動中的最後一個體驗的問題。 VEC 底部的 [DOM 路徑](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)有時會造成客戶無法看到最後一個體驗。(TGT-44578)
* 如果頁面要求授權或叫用重新導向，VEC 中的瀏覽 URL 無法反映目前頁面，而此頁面在正常瀏覽器工作階段是可見的，這個問題已經修正。(TGT-44350)
* 已修正導致客戶無法在[!UICONTROL Filter Incompatible Criteria] > [!UICONTROL Recommendations]中變更[!UICONTROL Settings]設定的問題。 (TGT-44398)
* 修正使用名稱中包含點的[!DNL Recommendations]報表套裝時，造成POST要求建立[!UICONTROL Analytics Classifications]摘要失敗的問題。 (TGT-44598)
...
如需有關所有at.js版本的資訊，請參閱[Adobe Target開發人員指南](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}中的[at.js版本詳細資料](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hant){target=_blank}。
...
>[!NOTE]
>
>若要確保傳遞在VEC中撰寫的變更，請確定您使用的是版本高於2.8的[!DNL Target]SDK ([at.js](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}或[Adobe Experience Platform Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/release-notes.html?lang=zh-Hant){target=_blank} (alloy.js)。
...
新的[!DNL Adobe Target]模型API（也稱為Blocklist API）允許用戶查看和管理用於[!UICONTROL Automated Personalization]&#x200B;(AP)和[!UICONTROL Auto-Target]&#x200B;(AT)活動的機器學習模型中的功能清單。

有關詳細資訊，請參閱[Adobe Target開發人員指南](https://experienceleague.adobe.com/docs/target-dev/developer/api/models-api/models-api.html?lang=zh-Hant){target=_blank}中的&#x200B;*Models API Overview*。
...
* 修正從[!UICONTROL Modifications]切換到[!UICONTROL Experiences]模式後，在VEC中建立或編輯活動時，造成[!UICONTROL Compose]頁面上[!UICONTROL Browse]面板中無法正確顯示某些元件的問題。 (TGT-43300)
* 已修正導致部分客戶無法封存使用[!UICONTROL A/B Test]的[!UICONTROL Auto-Target]活動的問題。 (TGT-40978)
* 已新增在單一報表群組內的多個位置自動使用單一產品建議的功能。(TGT-40689)
...
* 修正在建立[!UICONTROL Clicked an Element] (MVT)活動時，選取[!UICONTROL Multivariate Testing]轉換目標時導致顯示錯誤訊息的問題。 (TGT-43842)
* 修正無法針對[!UICONTROL Impressions] (AP)活動在下載的CSV報告檔案中顯示[!UICONTROL Automated Personalization]欄的問題。 (TGT-43780)
...
* 修正了一個問題，以確保客戶可以在[!DNL Recommendations]和[!UICONTROL A/B Test] (XT)活動中正確顯示包含建議的[!UICONTROL Experience Targeting]物件（設計、條件、集合等）的使用情況，並從[!DNL Target] UI和[!DNL Recommendations]後端刪除不再使用的條件物件。 (TGT-42331)
...
| *Adobe Target 開發人員指南* | 的 *Adobe Target開發人員指南* 合併所有 [!DNL Target] 開發人員內容。 指南包含實施 [!DNL Target] 和 [!DNL Recommendations]、[!DNL Target] SDK 及 [!DNL Target] API 的資訊。<br>如需詳細資訊，請參閱[Adobe Target開發人員指南](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hant){target=_blank}。 |
...
* 新增的「[使用者代理用戶端提示](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/user-agent-and-client-hints.html?lang=zh-Hant){target=_blank}」支援。
* 修正在[!UICONTROL Offer Decisions] (XT)活動中呈現[!UICONTROL Experience Targeting]時斷斷續續造成逾時的問題。 (TNT-44611)

### at.js 2.9.0 版 (2022 年 5 月 27 日)

* 新增的「[使用者代理用戶端提示](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/user-agent-and-client-hints.html?lang=zh-Hant){target=_blank}」支援。
* 修正了同一個頁面上多個 mbox 請求有不同印象 ID 的錯誤。
...
* 已修正導致部分客戶無法新增 (或編輯) 特定產品建議至 AP 活動的問題。 (TGT-43503)
* 已修正 [!DNL Target] UI 中顯示下列錯誤訊息的問題：「您的全域 mbox 可能不同步。 請嘗試重新儲存。」這個問題是 UI 問題，並不會影響客戶的實作。 (TGT-43475)
* 修復了一個問題，如果在部署新的[!UICONTROL Audiences] UI之前建立了改進和訪問群體，則該問題會阻止一個客戶編輯活動的體驗級改進和訪問群體。 (TGT-43433)
* 已修正允許客戶在編輯活動的通報客群時可選取重複 [!DNL Adobe Audience Manager] (AAM) 客群的問題。 (TGT-43430)
* 已修正導致客戶除了在不同的工作區外無法建立重複客群的問題。 (TGT-43423)
* 已修復阻止客戶刪除在[!UICONTROL Form-Based Experience Composer]中建立的活動中具有臨時優惠的位置的問題。 (TGT-43315)
* 已修正導致客戶在按下影像產品建議然後重新整理 UI 之後無法存取代碼產品建議的問題。 (TGT-43566)
...
* 已啟用[!DNL Target] UI權杖重新整理，讓組織已啟用[Business ID帳戶](https://helpx.adobe.com/tw/enterprise/using/identity.html){target=_blank}和原則式驗證(PBA)。 (TGT-42590)
...
* 將為所有[!UICONTROL Audiences]客戶啟用新的[!DNL Target Standard] UI。

### Target Standard/Premium客戶工程修正（2022年3月22日）

此維護發行包含下列增強功能：

* 新增在將[!DNL Analytics]用於使用`prefetch` (A4T)的活動時，傳回`pageLoad`檢視和[!UICONTROL Delivery API]點選量度的[!UICONTROL Analytics as the reporting source]裝載資料的功能。 (TNT-43198)
...
* 針對從[!UICONTROL Delete]、[!UICONTROL Audiences]和[!DNL Adobe Experience Platform]匯入的對象，在[!DNL Adobe Experience Cloud]清單中新增[!DNL Adobe Target Classic]選項。 還新增了大量刪除功能。(TGT-42914)
...
* 已修正導致某些客戶無法從[!UICONTROL IPs to exclude from [!DNL Target] reporting data] > [!UICONTROL Administration]中的「[!UICONTROL Reporting]」清單刪除IP位址的問題。 (TGT-43384)
...
| 功能 | 詳細資料 |
| --- | --- |
| [!UICONTROL Visual Experience Composer] (VEC) | 新增 [Web 元件](https://developer.mozilla.org/en-US/docs/Web/Web_Components)的支援。在自訂元素及其內部元素上，可建立和測試個人化體驗和產品建議。<br>如需詳細資訊，請參閱[可視化體驗撰寫器選項](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#custom)。 |
...
| 功能 | 詳細資料 |
| --- | --- |
| 購物車型推薦 | 新增一系列演算法，以根據訪客購物車的內容提供推薦。<br>如需詳細資訊，請參閱[建立條件](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md)中的「購物車型」、[計劃和實作推薦](https://experienceleague.adobe.com/docs/target-dev/developer/recommendations.html?lang=zh-Hant){target=_blank} 中的「購物車新增/購物車檢視/結帳頁面」和「排除訪客購物車中已有的項目」，以及[使推薦以推薦索引鍵為依據](/help/main/c-recommendations/c-algorithms/base-the-recommendation-on-a-recommendation-key.md)中的「購物車型」。 |

### [!DNL Target Standard/Premium] 21.10.3 (2021 年 10 月 19 日)

此維護發行包含下列增強功能、修正和變更。

* 已修正導致客戶無法透過按一下[!UICONTROL A4T]活動報告中的[!DNL Analysis Workspace]按鈕在[!UICONTROL View in Analytics]中開啟[!DNL Target]面板的問題。 (TGT-42099 和 TGT-42100)
* 修正使用[!UICONTROL Edit Design]編輯[!UICONTROL A/B Test]和[!UICONTROL Experience Targeting] (XT)活動時，[!UICONTROL Form-Based Experience Composer]按鈕不顯示的問題。 (TGT-41980)
* 修正在建立新的[!UICONTROL Compatible]活動時，[!UICONTROL Recommendations]核取方塊無法顯示在條件選取中的問題。 (TGT-42053)
...
透過[!DNL Target]使用[!UICONTROL Audiences] [!DNL Adobe Experience Platform Web SDK]時，已新增下列增強功能：
...
* 在 [!DNL Target]UI 的不同位置中，加入了警告圖示、彈出視窗和訊息，指示該客群已在來源刪除，並且不再可用於[!DNL Target]活動。
   ...
   * 對象細分（活動建立工作流程[!UICONTROL Replace Audience]步驟上的[!UICONTROL Targeting]）：
...
| 功能 | 詳細資料 |
| --- | --- |
| [!UICONTROL Audiences] UI重新整理 | 作為[!DNL Adobe Target]團隊持續努力改善[!DNL Target]個使用者的使用者體驗的一部分，此版本重新整理[!UICONTROL Audiences] UI中的[!UICONTROL Profile Scripts]和[!DNL Target]頁面。 本次更新統一並標準化先前不一致的設計模式，同時加入了新的增強功能，例如：<ul><li>能夠同時選擇和刪除多個客群</li><li>重新整理的[客群建立器設計](/help/main/c-target/c-audiences/create-audience.md)</li><li>[!UICONTROL Audience]資料庫規則產生器中的排除規則支援</li><li>全新的「對象Source」篩選器，能更快速地探索對象</li><li>工作階段持續搜尋和篩選器選項</li></ul>如需詳細資訊，請參閱[客群](/help/main/c-target/target.md)。 |
...
* 已修正導致VEC中[!UICONTROL Experiences]個活動的「編輯條件」按鈕無法在[!DNL Recommendations]頁面上正確顯示的問題。 (TGT-39512)
* 修正了在重複和複製到測試工作區時阻止活動同步的問題。 (TGT-40686)
* 修正了無法在VEC中使用&quot;[&quot;時修改具有](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)體驗片段[!UICONTROL Insert After]的選擇器的問題。 (TGT-41802)
...
* 修正在[!UICONTROL Auto Personalization]中建立之[!UICONTROL Form-Based Experience Composer]活動的報表，會參照報表中已刪除之優惠的問題。 此問題會導致顯示以下錯誤訊息：「擷取此報表的資料時遇到問題。如果問題仍然存在，請聯絡 Adobe 客戶服務。」(TGT-41028)
...
| 功能 | 詳細資料 |
| --- | --- |
| [!UICONTROL Analytics for Target] (A4T) | 從使用[!UICONTROL View in Analytics]作為報告來源(A4T)的活動按一下[!UICONTROL Reports]頁面上的&quot;[!DNL Analytics]&quot;連結，[!DNL Analysis Workspace]現在會開啟。 之前，連結會開啟 [!DNL Analytics] 報告。(TGT-36959) |
...
如需詳細資訊，請參閱[SDK開發人員指南](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/python/overview.html?lang=zh-Hant){target=_blank}中的[Adobe Target檔案](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hant){target=_blank}。
...
* 已修復[!UICONTROL Administration] > [!UICONTROL Implementation]中的問題，該問題有時會導致錯誤消息，指出「您的全局框可能未同步。 請嘗試重新儲存。」
...
| 功能 | 詳細資料 |
| --- | --- |
| 重定向支援[!UICONTROL Analytics for Target]&#x200B;(A4T) | 使用 [A4T](/help/main/c-integrating-target-with-mac/a4t/a4t.md) 時，Platform Web SDK 現在支援 [!DNL Target] 重新導向。<br>如需詳細資訊，請參閱 [Analytics for [!DNL Target] 實施](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)。 |

### at.js 2.5.0 版 (2021 年 5 月 13 日)

此 at.js 版本包含下列增強功能和變更：

* [針對 at.js 的裝置上決策](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=zh-Hant){target=_blank}支援。
...
| 功能 | 詳細資料 |
| --- | --- |
| 針對 at.js 的裝置上決策支援<br>(日期有待宣佈) | 裝置上決策可讓行銷人員和開發人員以幾乎零延遲的方式，在使用者的瀏覽器上提供測試和個人化。<br>如需詳細資訊，請參閱「[針對 at.js 的裝置上決策](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=zh-Hant){target=_blank}」。 |
...
### 推薦摘要處理伺服器的 IP 位址變更 (2021 年 3 月 16 日)

[!DNL Target Recommendations] 摘要處理伺服器的 IP 位址已於 2021 年 3 月 16 日更新。 如需詳細資訊，請參閱「[推薦摘要處理伺服器使用的 IP 位址](/help/main/c-recommendations/c-recommendations-faq/ip-addresses-marketing-cloud.md)」。
...
* 增加允許的產品建議大小限制 (TGT-38304)：

  | 類型 | 先前的限制 | 新限制 |
  | --- | --- | --- |
  | HTML | 256 KB | 1024 KB |
  | Target UI 的視覺產品建議 | 64 KB | 每個體驗 1024 KB |
  | 透過 API | 512 KB | 1024 KB |

* [!UICONTROL Personalization Insights] (AT)和[!UICONTROL Auto-Target] (AP)活動的[!UICONTROL Automated Personalization]報告現在每日產生。 您可以選擇一個報告，在過去15、30和60天內提供[!UICONTROL Automated Segments]或[!UICONTROL Important Attributes]。 45 天與 90 天選項已經移除，讓其他的回顧視窗設定可以每日執行。(TGT-39472)
* 修復了當客戶在活動的[!UICONTROL Edit Dependency]頁上按一下[!UICONTROL Goals & Settings]時導致當前依賴項不顯示的問題。 (TGT-39340)
* 已修復刷新工作區的[!UICONTROL Audience Library]時的問題。 在重新整理前，目前所選工作區的客群已顯示。重新整理後，[!UICONTROL Default Workspace]及其對象已顯示。 目前工作區及其客群在重新整理後持續存在。(TGT-38871)
...
* 在[!DNL Adobe Analytics]活動中使用[!UICONTROL Analytics as the reporting source] (A4T)時，在選取[!UICONTROL Auto-Target]量度時新增警告。 [!UICONTROL Auto-Target]模型已最佳化，以搭配二進位（轉換型）量度。 選取連續量度（例如收入）可能會有次優結果，而[!UICONTROL Personalization Insights]報告可能不準確。 (TGT-38926)
...
* 已產生使用A4T和[!UICONTROL Automated Segments]轉換量度之[!UICONTROL Important Attributes]活動的[!UICONTROL Auto-Target]和[!DNL Analytics]報告，且看起來與使用[!DNL Target]作為報告來源時相同。 (TGT-38931)
...
* 已修復[!UICONTROL Recommendations]中導致[!UICONTROL Create Sequence]頁在更改行業垂直後掛起的問題。 (TGT-38160)
* 已修復阻止用戶從[!UICONTROL Automated Personalization]&#x200B;(AP)活動中的優惠中刪除受眾的問題。 (TGT-39058)
...
| 功能 | 詳細資料 |
| --- | --- |
| 裝置上決策 | 裝置上決策可讓行銷人員和產品開發人員在使用者的裝置內、跨通道、幾乎零延遲的情況下，提供實驗和機器學習驅動的個人化。<br>速度與效能至關重要——在客戶洞察與使用者滿意度方面。<br>裝置上決策可讓您將 A/B 測試與體驗鎖定 (XT) 活動類型中的關鍵個人化和實驗指示編譯為「最佳化成品」。JSON 物件會透過 CDN 載入到客戶裝置上。而且，由於裝置上的決策功能會以原生方式與 [!DNL Adobe Experience Cloud] 產品連接，[!DNL Target] 使用者可獲得快速分析和更快速的體驗互動。<br>如需詳細資訊，請參閱*[at.js的裝置上決策](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/on-device-decisioning/on-device-decisioning.html?lang=zh-Hant){target=_blank}和伺服器端的[裝置上決策簡介](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=zh-Hant){target=_blank}。 |

此發行包含下列增強功能、修正和變更：

* 修正了無法在[!UICONTROL Average Lift Confidence Interval]列之[!UICONTROL Confidence]報告中顯示[!DNL Auto-Target]和[!UICONTROL Total]的問題。 所有個別體驗的量度都會正確顯示。(TGT-37301)
* 修正自9月15日下午2[!DNL Adobe Target Premium] (PDT)至10月6日上午9[!UICONTROL Auto-Target] (PDT)影響:30使用者:25報告的問題。 檢視受影響的轉換量度（使用&quot;[!UICONTROL Viewed a page]&quot;或&quot;[!UICONTROL Clicked on mbox]&quot;選項設定）的報告時，轉換率報告不正確。 目前沒有已知的傳送問題。
* 已在[!UICONTROL Last Updated At]表格中新增可選取的[!UICONTROL Catalog Search]欄及[!UICONTROL Last Updated At]篩選器。 此增強功能可節省時間和精力，因為您不需要開啟每個個別項目來查看上次更新時間，而且您可以依上次更新項目的日期進行篩選。
...
* 修正當工作區名稱（在[!UICONTROL Activity]中指定）包含撇號時，使用者無法顯示[!UICONTROL Adobe Admin Console for Enterprise]頁面的問題。 (TGT-37709)
* 修正在選取最佳化和轉換量度時， [!UICONTROL Auto-Allocate]個活動中即使已指定報表套裝，錯誤訊息仍錯誤地通知使用者選取報表套裝的問題。 (TGT-37689)
* 修正在導覽至[!UICONTROL Goals and Settings]頁面後，有時導致[!UICONTROL Targeting]頁面上的量度空白的問題。 (TGT-37691)
...
如需更多資訊，請參閱[管理 Target 概觀](/help/main/administrating-target/administrating-target.md)。
...
| [!DNL Analysis Workspace]<br>2020 年 6月 25 日的 A4T 支援 | [!UICONTROL Anaytics for Target]現在支援[!DNL Analysis Workspace] (A4T)。 [!UICONTROL Analytics for Target (A4T) panel]可讓您在[!DNL Adobe Target]中分析您的[!DNL Analysis Workspace]活動和體驗。<br>如需詳細資訊，請參閱&#x200B;*《 Analytics 工具指南》*&#x200B;中的 *A4T 報告*&#x200B;和 [Analytics for Target (A4T)](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/panels/a4t-panel.html?lang=zh-Hant&?lang=zh-Hant)面板的 [Analytics 的報告 (A4T)](/help/main/c-integrating-target-with-mac/a4t/reporting.md)。 |
...
| 功能/增強功能 | 說明 |
| --- | --- |
| at.js 1.8.2 | 此 at.js 版本為維護版本，包含下列修正：<ul><li>修正使用 CNAME 和 Edge Override (at.js 1) 時的問題。*x* 可能會錯誤建立伺服器網域，導致請 [!DNL Target] 請求失敗。(TNT-35064)</li></ul>如需詳細資訊，請參閱[&#x200B; at.js 版本詳細資訊](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}。 |
| at.js 2.3.1 | 此 at.js 版本為維護版本，包含下列增強功能和修正：<ul><li>透過 [targetGlobalSettings](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/targetglobalsettings.html?lang=zh-Hant){target=_blank} 將 `deviceIdLifetime` 設定設為可覆寫。(TNT-36349)</li><li>修正使用 CNAME 和 Edge Override (at.js 2) 時的問題。*x* 可能會錯誤建立伺服器網域，導致請 [!DNL Target] 請求失敗。(TNT-35065)</li><li>修正使用 [!DNL Target] [!DNL Launch]擴充功能 v2 和 [!DNL Adobe Analytics] [!DNL Launch]擴充功能時，[!DNL Target] 延遲 [!DNL Analytics]`sendBeacon` 呼叫的問題。(TNT-36407、TNT-35990、TNT-36000)</li></ul>如需詳細資訊，請參閱[&#x200B; at.js 版本詳細資訊](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}。 |
...
* 修正 Target UI 中，當使用者在單一體驗中取代內容後，導致其他推薦產品建議預覽顯示已編輯內容的問題。(TGT-36053 和 TGT-36894)
* 修正造成部分使用者無法從推薦目錄刪除項目的問題。 (TGT-36455)
* 修正造成使用者無法將推薦標準儲存在多頁活動上的問題。(TGT-36249)
* 修正造成行為資料來源選項按鈕在第二次連續編輯標準時消失的問題。(TGT-36796)
* 修正造成推薦演算法在延長期間內顯示「擷取結果」的顯示問題。(TGT-36550 和 TGT-36551)
* 已更新多種語言本地化的大量 UI 字串。
...
如需詳細資訊，請參閱[&#x200B; at.js 版本詳細資訊](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}。
...
* 我們淘汰目前版本的Adobe Analytics API不支援的Adobe Analytics量度，並將Target (A4T)活動的可用量度清單標準化。 這將使我們在未來的 Adobe Target 版本中可擴充 A4T 支援。
...
* 當您登入 [!DNL Adobe Experience Cloud] 時，將會進入新的標題導覽。它看起來與先前的導覽非常類似，頂端有黑色列，但提供下列改良功能：
   ...
   * 標頭的[!DNL Target]下拉式清單中目前無法使用[!UICONTROL Notifications]的通知。
...
| 功能/增強功能 | 說明 |
| --- | --- |
| ![Premium badge](/help/main/assets/premium.png) 多值屬性 | 有時，您想要使用多值欄位。 考量下列範例：<ul><li>您提供影片給使用者。某部電影有多位演員。</li><li>你賣音樂會的票。給定用戶有多個喜愛的樂隊。</li><li>你賣衣服。襯衫有多種尺寸。</li></ul>若要處理這些案例中的推薦，您可以將多值資料傳遞至 Target 推薦，並使用特殊的多值運算子。<br>如需詳細資訊，請參閱[使用多值屬性](/help/main/c-recommendations/c-algorithms/work-with-multi-value-attributes.md)。 |

### Target Standard/Premium 19.10.1 (2019 年 10 月 22 日)

| 功能/增強功能 | 說明 |
| --- | --- |
| ![Premium badge](/help/main/assets/premium.png)以使用者為基礎的推薦<br> (2019 年 10 月 24 日) | 根據每位訪客的瀏覽、檢視和購買記錄來建議項目。這些項目通常稱為「為您推薦」。<br>此標準可讓您為新訪客和回訪訪客提供個人化內容和體驗。建議清單會針對訪客的最近活動加權，並會在工作階段中更新，當訪客瀏覽您的網站時，推薦清單會變得更個人化。<br>如需詳細資訊，請參閱[「標準／演算法」](/help/main/c-recommendations/c-algorithms/algorithms.md#criteria-algorithms)中的「使用者導向的推薦」。 |
...
* 推薦產品建議現在可新增至 A/B 測試與體驗定位活動中除 DIV 以外的元素 (例如P、UL、H1)。(TGT-34333)
...
| 功能/增強功能 | 說明 |
| --- | --- |
| ![Premium badge](/help/main/assets/premium.png)<br>在 A/B 測試與體驗鎖定 (XT) 活動中的推薦 | 推薦產品建議 (運算法) 狀態會顯示在包含推薦產品建議之 A/B 測試和 XT 活動的概觀頁面上。狀態包括：可用結果、不可用結果和摘要失敗(TGT-33649)<br>請參閱「[推薦作為產品建議](/help/main/c-recommendations/recommendations-as-an-offer.md#status)」。 |
| 透過 Experience Cloud ID (ECID) 資料庫提供 at.js 2.0 以上版本的跨網域追蹤支援 | 之前的 at.js 2 不支援跨網域追蹤。*x* 版本不支援此函數。透過此版本，使用 at.js 2.0 或以上版本的客戶現在可以透過 ECID 資料庫利用跨網域追蹤功能。ECID 資料庫必須安裝在頁面上並搭配使用 at.js 2.0 或以上版本，這樣跨網域追蹤功能才能運作。必須使用 [Experience Cloud ID library 4.3.0+](https://experienceleague.adobe.com/docs/id-service/using/release-notes/release-notes.html?lang=zh-Hant&?lang=zh-Hant)。<br>請參閱 [at.js 2.x 中的跨網域追蹤支援](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}。 |
| Target可透過Apple ID (ECID)資料庫4.3支援Experience Cloud的ITP 2.1和ITP 2.2 | 現在，Target客戶可以利用Apple的CNAME認證計畫，減少Adobe的ITP 2.1和ITP 2.2的影響。<br>透過此版本，Target推出與ECID資料庫4.3的流暢整合，利用伺服器端Cookie減少ITP 2.1和ITP 2.2的影響。強烈建議Target客戶部署[ECID library 4.3+](https://experienceleague.adobe.com/docs/id-service/using/release-notes/release-notes.html?lang=zh-Hant&?lang=zh-Hant)並搭配使用Target的JavaScript資料庫，以緩解任何未來ITP版本的影響。 ECID 資料庫將繼續推出增強功能，針對瀏覽器所推出的不斷變化的 Cookie 原則，提供完善的解決方案。<br>請參閱 [Apple 智慧型追蹤預防 (ITP) 2.x](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/apple-itp-2x.html?lang=zh-Hant){target=_blank}。 |

**增強功能、修正和變更**

* 修正在新增重複值時會阻止清除推薦活動中的排除值的問題。(TGT-34996)
* 您現在可以從 Targeting 頁面移除推薦活動中的設計 (三步驟引導式工作流程的步驟 2)。請注意，若要移除設計，必須選取一個以上的設計。(TGT-35118)
...
* 修正推薦產品建議無法在一些客戶的網站上顯示的問題。客戶可以看到傳送 API 呼叫中的產品建議內容，但網站上未套用該產品建議。(TNT-32680)
...
如需此版本和舊版 at.js 的相關資訊，請參閱 [at.js 版本詳細資料](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}。
...
| 功能/增強功能 | 說明 |
| --- | --- |
| 可視化體驗撰寫器 (VEC) | **新增 VEC 功能表選項**：當您在 VEC 中按一下頁面元素時，會出現一個功能表，顯示該元素類型的可用選項。<ul><li>現在，您可以使用[!UICONTROL Styles > Background]選項更改所選元素的背景影像和顏色。 (TGT-15001)</li></ul>請參閱[可視化體驗選項](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)中的&#x200B;*樣式*。<br>**點擊追蹤改善**：我們已改善 VEC 和單頁應用程式 (SPA) VEC 內設定點擊追蹤的程序。<ul><li>選取要在點擊追蹤中使用的元素時，所有可用元素的名稱都會顯示在右側的修改面板中，讓您快速輕鬆地選取所需元素。</li><li>三部分指導活動工作流的[!UICONTROL Goals & Settings]頁顯示一個數字，表示為按一下跟蹤選擇的元素數。 您可以將游標移至此數字上，查看所有所選元素的名稱。(TGT-33878)</li></ul>請參閱[點擊追蹤](/help/main/c-activities/r-success-metrics/click-tracking.md)。 |
...
| ![Premium 徽章](/help/main/assets/premium.png) Automated Personalization (AP) 和自動鎖定目標 | **使用特定體驗作為控制**：建立 AP 或自動鎖定目標活動時，您可以選取要用來作為控制的體驗。此功能可讓您根據活動中設定的流量配置百分比，將整個控制流量傳送至特定體驗。接著，您可以根據該體驗之控制流量，評估個人化流量的效能報表。將繼續提供目前的控制選項 (隨機提供的體驗)。（TGT-32801、TGT-26572和TGT-26571）<br>請參閱[為您的Automated Personalization或自動目標活動選擇控制項](/help/main/c-activities/t-automated-personalization/experience-as-control.md)。<br>**個人化洞察報告**：這是可讓行銷人員方便使用的屬性命名，當訪客看到特定位置中的特定內容時，可提供更具意義的資訊。(TGT-33421 和 TGT-34957)<br>請參閱[Target 個人化演算法的資料收集](/help/main/c-activities/t-automated-personalization/ap-data.md)。 |
| ![Premium 徽章](/help/main/assets/premium.png) 推薦 | 建立「最近查看的項目」邏輯時，您可以使用「先前建議購買的項目」切換按鈕。(TGT-34030)<br>有關詳細資訊，請參閱「建立條件」中的[最近查看的項目](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md#previously-purchased)。 |
| Google Chrome SameSite Cookie 原則 | Google 最近宣佈自 Chrome 76 起 (該版本預定在 2019 年 7 月 30 日發行)，開發人員必須明確指定哪些 Cookie 可以跨多個網站運作，以及哪些 Cookie 可以追蹤使用者。<br>在產業大幅演進並為消費者打造更安全 Web 環境的同時，Target 全心致力於提供個人化體驗，同時符合並超越訪客對於隱私權的期望。請參閱 <br>[Google Chrome SameSite Cookie 原則](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/google-chrome-samesite-cookie-policies.html?lang=zh-Hant){target=_blank}。 |
...
| 功能/增強功能 | 說明 |
| --- | --- |
| Adobe 選擇加入支援 | 「Adobe 選擇加入」是簡化 Adobe 解決方案與同意管理平台整合的方法。<br>如需 Adobe 選擇加入的詳細資訊，請參閱[隱私權與一般資料保護規範 (GDPR)](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=zh-Hant){target=_blank}。 |
...
| 傳送通知 | 可讓開發人員在透過體驗的程式碼 (而不是透過 `applyOffer()` 或 `applyOffers()`) 呈現。<br>如需詳細資訊，請參閱 [adobe.target.sendNotifications(options)](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-sendnotifications-atjs-21.html?lang=zh-Hant){target=_blank}。 |
...
* 修正導致一小組洞察報告傳回 0% 轉換率 (而非實際轉換率) 的問題。(TNT-32125)
...
| 功能/增強功能 | 說明 |
| --- | --- |
| at.js 版本 2.0.0<br>2019 年 2 月 19 日 | at.js 2.x 現已可用。<br>最新版本的 at.js 提供豐富的功能組合，讓您的業務得以奠基於新一代的用戶端技術，同時發展個人特色。本次的新版本著重於升級 at.js，進而與單一頁面應用程式 (SPA) 產生和諧互動。<br>以下是幾個使用 at.js 2.x 特有 (舊版未提供) 的優點:<ul><li>可以在頁面載入時將所有產品建議加入快取，把多次伺服器呼叫減少為一次。</li><li>大幅改善一般使用者在網站上的體驗，因為產品建議能透過快取立即顯示，避免傳統伺服器呼叫引發的延遲時間。</li><li>只要編寫一行程式碼以及請開發人員設定一次，行銷人員就能透過單一頁面應用程式上的可視化體驗撰寫器 (VEC) 建立及執行 A/B 和體驗 (XT) 活動。</li></ul>at.js 2.x 引進以下新函數:<ul><li>getOffers()</li><li>applyOffers()</li><li>triggerView()</li></ul>導入 at.js 2.x 後，以下函數已遭到淘汰:<ul><li>mboxCreate()</li><li>mboxDefine</li><li>registerExtension()</li></ul>如需詳細資訊，請參閱[從 at.js 1.x 升級為 at.js 2.x](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank} 與 [at.js 函數](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}。<br>**注意**：如果您需要[一般資料保護規範](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=zh-Hant){target=_blank} (GDPR){target=_blank}的Adobe選擇加入支援，目前必須使用at.js 1.7.0。at.js 2.x不支援選擇加入支援。 |
| at.js 1.7.0 版<br>，2019 年 2 月 14 日 | at.js 1.7.0 現已可用。<br>該版本提供「Adobe 選擇加入」支援。「Adobe 選擇加入」是簡化 Adobe 解決方案與同意管理平台整合的方法。<br>如需Adobe選擇加入的詳細資訊，請參閱[隱私權與一般資料保護規範](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=zh-Hant){target=_blank} (GDPR){target=_blank}。<br>該版本另修正一個 Target 問題，亦即將重新導向 URL 參數覆寫成來自重新導向 URL 的參數。<br>**注意**：如果您需要 GDPR 的「Adobe 選擇加入」支援，目前必須使用 at.js 1.7.0。「選擇加入」支援不支援 at.js 2.x。<br>如需所有版本的清單，請參閱 [at.js 版本詳細資料](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant){target=_blank}。 |
...
| 功能/增強功能 | 說明 |
| --- | --- |
| 單一頁面應用程式可視化體驗撰寫器 | 適用於單頁應用程式 (SPA) 的可視化體驗撰寫器 (VEC) 能讓行銷人員在 SPA 上，自己動手建立測試並個人化內容，無需持續開發的相依性。VEC 可用來在 React 和 Angular 等最熱門的架構上建立活動。(TGT-27916)<br>如需詳細資訊，請參閱[單一頁面應用程式 (SPA) 可視化體驗撰寫器](/help/main/c-experiences/spa-visual-experience-composer.md)和[單一頁面應用程式整合](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/target-atjs-single-page-application.html?lang=zh-Hant){target=_blank}。<br>除了前述文章之外，我們另準備了許多有關 SPA 和 at.js 等主題等文章，為您解說這項功能及如何實作。如需詳細資訊，請參閱[文件變更](/help/main/r-release-notes/doc-change.md)。 |
...
| at.js<br> (2019 年 1 月 16 日) | at.js 1.6.4 是解決下列問題的維護版本:<ul><li>修正 Microsoft Internet Explorer 11 中導致套用重複產品建議的競爭條件顯現。(TNT-31374)</li><li>修正當預設產品建議具有 click-token 和 html 產品建議時會影響點擊追蹤的問題。(TNT-31493)</li><li>延伸每個 Target 要求的 mboxEdgeCluster Cookie。唯有 mboxEdgeOverride 啟用時才會使用。(TNT-31485)</li></ul> |
  ...
  <tr> 
   ...
   <td colname="col2"> <p>at.js 版本 1.6.3 現已可用。 </p> <p> 
     <ul id="ul_2C7CB74B1AAF4B52B6EB382977F7DC28"> 
      ...
      <li id="li_6504E90D7C534A1BB9A2DE8510CE3B90"> <p>修正 at.js 1.6.2 導入的問題，亦即將來自不同活動的可視化體驗撰寫器 (VEC) 產品建議套用至同一個 CSS 選取器時，不會遵守活動優先順序。(TNT-31052) </p> </li> 
     ...
     </ul> </p> <p>如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="dita" scope="local">at.js 版本詳細資料</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_EF529199D1C741F7BDBC9C41A37B7D26"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1" class="premium"> <p>個人化洞察報告 </p> <p> <p>注意：2018 年 11 月 14 日推出。 </p> </p> </td> 
   <td colname="col2"> <p><span class="wintitle">Automated Personalization (AP)</span> 和<span class="wintitle">自動鎖定目標 (AT)</span> 活動的使用者可以使用兩個專用報表。 </p> <p> 
     <ul id="ul_C338AC34C57C49E1A8DFA471167EC40A"> 
      <li id="li_2329BFC8CC524EBBA99C2F8EDC745B90"> <p><b><span class="wintitle">自動化區段</span>:</b> 不同訪客對您的 AP/AT 活動中的產品建議/體驗有不同的反應。此報表顯示 Target 的個人化模型定義之不同自動化區段如何回應活動中的產品建議/體驗。 </p> </li> 
...
<table id="table_B1911F55CCE1428881D258380A8254A9"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>您現在可以複製體驗鎖定目標 (XT) 活動中的體驗，不必從頭再次建立體驗，即可進行微幅變更。A/B 測試已可使用此功能。(TGT-31504) </p> <p>請參閱<a href="https://experienceleague.adobe.com/docs/target/using/activities/experience-targeting/create-targeting/xt-add-experience.html?lang=zh-Hant&?lang=zh-Hant" format="html" scope="external">建立體驗</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>Automated Personalization (AP) 活動的產品建議 </p> </td> 
   <td colname="col2"> <p>2018 年 9 月版新增增強功能，讓您根據報表群組篩選產品建議。您現在可以篩選「未指派的產品建議」，以便為目前未指派至任何報表群組的產品建議，指派報表群組。(TGT-31882) </p> <p>請參閱<a href="https://experienceleague.adobe.com/docs/target/using/activities/automated-personalization/create-ap-activity.html?lang=zh-Hant&?lang=zh-Hant" format="html" scope="external">建立 Automated Personalization 活動</a>。 </p> </td> 
  </tr> 
  <tr> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>此維護版本解決下列問題: </p> <p> 
     ...
     </ul> </p> <p> <p>重要：此外，at.js 1.6.2 版也包含 1.6.1 和 1.6.0 版所有的增強功能和修正。這兩個版本已無法下載。如果使用 1.6.1 或 1.6.0，建議升級為 1.6.2 版。 </p> </p> <p>如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="html" scope="external">at.js 版本詳細資料</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_9C39ACD865CE4167BDBAA093EDFD3B68"> 
      <li id="li_19710BA5965E4F858B128E1E9FF89471"> <p>您現在可以在排除群組中使用相同位置的多個產品建議。若為大量排除項目 (約 1,000 個)，您也會在編寫 Automated Personalization (AP) 活動時，發現「管理內容」對話方塊和預覽頁面載入更快。(TGT-31329)請參閱<a href="/help/main/c-activities/t-automated-personalization/managing-exclusions.md#topic_30B4E4F89C914EB2B20B038C0299ED2E" format="dita" scope="local">管理排除專案</a>。 </p> </li> 
      <li id="li_542C66E2998541BC87D0A96F4672C665"> <p>您現在可以依報表群組篩選產品建議。(TGT-31643)請參閱<a href="/help/main/c-activities/t-automated-personalization/create-ap-activity.md#task_8AAF837796D74CF893CA2F88BA1491C9" format="dita" scope="local">建立Automated Personalization活動</a>。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_4785030753B24AA1A973E1DF790B83DD"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1" class="premium"> <p>個人化洞察報告 </p> </td> 
   <td colname="col2"> <p>針對您的「 Automated Personalization 」(AP) 和「自動鎖定目標」(AT) 活動存取專業化的報表: </p> <p> 
     <ul id="ul_54652C5AE0984657BB9A0E46673CB2F1"> 
      <li id="li_0807959BA7D94114BE47A43D3454CAB4"> <p><b>「自動化區段」：</b>瞭解 Target 之個人化模型所定義的不同自動化區段，會如何回應您活動中的產品建議/體驗。 </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_406B95728467496CA6CC5892F88B69FE"> 
      ...
      <li id="li_27FEBEE245E64ADF9ADF561C6CBBDE8F"> <p>多虧有了更大的編輯視窗，您可以更有效率地編輯產品建議。(TGT-31052) </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_7E3513EABA4948DC92EADCCE0234A9FF"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p>推薦 </p> </td> 
   <td colname="col2"> <p> 
...
* Automated Personalization (AP) 和多變數測試 (MVT) 活動的 Target 步驟 (三步驟引導工作流程的步驟 2) 中的圖表已重新設計成更加符合 A/B、體驗鎖定目標 (XT)，以及推薦活動。(TGT-30712)
...
<table id="table_5A60FFE5E86148F4BDC6A7031D03D6BA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>已針對 Target UI 和內容傳送，新增 Microsoft Edge 支援。 </p> <p>如需詳細資訊，請參閱 .<a href="https://experienceleague.adobe.com/docs/target-dev/developer/implementation/supported-browsers.html?lang=zh-Hant" format="dita" scope="local">支援的瀏覽器</a> (TGT-14102) </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>推薦 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_F382C8E7708846A08676E1534BC92878"> 
      <li id="li_70E89504525C4119B588C230DCE772E8"> <p>您可以檢視個人資料指令碼資訊快顯卡，這類似產品建議資訊卡片。您可以透過設定檔指令碼資訊卡，檢視參考特定設定檔指令碼的活動，及其他實用中繼資料。(TGT-28253) </p> <p>有關詳細資訊，請參閱<a href="/help/main/c-target/c-visitor-profile/profile-parameters.md#concept_8C07AEAB0A144FECA8B4FEB091AED4D2" format="dita" scope="local">配置檔案指令碼屬性</a>中的「查看配置檔案指令碼資訊卡」部分。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>客群 </p> </td> 
...
<table id="table_76576D9D931B4DA99900F2C03175938E"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>at.js </p> </td> 
   <td colname="col2"> <p>at.js 1.3.0 版現已可用。如需詳細資訊，請參閱<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hant" format="dita" scope="local">下載 at.js</a> 和 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="dita" scope="local">at.js 版本詳細資料</a>。 </p> <p> 
     <ul id="ul_349BEB37B6C94FF0801F121042037803"> 
      <li id="li_4C2F82F4DD394ED5A0BFF978B15FEDDF"> <p>下列新事件可協助追蹤、除錯和自訂與 at.js 的互動: </p> <p> 
        ...
        </ul> </p> <p>如需詳細資訊，請參閱<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hant" format="dita" scope="local"> at.js自訂事件</a>。 </p> </li> 
      <li id="li_E2704294F8BA47FFAABE7572F67FB5C0"> <p>您可以使用來自資料提供者的其他參數來擴大 at.js 要求。資料提供者應該已新增至 <span class="codeph">dataProviders 機碼</span> 下的 <span class="codeph">window.targetGlobalSettings</span>。 </p> <p>如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hant" format="dita" scope="local">targetGlobalSettings()</a> 中的「資料提供者」。 </p> </li> 
...
<table id="table_AE38682151A948AEA21E35A353F18D76"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p>推薦：自訂條件的包含規則、集合和排除 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p>推薦：最近查看的條件的包含規則、集合和排除 </p> </td> 
...
<table id="table_0F5BF9370E214302BDFE0AC2D66EC773"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>at.js </p> </td> 
   <td colname="col2"> <p>at.js 1.2.3 新增 JSON 產品建議的支援。只有在使用表單式體驗撰寫器建立的活動中才支援 JSON 產品建議。目前使用 JSON 產品建議的唯一方式是透過直接 API 呼叫。請參閱<a href="/help/main/c-experiences/c-manage-content/create-json-offer.md#concept_63C7BEE1F0DB4A7596D997219B7C136D" format="dita" scope="local">建立 JSON 產品建議</a>。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_872FE2BE61CC4A5CA369D9A6C730686E"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p>產品建議 </p> </td> 
   <td colname="col2"> <p> 
     <ul id="ul_BD0C5B260E7E4F139FBC1FBA286C0B81"> 
      <li id="li_FCDBABE6C5034A3596F5BBF024245FB9"> <p>Target 現在支援在產品建議資料庫中建立 JSON 產品建議，以用於表單式體驗撰寫器。請參閱<a href="/help/main/c-experiences/c-manage-content/create-json-offer.md#concept_63C7BEE1F0DB4A7596D997219B7C136D" format="dita" scope="local">建立 JSON 產品建議</a>。(TGT-27064) </p> </li> 
      <li id="li_5500AE7DCF4146E88E4619382CE8E836"> <p>您現在可以在每個產品建議的定義快顯卡中，檢視參考代碼產品建議的活動。此功能不適用影像產品建議。檢視<a href="/help/main/c-experiences/c-manage-content/manage-content.md#concept_17874A6FCBB743AA84C5988E8571CCF3" format="dita" scope="local">選件</a>。 (TGT-26277) </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>推薦 </p> </td> 
...
<table id="table_793CDDF1BD9E48BDBABBF6CD979BE186"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>at.js </p> </td> 
   <td colname="col2"> <p>at.js 1.2.2 版現已可用。如需詳細資訊，請參閱<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hant" format="dita" scope="local">下載 at.js</a>。 </p> <p> 
...
<table id="table_6ADDF3552AD04666B76F2D3F457BB042"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>產品建議 </p> </td> 
   <td colname="col2"> <p> 如果使用者具有「編輯者」權限，該使用者無法編輯參考了線上或排程活動的產品建議。 </p> <p> <p>注意：針對使用 <a href="https://experienceleague.adobe.com/docs/target/using/administer/manage-users/enterprise/property-channel.html?lang=zh-Hant&?lang=zh-Hant" format="html" scope="external">企業使用者權限</a>的 Target Premium 客戶，如果使用者選取「所有工作區」選項，則 Target 會使用使用者在各工作區間的最高權限。如果最高權限為「編輯者」，則 Target 會限制編輯 (如上所述) </p>。 </p> <p>這些限制會套用至所有產品建議，而不僅僅是在 Target 中建立的產品建議。(TGT-27276) </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_667DDEDDC5284C8393F8BCA5CD9EF12A"> 
      <li id="li_E00DB93297EC4100B46E42D867757DAA"> <p>您現在可以在產品建議資料庫中的快顯卡檢視產品建議定義詳細資料，而不需開啟產品建議。(TGT-26377) </p> <p> <img src="assets/offer-card.png" id="image_1980AE8E9BED424085CC482C773C20EC" /> </p> <p>如需詳細資訊，請參閱<a href="/help/main/c-experiences/c-manage-content/manage-content.md#concept_17874A6FCBB743AA84C5988E8571CCF3" format="dita" scope="local">產品建議</a>。 </p> </li> 
      <li id="li_F71AC4FDAC0E4BEE81D39490E82686C0"> <p>您可以在建立活動時於產品建議選取器中複製和編輯產品建議和資料夾。(TGT-26936) </p> <p> <img src="assets/offer-picker.png" id="image_1077A6C7A8DD40FB9370CB55BD7260E5" /> </p> <p>如需詳細資訊，請參閱<a href="/help/main/c-experiences/c-manage-content/manage-content.md#concept_17874A6FCBB743AA84C5988E8571CCF3" format="dita" scope="local">產品建議</a>。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
* 您可以在對象資料庫和產品建議資料庫中，依工作區篩選 (適用具有企業使用者權限的 Target Premium 使用者)。(TGT-26813)
...
<table id="table_8457FAE3508F454F9DFDEF093FBD7E40"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><b>2017 年 10 月 13 日</b> </p> <p> <span class="filepath">at.js</span> 版本 1.2.1 現已可用。如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="dita" scope="local">at.js 版本詳細資料</a>。 </p> <p> 
...
<table id="table_0A8817F64F434875A485FD671C6988AB"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>表單式體驗撰寫器和 HTML 產品建議編輯器，現在使用與可視化體驗撰寫器 (VEC) 在自訂代碼中所使用相同的代碼編輯器。(TGT-25808) </p> <p>在表單式體驗撰寫器中使用代碼編輯器和建立 HTML 產品建議時，此增強提供您下列功能: </p> <p> 
     <ul id="ul_CBB17806FBF34774A8160A61204ED014"> 
      <li id="li_22665F583F1742E280D5BC7EC4203007"> <p>行號現在會顯示，以獲得更好的使用性。 </p> </li> 
      <li id="li_B0D863CDAD2E46A4B133BB86886EB527"> <p>將語法醒目提示可幫助您避免不正確的 HTML 產品建議語法。 </p> </li> 
...
* 具有核准者權限的使用者，現在可以產生和啟用設定檔 API 驗證 Token。(TGT-24074)
  ...
  如需詳細資訊，請參閱[設定檔 API 設定](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/methods/profile-api-settings.html?lang=zh-Hant){target=_blank}。
...
* 改進當使用者的 Target 作業逾時時，在「管理」、「客群」和「推薦」頁面上設定選項的工作流程。當使用者按一下「儲存」，會顯示工作階段已到期訊息，但在重新登入之後，對話方塊會通知使用者已成功登入，而 Target 中的 UI 會維持在相同的頁面上，不會遺失資料。(TGT-25557)
...
<table id="table_701D8D53D1DF4F28ADAC6EC221B0208A"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="filepath"> at.js </span> </p> </td> 
   <td colname="col2"> <p><b>2017 年 9 月 27 日</b> </p> <p> <span class="filepath">at.js</span> 版本 1.2.0 現在已包括多數錯誤修正的維護版本形式提供。如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="dita" scope="local">at.js 版本詳細資料</a>。 </p> <p> 
     <ul id="ul_D11024549C3643C7A756988087498D24"> 
      ...
      <li id="li_7D2A11B17672419583F9632CDA00D28F"> <p>已修正在具有全域 mbox 或其他地區 mbox 的重新導向產品建議中造成閃爍的問題。(TNT-27978) </p> </li> 
      ...
      <li id="li_B859939C1B5A4DF78CF8ADF236B88306"> <p>進行變更以確保 <span class="codeph">sdidParamExpiry</span> 已正確初始化，並傳遞至訪客 API。這是已新增至 <span class="codeph">at.js 1.1.0</span> 的迴歸。先前的 <span class="filepath">at.js</span> 版本不受影響。這只會影響使用重新導向產品建議和 A4T 的用戶端。(TNT-27791) </p> </li> 
...
<table id="table_1B43199F1AE64E69AE65313B23741444"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="filepath"> at.js </span> </p> </td> 
   <td colname="col2"> <p><b>2017 年 8 月 3 日</b> </p> <p> <span class="filepath">at.js</span> 1.1 版現已可用。如需詳細資訊，請參閱<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hant" format="dita" scope="local">下載 at.js</a>。 </p> <p><span class="filepath">at.js</span> 版本 1.1 中包括下列增強功能和修正: </p> <p> 
...
* 改善在使用者的 [!DNL Target] 工作階段建立或編輯活動或產品建議時逾時的工作流程。當使用者按一下[!UICONTROL Save]時，會顯示工作階段已到期訊息，但在重新登入之後，對話方塊會通知使用者已成功登入，而[!DNL Target]中的UI會維持在相同的頁面上，不會遺失資料。
...
<table id="table_8C4DB1B83B874E4C85CE9FF352E7B857"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p>推薦：條件和促銷活動 </p> </td> 
...
<table id="table_6167ECB7B44F40DCADF299F46F1F795C"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="filepath"> at.js </span> </p> </td> 
   <td colname="col2"> <p> <span class="filepath">at.js</span> 0.9.6 版現已可用。如需詳細資訊，請參閱<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hant" format="dita" scope="local">下載 at.js</a>。 </p> <p><span class="filepath">at.js</span> 版本 0.9.6 中包括下列增強功能和修正: </p> <p> 
     <ul id="ul_108DF85393614C69988E299485D338FD"> 
      <li id="li_4117C900982240B5AFFCFE1B2716A443"> <p>重新導向產品建議支援 A4T。下載和安裝 <span class="filepath">at.js</span> 版本 0.9.6 之後，您可以在使用 <span class="keyword">Adobe Analytics</span> 做為 <span class="keyword">Target</span> (A4T) 報表來源的活動中使用重新導向產品建議。除了 <span class="filepath">at.js</span> 版本 0.9.6，還有您的實作必須符合以便使用重新導向產品建議和 A4T 的其他基本需求。如需詳細資訊和須知的其他重要資訊，請參閱<a href="/help/main/c-integrating-target-with-mac/a4t/r-a4t-faq/a4t-faq-redirect-offers.md#concept_21BF213F10E1414A9DCD4A98AF207905" format="dita" scope="local">重新導向產品建議 - A4T 常見問答</a>。 </p> </li> 
      ...
      <li id="li_B11CE11D9A594CB1ABB85BD0D93C4A15"> <p>已新增 <span class="codeph">selectorsPollingTimeout</span> 設定。如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hant" format="dita" scope="local">targetGlobalSettings()</a>。 </p> </li> 
      <li id="li_D6F862099A374FE394F4DA3520A1BBF0"> <p>來自 <span class="codeph">getOffer()</span> 回應的格式已變更。如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hant" format="dita" scope="local">adobe.target.getOffer(options)</a>。 </p> </li> 
     ...
     </ul> </p> <p>如需此內容和先前版本 <span class="filepath">at.js</span> 的相關資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="dita" scope="local">at.js 版本詳細資料</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_4BA8DA701BC64427957355E144570EFE"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>目標分析 (A4T) </p> <p>重新導向產品建議 </p> </td> 
...
* 在[!UICONTROL Offer Level Report]報表的[!UICONTROL Automated Personalization]中，「檢視」現在是「造訪」。
...
>[!NOTE]
>
>具有FP-11577 （或更新版本）的[!DNL Adobe Experience Manager] 6.2現在支援[!DNL at.js]實作與其[!UICONTROL Adobe Target Cloud Services]整合。 如需詳細資訊，請參閱 *Adobe Experience Manager 6.2* 文件中的[功能套件](https://experienceleague.adobe.com/docs/?lang=zh-Hant)和[與 Adobe Target 整合](https://experienceleague.adobe.com/docs/?lang=zh-Hant)。
...
* 在清單中載入所有活動名稱之前，[!UICONTROL Create Activity]頁面上的[!UICONTROL Activities]按鈕現在處於作用中狀態。 這項改善可讓使用者更快速建立新活動，特別是當帳戶有許多已設定的活動時。(TGT-21470)
...
* 在所有使用[!UICONTROL Goals & Settings] (A4T)報表的活動的[!UICONTROL Analytics for Target]頁面上新增工具提示，通知使用者如果活動的頁面已載入at.js （0.9.1版或更新版本），則不需要追蹤伺服器。 (TGT-22607)
  ...
  <tr> 
   ...
   <td colname="col2"> <p>2017 年 1 月 18 日 </p> <p> <span class="codeph"> at.js</span> 版本 0.9.4 包含下列變更: </p> <p> 
     <ul id="ul_8F149C28E2D946B9888B4D2F45167C3C"> 
      <li id="li_93E866BBFE374E93BCDB65BCFAC33B62"> <p> mbox 名稱現在可以包含特殊字元，包括 &amp; 符號 (&amp;)。(TNT-26144) </p> <p>如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hant" format="dita" scope="local">at.js 設定</a>。 </p> </li> 
     ...
     </ul> </p> <p>如需每個 <span class="codeph">at.js</span> 版本中變更的詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="html" scope="external">at.js 版本詳細資料</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_4F7D4A71F5DF4E8782C7DBEEEF24AD04"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>內容/產品建議 </p> </td> 
   <td colname="col2"> <p>下列增強功能現在可供產品建議使用: </p> <p> 
     <ul id="ul_7D8E81443E0F48B6A0C1D1DF6F27D292"> 
      <li id="li_EA529EF4EBC2416E9D3B9E7251E7AAAB"> <p>「內容」頁面已重新命名為「產品建議」。此外，現在右側有兩個索引標籤，用來分隔代碼產品建議與影像產品建議。 </p> <p>在此版本以前，如果您在相同的資料夾中有程式碼和影像，Target 會將它們分割到兩個重複的資料夾。 </p> </li> 
      <li id="li_9574FA6BDCFB4BAB938273BF7F4B21C8"> <p>透過 Target Classic、Adobe Experience Manager (AEM)、Adobe Mobile Services (AMS) 和 API 建立的產品建議，現在會在 Target Standard/Premium 使用者介面中顯示。在 Target Classic 中建立的產品建議可在 Target Standard/Premium 中編輯。(TGT-15738) </p> <p> 在過去兩年中使用這些方法更新的產品建議將會顯示在 Target Standard/Premium 中 (即 2015 年 1 月之後版本)。 </p> </li> 
      <li id="li_CAD67C9EBB564525ABD2269D918275F8"> <p>您現在可以依來源和類型篩選產品建議。 </p> </li> 
     </ul> </p> <p>如需詳細資訊，請參閱<a href="/help/main/c-experiences/c-manage-content/manage-content.md#concept_17874A6FCBB743AA84C5988E8571CCF3" format="dita" scope="local">產品建議</a>。 </p> <p>已對地理位置鎖定目標進行下列增強: </p> <p> 
     <ul id="ul_DD8B50F980B8447A8C37EA96530D8949"> 
      <li id="li_348E04AB29B14E6F83E3A7E7BF7D75B8"> <p>您現在可以直接使用 <span class="codeph">profile.geolocation</span> 值做為產品建議、外掛程式等中的 Token。(TNT-25967) </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>下列報表增強功能現在可供 Target 報表使用。 </p> <p> 
     ...
     <ul id="ul_18B0636A41B94F9F903ABFE3E13285DA"> 
      <li id="li_2422075AA0A34F868809C5D580FC5D4B"> <p>產品建議層級 CSV 報表現在具有關於每個產品建議的其他詳細資料。(TGT-18995) </p> </li> 
      <li id="li_659D126E846348D4BE4544962F41539F"> <p>下載的產品建議層級 CSV 檔案，現在一律包括來自 <span class="wintitle">Automated Personalization</span> 報表的控制和鎖定區段的資料。(TGT-22000) </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>下列增強功能現在可供 Target 活動使用: </p> <p> 
     <ul id="ul_436556860E6C4AEEB35411A02E78A199"> 
      ...
      <li id="li_1489EA6C30C94B2AB394189E5FAFF6F6"> <p>Automated Personalization (AP) 活動中允許的匿名產品建議名稱長度已從 30 個增加至 250 字元。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>客群 </p> </td> 
...
* 進行數個改善以減少活動同步錯誤。
* 您不再可以使用 [!DNL Analytics for Target] (A4T) 來建立活動重新導向產品建議。
...
<table id="table_F8C01B2A9F07443490DB3025AC3AAC2A"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> 推薦：條件設定順序 </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> 推薦：插入外部促銷活動 </td> 
   <td colname="col2"> <p>新增提示的項目並控制其在您的推薦設計中的放置位置。 </p> <p>如需詳細資訊，請參閱<a href="/help/main/c-recommendations/t-create-recs-activity/adding-promotions.md#task_CC5BD28C364742218C1ACAF0D45E0E14" format="dita" scope="local">新增促銷活動</a>。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>2016 年 10 月 10 日 </p> <p> <span class="codeph">at.js</span> 0.9.3 版現已可用。 </p> <p> 
     <ul id="ul_E4D300700390433E9EF8D5C9D3AA7669"> 
      ...
      <li id="li_1130509832CE429DB6DE636404CC54E1"> <p>確保在動態遠端產品建議失敗 (例如，如果 URL 不正確並傳回 404 錯誤) 時會轉譯預設內容。 </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>2016 年 9 月 21 日 </p> <p> <span class="codeph">at.js</span> 0.9.2 版現已可用。 </p> <p> 
     <ul id="ul_0778A9049C9D48A7B6CB4B79A95F0F4C"> 
      ...
      <li id="li_3FB2914F8D2F4AFFAA9B4622E8CA1EFF"> <p>已修正關於可視化體驗撰寫器 (VEC) 產品建議建立的問題。在此版本之前，Target 會隱藏選取器，並且只有在所有選取器都符合時才取消隱藏。在 <span class="codeph">at.js</span> 0.9.2 中，Target 會在選取器符合時便加以取消隱藏。 </p> </li> 
     </ul> </p> <p>如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="dita" scope="local">at.js 版本詳細資料</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_AE048CB9EA1C4C7BBC2E9D90D26F7395"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p class="premium">Automated Personalization：產品建議效能報表 </p> </td> 
   <td colname="col2"> <p>下載具有所有 Automated Personalization 活動成功度量的產品建議效能報表。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_33B60910EAE24BAFA778F280F72FB683"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>2016 年 7 月 14 日 </p> <p> <span class="filepath">at.js</span> 0.9.1 版現已可用。 </p> <p>如需詳細資訊，請參閱 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hant" format="dita" scope="local">at.js 版本詳細資料</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_EBA34BD2F5C745DD9EC5231AD79F6C00"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> 多值推薦屬性 </td> 
   <td colname="col2"> <p>所有自訂 <span class="keyword">推薦</span> 屬性現在可以包含多個實體值。 </p> <p>如需詳細資訊，請參閱<a href="/help/main/c-recommendations/c-products/custom-entity-attributes.md#concept_E5CF39BCAC8140309A73828706288322" format="dita" scope="local">自訂實體屬性</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 動態/遠端產品建議支援 </td> 
   <td colname="col2"> <p>動態內容可以屬於 <span class="keyword">Target Standard/Premium</span> 中的任何表單型活動。動態內容儲存在 <span class="keyword">Target</span> 外部。 </p> <p>如需詳細資訊，請參閱<a href="/help/main/c-experiences/c-manage-content/about-remote-offers.md#concept_657016A0E6174C22B89036E9C8A0170F" format="dita" scope="local">建立遠端產品建議</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 複製對象和設定檔指令碼 </td> 
   <td colname="col2"> <p>您現在可以複製現有對象，然後可以編輯對象以建立類似的對象。 </p> <p>有關詳細資訊，請參閱<a href="/help/main/c-target/c-audiences/create-audience.md#task_E18BD77A9A8F4ED0AC50569F94556558" format="dita" scope="local">建立訪問群體</a>。 </p> <p>您也可以複製現有的設定檔指令碼。 </p> <p>有關詳細資訊，請參閱<a href="/help/main/c-target/c-visitor-profile/profile-parameters.md#concept_8C07AEAB0A144FECA8B4FEB091AED4D2" format="dita" scope="local">配置檔案指令碼屬性</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 使用類別來判斷元素選取器 </td> 
   <td colname="col2"> <p>在 Automated Personalization 和多變數測試活動中，元素選取器可以現在根據類別或 ID。在過去的版本中，此選項僅可供 A/B 測試活動使用。 </p> <p>有關詳細資訊，請參閱Visual Experience Composer <a href="/help/main/c-experiences/c-visual-experience-composer/vec-selectors.md#concept_4EB7663E255F439B8D24079D23479337" format="dita" scope="local">中使用的</a>元素選擇器。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> 推薦：內容相似度 </td> 
...
<table id="table_699755B33F8F48ECABB6FC7E78289A79"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在 Automated Personalization 活動中，會對每個要求評估輸入條件 (URL 鎖定目標、範本規則、客群目標)，以獲得更準確的產品建議傳送。 </p> <p>如需詳細資訊，請參閱<a href="/help/main/c-activities/t-automated-personalization/automated-personalization.md#task_8AAF837796D74CF893CA2F88BA1491C9" format="dita" scope="local"> Automated Personalization</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
| Automated Personalization 效能增強功能 | Automated Personalization 已經過重新設計，針對大量產品建議/位置組合的執行效能更佳。 |
...
<table id="table_8525ECC9B6D0435ABEF8C27F747B7A0C"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Target 介面現已支援 Internet Explorer 11。 </p> <p>請參閱<a href="https://experienceleague.adobe.com/docs/target-dev/developer/implementation/supported-browsers.html?lang=zh-Hant" format="dita" scope="local">支援的瀏覽器</a>。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p class="premium">推薦: </p> <p>在全域 Mbox 交付編碼中進行變更 </p> </td> 
...
* 修正使用表格工作流程時，在 `<div>` 元素中包住「非 HTML推薦」範本的問題。
* 修正造成暫停大量活動的衝突計算問題。
...
<table id="table_EE13D9B959DA4FB0AD6BC03FBF78AEF6"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p class="Premium">排除過去的購買項目 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p> 屬性加權 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p>設定處理摘要的時間 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p>使用摘要清單來設定摘要永不執行 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p>根據內容相似度設定新的條件類型 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p>增強功能：產業相關條件設定 </p> </td> 
   <td colname="col2"> <p>不相關的選項在設定期間已消除。過去在某些產業 (例如媒體)，某些設定選項並不一定相關。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p>新的立即可用條件 </p> </td> 
   <td colname="col2"> <p> 
...
<table id="table_907A952F54084C2A9C61F10E2B7A7BFF"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p class="Premium">表格型建議 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p class="Premium">推薦 </p> <p> 顯示設計中關鍵的相關資訊 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 如果最佳化目標是轉換量度，則產品建議詳細資料報表現在可在提升度和增量轉換中顯示熱門預測變數。此報表以前是只限收入型報表，因此此能力可確保沒有收入資料的活動仍可產生相關且可執行的前瞻分析。 </p> <p>請參閱<a href="/help/main/c-reports/personalization-reports/reports-ap.md" format="dita" scope="local"> Automated Personalization報告</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Adobe Campaign 電子郵件已與 Target Standard 整合 </td> 
   <td colname="col2"> <p> 以前，必須使用 Target Classic 才能使用 Adobe Campaign 來設定定位電子郵件促銷活動。隨著 Target Standard 推出兩個新功能 (表格型活動建立和重新導向產品建議)，現在可以使用 Target Standard 來搭配 Adobe Campaign 設定定位電子郵件促銷活動。</p></td> 
  </tr> 
  <tr> 
   <td colname="col1"> 表格型活動建立中的重新導向產品建議 </td> 
   <td colname="col2"> <p> Target Standard 表格型活動建立流程已新增支援 Target Classic 的重新導向產品建議功能。 </p> <p>請參閱<a href="/help/main/c-experiences/form-experience-composer.md#task_FAC842A6535045B68B4C1AD3E657E56E" format="dita" scope="local">表單式體驗撰寫器</a>。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_DF5B434D639345B4ACE2467B8966A908"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p>Automated Personalization </p> <p>產品建議層級定位 </p> </td> 
   <td colname="col2"> <p>可讓行銷人員套用定位規則至 Automated Personalization 中的產品建議。因此可以排除特定產品建議，不顯示給指定的人員群組。 </p> <p>請參閱<a href="/help/main/c-activities/t-automated-personalization/ap-target-offers.md#task_F207ED7A41B84FD39BB6FCBFABF4B23E" format="dita" scope="local"> Target AP 產品建議</a>。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_46FF5AF77D824ADC941B1E472234F05C"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>多頁活動可讓您利用每個頁面的特定設計，來建立多個頁面的歷程記錄。 </p> <p>例如，您可能要測試某個產品建議，在購買超過某個金額時提供免運費。您可能想要讓該產品建議出現在您的登陸頁面、類別頁面和某些產品頁面，但想要它有不同的大小且位在每個頁面類型上不同位置。您可以在首頁上顯示一個顯眼的產品建議，然後在其他相關頁面上以較小的產品建議強化該產品建議。 </p> <p>您也可以使用多頁活動來為您的桌面和非回應式行動網站定義不同的配置。 </p> <p>請參閱<a href="/help/main/c-experiences/c-visual-experience-composer/multipage-activity.md#concept_277E096063E14813AC5D8EDFA1D2ED48" format="dita" scope="local">多頁活動</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 表單型活動建立 </td> 
   <td colname="col2"> <p>不使用表單型體驗撰寫器建立活動。而是透過表單來選擇位置和產品建議。以此方式，即可以在電子郵件、行動應用程式、資訊站和使用可視化體驗撰寫器無法運作的其他位置中傳送 Target Standard 活動。 </p> <p>請參閱<a href="/help/main/c-experiences/form-experience-composer.md#task_FAC842A6535045B68B4C1AD3E657E56E" format="dita" scope="local">表單式體驗撰寫器</a>。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1" class="premium"> 推薦Premium：Facet 名稱搜尋 </td> 
...
<table id="table_C0B37E1730014ADA8C36310093F5C43A"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 一旦可預測模型傳遞了必要的品質條件並且為有效，即會將它視為就緒並用來計算產品建議決策的個人化分數。當模型就緒而 Target 可以開始傳送個人化的內容時，時鐘圖示會變更為勾選記號。由於提升僅預期在模型就緒之後，視覺指示可讓您設定正確的預期。使用可視化體驗撰寫器中的流量估算程式來取得模型何時會就緒的指導方針。 </p></td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>Premium 推薦：可視化體驗撰寫器中的瀏覽和導覽 </p> </td> 
  ...
  <tr> 
   <td colname="col1" class="premium"> <p>Premium 推薦：Facet 化的目錄搜尋 </p> </td> 
...
* 修正 Automated Personalization 活動中流量估算程式中使用了組合的數量，而非各位置產品建議總和的問題。
...
<table id="table_5A2F2058ACFB455E9F69484CA0C8D3DE"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在可視化體驗撰寫器內在視覺上預覽每個內容組合。 </p> <p>檢視Automated Personalization測試<a href="/help/main/c-activities/t-automated-personalization/ap-preview-experiences.md#task_21A700587E88453A9FC2210C0DE53A28" format="dita" scope="local">的</a>預覽體驗。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p class="premium">推薦：改善內容檢視 </p> </td> 
   <td colname="col2"> <p>在檢視或編輯集合時，您現在可以查看符合收集或排除的每個項目。 </p></td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p class="premium">推薦：改善搜尋結果 </p> </td> 
...
<table id="table_1558E5A5BAB64CC281C193F5A49E2ECE"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p class="premium">新活動類型：推薦 </p> </td> 
   <td colname="col2"> <p>推薦活動可依據先前的使用者行為，自動顯示可能使客戶感興趣的產品或內容。推薦可協助引導客戶至他們可能不知道的相關項目。 </p> <p>Recommendations 屬於 Target Premium 解決方案的一部分。Target Standard 若沒有 Target Premium 授權則不包含此功能。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_5D4C3C5695BA4A88BC65E2721CFB073A"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 全階乘多變數測試會將您內容區域中，所有可能的產品建議組合進行比較，協助您決定最佳的內容組合。多變數測試也會顯示哪些內容所在的領域對活動成功最有貢獻。通常在使用 A/B 測試決定最佳版面之後，會使用多變數測試來最佳化整個頁面。使用多變數測試，您可以最佳化頁面上的個別資產 (例如主影像、標題或促銷內容)。 </p> <p>如需簡介影片，請參閱 <a href="https://my.adobeconnect.com/p2k6u8iiu6l/" format="https" scope="external">https://my.adobeconnect.com/p2k6u8iiu6l/</a>。 </p> </td> 
  </tr> 
  <tr> 
...
| 功能 | 說明 |
|---|---|
| 重新導向產品建議 | 將體驗重新導向至不同的 URL，使得您可以對另一個頁面測試一個頁面。請參閱[建立重新導向產品建議](/help/main/c-experiences/c-visual-experience-composer/redirect-offer.md#task_9578678D42784F5EB9638F8AC8C911FA)。 |
...
| 功能/增強功能 | 說明 |
|---|---|
| 可插入和編輯 JavaScript | 在操作菜單中選擇&#x200B;**[!UICONTROL Edit HTML]**&#x200B;時，已添加在體驗編輯器中編輯和插入自定義JavaScript的功能。 |
...
| 功能/增強功能 | 說明 |
|---|---|
| 增強 HTML 產品建議與 [!DNL Target Classic] 的同步功能，提高字元上限。 | 提高在「內容」下建立之 HTML 產品建議的字元上限，以符合同步至 [!DNL Target Classic] 時的 HTML 產品建議 256 KB 限制。 |
| 改良在「體驗編輯器」中建立錯誤時的使用者經驗。 | 當頁面上的 DOM 結構改變中斷選擇器時，「體驗編輯器」會顯示訊息。 |

**修正**

* 修正在活動間進行瀏覽時，不會產生報告圖表的問題。
* 修正使用者在&#x200B;**[!UICONTROL Select Link]**&#x200B;頁面上按一下[!UICONTROL Goals and Settings]時，選取的連結未標示為選取的問題。

* 修正在[!UICONTROL Activity List]頁面上啟用新活動後，無法在[!UICONTROL Overview]中顯示的錯誤。

* 修正使用者無法選取點擊追蹤連結的問題。
* 修正在產品建議層級報表中顯示重複產品建議的問題。
* 修正無法插入 mbox 元素的問題。
* 修正連結點擊次數轉換無法作用的問題。
...
* 在某些叢集上，對於在 mbox 外部使用該產品建議的活動，編輯可重複使用的產品建議可能不會反映在客戶的網站上。
* 頁面區域中的交換影像如果不是由 mbox 控制，可能會造成 404 錯誤。
* 建立新對象或編輯和儲存現有對象時，在您重新整理畫面或搜尋對象之前，它不會顯示在對象清單中。
* 從 Target Standard 刪除 HTML 產品建議時，不會將它從 Target Advanced 刪除。
...
* 修正造成更新的影像產品建議內容無法在 Scene7 和 Target Advanced 中更新的問題。
* 修正套用搜尋篩選會清除「報表對象」中所選取對象的問題。
* 增強圖表，當測試上線少於兩天時，將預設為每小時的結果。
* 修正造成複製非同步的活動失敗的問題。
* 新增鍵盤輸入功能至位置的下拉式功能表。
* 改善刪除活動中使用的產品建議時顯示的錯誤訊息。
...
* 修正當交換影像時，在 Advanced 中會為單一產品建議建立多個產品建議的問題。
* 修正內容選擇器中無法對影像執行搜尋的問題。
* 修正依名稱或狀態排序時，活動清單排序反轉的瑕疵。
* 修正無法刪除活動中不再使用之匿名產品建議的問題。
* 修正在編輯活動時，共用卡片上顯示錯誤體驗名稱的瑕疵。
* 修正更新的影像產品建議未正確更新 Scene7 和 Target Advanced 中內容的瑕疵。
