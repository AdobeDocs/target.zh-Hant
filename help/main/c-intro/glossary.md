...
Automated Personalization (AP) 結合產品建議或訊息，並使用進階機器學習來根據訪客的個別客戶輪廓比對每位訪客的不同變數，以便個人化內容並促進轉換。
...
[!UICONTROL 增強體驗撰寫器]是[!UICONTROL 視覺化體驗撰寫器]的擴充功能，可協助您編輯iframe-busting網站或尚未包含[!DNL Target]實作的頁面的體驗。 如果您在[!UICONTROL 可視化體驗撰寫器]中開啟您的頁面遇到問題，請嘗試使用[!UICONTROL 增強體驗撰寫器]。
...
產品建議、影像、文字、按鈕、影片或將這些不同元素加以組合，放在頁面、整個網頁或一組頁面上，可能會形成購買漏斗或一些其他的頁面邏輯順序。 也可以是語音助理的回應、客戶服務指令碼，或甚至是飲料機的個人化口味。 您可以在 [!DNL Target] 活動中測試或個人化體驗。
...
非可視化體驗和產品建議建立介面，適合在可視化體驗撰寫器無法使用或不實用的情況下，用於建立可供 A/B 測試、體驗鎖定目標、Automated Personalization 和推薦活動使用的體驗。 例如，您可以使用表單式撰寫器，建立可在電子郵件、資訊站和語音助理中傳送的體驗和產品建議。

## G {#section_D7B3B62BD3A449EFAA11DF9704773B28}

**地理位置參數**

可讓您依訪客的地理位置鎖定活動和體驗。 又叫做地理定位參數。
...
頁面上的特定內容區域，經常包含單一 DOM 元素。 在 MVT 方法中，有時會將位置稱為&#x200B;*係數*。 全階乘多變數測試會比較您的位置中所有可能的產品建議組合。
...
多變數測試 (MVT) 會比較頁面上元素中產品建議的組合，以判斷哪個組合對特定客群執行時效果最佳，並識別哪個元素最能影響活動的成功
...
* 這是訪客自 [訪客輪廓期限](/help/main/c-target/c-visitor-profile/visitor-profile-lifetime.md) 過期後第一次造訪網站。
...
產品建議是指進行活動和測試時 mbox 中顯示的內容。 體驗決定當符合特殊條件時所顯示的產品建議。
...
將值傳遞給目的地頁面的 mbox，作為轉介 URL 參數和值。 使用這些來鎖定訪客來源，例如，
...
報表提供關於活動進度和結果的資訊，可協助您根據資料進行決策。 報告資料可協助您決定要在何時終止測試、顯示哪個體驗或產品建議是獲勝者，以及提供判斷後續動作所需的洞察。

## S {#section_9482B06EBB09486784492F838289E54D}

**指令碼輪廓參數**

類似 in-mbox 輪廓參數，但使用在 Target 後端 (而不是在瀏覽器) 中執行的 JavaScript 加以定義。
...
用來測量活動成功的參數。 成功量度包含重要的商務測量，可讓您判斷 Target 活動中指定體驗或產品建議的成功程度。 例如，您可以判斷新產品建議或將項目新增至購物車是否會提高每位訪客帶來的收入。 成功量度非常適合用於探索關於註冊、訂購或購買漏斗的問題，但也適合探索關於訪客或客戶參與度的問題。
...
在 Target Standard 中，這個 WYSIWYG 使用者介面可讓您輕鬆建立及測試網站內容中的個人化體驗和產品建議。 您可以拖放、切換及修改網頁 (或產品建議) 的版面與內容，藉此建立 Target 活動的體驗和產品建議。
