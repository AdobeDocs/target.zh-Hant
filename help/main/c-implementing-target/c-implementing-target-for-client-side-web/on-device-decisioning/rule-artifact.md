---
keywords: 實現；javascript library;js;atjs；設備上決策；設備上決策；規則項目
description: 瞭解設備上決策所使用的JSON規則項目。
title: JSON規則項目在設備上決策中是做什麼的？
feature: at.js
role: Developer
...
規則項目是您的JSON表示 [!DNL Adobe Target] 設備上決策活動。 規則對象由 [!DNL Target] 並傳播到Akamai CDN，以確保存在規則對象，該對象盡可能接近您的訪問者。 規則項目包含元資料，可確保活動的精確執行和交付，同時還允許通過事件跟蹤進行即時分析。
