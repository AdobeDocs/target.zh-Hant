...
>
>中的標籤 [Adobe Experience Platform](https://developer.adobe.com/target/implement/client-side/atjs/how-to-deployatjs/implement-target-using-adobe-launch/){target=_blank}是升級at.js的首選方法。 擴展開發人員不斷向其擴展添加新功能，並經常修復錯誤。 這些更新被打包到擴展的新版本中，並在 [!DNL Adobe Experience Platform] 編錄為升級。 有關詳細資訊，請參見 [擴展升級](https://experienceleague.adobe.com/docs/experience-platform/tags/ui/extensions/extension-upgrade.html){target=_blank} *標籤概述* 的子菜單。

## at.js 2.9.0 版 (2022 年 5 月 27 日)

* 已添加 [用戶代理客戶端提示](https://developer.adobe.com/target/implement/client-side/atjs/user-agent-and-client-hints/){target=_blank}支援。
...
* 使 `deviceIdLifetime` 設定可覆蓋通道 [目標全局設定](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}。 (TNT-36349)
...
* 支援在應用已交付的目標提供時，將內容安全策略設定為附加到頁DOM的SCRIPT和STYLE標籤上的不可用項。 客戶可以設定 `targetGlobalSettings.cspScriptNonce` 和 `targetGlobalSettings.cspStyleNonce` 以便at.js可以在應用的優惠中設定相應的指令碼和樣式標籤。 請參閱  [目標全局設定](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}以瞭解詳細資訊。
...
* at.js 2.2版還提供了名為 `serverState`。 當實現Target的混合整合時，可使用此設定來優化頁面效能。 混合整合意指您在用戶端上同時使用 at.js v2.2+ 和伺服器端的傳送 API 或 Target SDK 來傳送體驗。`serverState` 讓 at.js v2.2+ 能夠直接從伺服器端擷取並傳回至用戶端的內容套用體驗，做為所提供頁面的一部分。有關詳細資訊，請參閱中的「serverState」 [目標全局設定](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}。
...
* **Adobe 選擇加入支援**: Adobe 選擇加入是簡化 Adobe 解決方案與同意管理平台整合的方法。有關Adobe選擇加入的詳細資訊，請參見 [隱私和一般資料保護法規(GDPR)](https://developer.adobe.com/target/before-implement/privacy/cmp-privacy-and-general-data-protection-regulation/){target=_blank}。
...
* **傳送通知**: 可讓開發人員在透過體驗的程式碼 (而不是透過 `applyOffer()` 或 `applyOffers()`) 呈現體驗時傳送通知。
   ...
   有關詳細資訊，請參見 [adobe.target.sendNotifications（選項）](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/adobe-target-sendnotifications-atjs-21/){target=_blank}。
...
有關詳細資訊，請參見 [從at.js 1.x升級到at.js 2.x](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank和 [at.js函式](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/atjs-functions/){target=_blank}。

>[!NOTE]
>
>如果您需要Adobe選擇支援， [一般資料保護法規](https://developer.adobe.com/target/before-implement/privacy/cmp-privacy-and-general-data-protection-regulation/){target=_blank}(GDPR){target=_blank}，您當前必須使用at.js 1.7.0或at.js 2.1.0。

## at.js 版本 1.7.0 {#at-js-170}

at.js 1.7.0 提供 Adobe 選擇加入支援。「Adobe 選擇加入」是簡化 Adobe 解決方案與同意管理平台整合的方法。

有關Adobe選擇加入的詳細資訊，請參見 [隱私與一般資料保護法規](https://developer.adobe.com/target/before-implement/privacy/cmp-privacy-and-general-data-protection-regulation/){target=_blank}(GDPR){target=_blank}。

此版本也修正 Target 可能將重新導向 URL 參數覆寫為來自重新導向 URL 之參數的問題。

>[!NOTE]
>
>如果您需要AdobeGDPR的選擇加入支援，則當前必須使用at.js 1.7.0或2.1.0。<br>有關所有版本的清單，請參見 [at.js版本詳細資訊](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank}。
...
* 下列新事件可協助追蹤、除錯和自訂與 at.js 的互動:
   ...
   * CONTENT_RENDERING_REDIRECT
   ...
   如需詳細資訊，請參閱 [at.js自定義事件](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/atjs-custom-events/){target=_blank}。

* 您可以使用來自資料提供者的其他參數來擴大 at.js 要求。資料提供者應新增至 `window.targetGlobalSettings` 下的 `dataProviders key`。

   有關詳細資訊，請參見 [資料提供程式](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}。
...
* at.js 1.3.0 不再支援 Microsoft Internet Explorer 9 上的內容傳遞。如需支援瀏覽器的清單，請參閱 [支援的瀏覽器](https://developer.adobe.com/target/before-implement/supported-browsers/){target=_blank}。 今後，所有要求會透過 `XMLHttpRequest` 執行，具有 CORS 支援而不沒有 JSONP 要求。此變更大幅改善安全性。
...
* 以非同步方式載入 Target 資料庫。有關詳細資訊，請參見 [at.js常見問題](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-faq/target-atjs-faq/){target=_blank}。
...
* 已新增 `selectorsPollingTimeout` 設定。有關詳細資訊，請參見 [targetGlobalSettings()](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/targetglobalsettings/){target=_blank}。
* 來自 `getOffer()` () 的回應格式已變更。有關詳細資訊，請參見 [adobe.target.getOffer（選項）](https://developer.adobe.com/target/implement/client-side/atjs/atjs-functions/adobe-target-getoffer/){target=_blank}。
...
* mbox 名稱現在可以包含特殊字元，包括 &amp; 符號 (&amp;)。
   ...
   有關允許的特殊字元的清單，請參見 [at.js配置](https://developer.adobe.com/target/implement/client-side/atjs/how-to-deployatjs/implement-target-without-a-tag-manager/){target=_blank}。
