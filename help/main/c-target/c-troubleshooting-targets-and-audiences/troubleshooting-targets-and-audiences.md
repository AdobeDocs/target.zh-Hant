...
若要在建立活動時套用對象URL目標定位，請在&#x200B;**[!UICONTROL 體驗]**&#x200B;頁面上（三步驟引導式工作流程的步驟一），按一下&#x200B;**[!UICONTROL 設定]**&#x200B;圖示（![設定圖示](/help/main/assets/icons/Setting.svg)）、按一下&#x200B;**[!UICONTROL 頁面傳送]**，然後指定所要的URL。
...
透過[!UICONTROL 範本規則]新增的受眾URL目標定位和URL目標定位評估為URL目標定位（請參閱下列URL目標定位）。

### URL目標定位 {#url-targeting}

若要套用URL目標定位，在建立對象時，請按一下&#x200B;**[!UICONTROL 網站頁面]**&#x200B;並將其拖放至[!UICONTROL 建立對象]窗格，按一下&#x200B;**[!UICONTROL 網站頁面]**，從第一個下拉式清單中選取選項（[!UICONTROL 目前頁面]、[!UICONTROL 上一頁]或[!UICONTROL 登陸頁面]），從第二個下拉式清單中選取[!UICONTROL URL]，指定評估器，然後指定所要的URL。
...
[!DNL Target] 具有每個 mbox 呼叫 50 個獨特輪廓屬性的限制。 如果您需要傳遞超過50個設定檔屬性至[!DNL Target]，可以使用[!UICONTROL 設定檔更新] API方法來傳遞它們。 如需詳細資訊，請參閱 Adobe Target API 文件中的[設定檔更新](https://developers.adobetarget.com/api/#authentication-tokens)。
