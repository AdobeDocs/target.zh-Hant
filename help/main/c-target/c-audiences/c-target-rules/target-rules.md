...
您可以藉由新增更多條件，或在其他類別中建立額外條件，以進一步重新定義規則。 例如，您只能鎖定從Google存取您網站的舊金山Firefox使用者。 設定[!UICONTROL 地理]類別以鎖定來自舊金山、[!UICONTROL 瀏覽器]類別以鎖定使用Firefox的使用者，以及設定[!UICONTROL 流量來源]類別以鎖定來自[!UICONTROL Google]的使用者。 跨類別建立的規則會與AND運運算元結合。

若要建立包含跨類別之OR運算的複雜目標規則，請建立運算式目標。

您也可以鎖定自訂輪廓參數和 `user.` 參數。 新增對象時，請拖放&#x200B;**[!UICONTROL 訪客設定檔]**，然後選擇您要用來鎖定目標活動的引數。 如果未顯示所需的引數，表示引數並未由mbox引發。
