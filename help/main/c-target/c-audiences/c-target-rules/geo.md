...
大部分行動裝置使用者透過WiFi存取內容，這表示 [!DNL Target]的IP型地理定位與案頭一樣準確。 透過行動通訊基地台的連線可能較不準確，因為訪客的 IP 位址是以接收訊號的基地台為準。某些行動地理位置問題可使用 [地理位置API](https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API).
...
不， [!DNL Target] 僅在工作階段期間使用地理資料，則會捨棄資料。
