---
keywords: 相關性;類別相關性
description: 了解類別相關性，位於 [!DNL Adobe Target] 會自動擷取使用者造訪的類別，然後計算該類別的使用者相關性，以便進行定位和劃分。
title: 類別相關性為何？
feature: Audiences
...
中的類別相關性功能 [!DNL Adobe Target] 自動擷取使用者在您網站上瀏覽的類別，然後計算每個類別的使用者相關性，以便進行定位和劃分。 類別相關性有助於確保將內容鎖定在最有可能對該資訊採取行動的訪客。
...
您可在描述檔指令碼中使用 `user.categoryAffinities[]`，傳回訪客已填入的相似性陣列。如需詳細資訊，請參閱 [設定檔屬性中物件和方法底下的user.categoryAffinities](/help/main/c-target/c-visitor-profile/profile-parameters.md#objects).
...
>[!NOTE]
>
>若數個類別在單一mbox呼叫中傳遞，則 `categoryAffinities` 不保證。 任意類別首先被記錄，得分為10。
