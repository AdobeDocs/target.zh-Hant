...
由於個人資料指令碼於伺服器端執行，因此其無法直接讀取頁面。資料必須透過mbox要求或其他方式傳入 [將資料傳入Target的方法](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/methods/methods-to-get-data-into-target.html){target=_blank}. 將資料傳入 [!DNL Target] 後，個人資料指令碼即可讀取資料當作 mbox 參數或個人資料參數。
