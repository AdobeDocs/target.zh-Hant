...
description: 了解如何使用設定檔屬性來設定Adobe中的測試 [!DNL Target] 會比較多個活動，但不會讓相同的訪客參與每個活動。
...
您可以在 [!DNL Adobe Target] 設定測試來比較兩個或多個活動，但不讓相同的訪客參與每個活動。
...
建立設定檔屬性後，請設定第一個活動，要求使用者設定檔參數來定位所需的母體 `user.twogroups` 符合為GroupA指定的值。
