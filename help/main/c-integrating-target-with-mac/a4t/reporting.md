...
description: 了解如何將Analytics用於 [!DNL Target] (A4T)。 A4T可讓您存取 [!DNL Target] 使用Analytics量度和受眾區段的活動。
title: 如何在A4T中使用報表？
...
使用 [!DNL Adobe Analytics] 作為您的報表來源 [!DNL Adobe Target] (A4T)可讓您存取 [!DNL Analytics] 報表 [!DNL Target] 活動。

您可以同時在 [!DNL Analytics] 和 [!DNL Target].

針對使用的報表最佳實務 [!DNL Analytics] for [!DNL Target], [造訪Adobe Spark Page](https://spark.adobe.com/page/Lo3Spm4oBOvwF/).

## 總覽 {#section_035A62D65608423285D8A5A54731E2C5}

兩者 [!DNL Analytics] 和 [!DNL Target] 報表會測量加入者（進入測試的人員），而非網站的訪客。

每次訪客在頁面上看到活動內容時， [!DNL Target] 會對進行直接伺服器對伺服器呼叫 [!DNL Analytics]，包括訪客看到的活動和體驗。 [!DNL Target] 同時呼叫 [!DNL Analytics] 每次轉換時。 [!DNL Analytics] 將轉換新增為特定的 [!DNL Analytics] 事件「活動轉換」，此事件會與 [!DNL Analytics].

當 [!UICONTROL 選擇] 操作，然後排序 *加入者*，則報表中只會顯示在所選時段內收到加入者的體驗。

>[!NOTE]
>
>由提供支援的報表 [!DNL Target] 延遲四分鐘。 針對由A4T提供支援的活動，在 [!DNL Target] 和 [!DNL Analytics] 報表時，最多可能需要24小時的時間，活動才能依體驗劃分報表資料。 在這最初 24 小時收集的資料仍然正確，且會指派給正確的體驗。

## Analytics 中的報表 {#analytics}

在 [!DNL Analytics]，啟用A4T整合後，有數個維度和量度可供使用。

### 維度

* [!UICONTROL Analytics for Target]  — 透過整合傳入的父ID。 此維度的格式為 `Activity ID:Experience ID:3rd ID`. 以下維度是此維度的分類。
* [!UICONTROL Target 活動]
* [!UICONTROL 目標體驗]
...
* [!UICONTROL 活動曝光數]  — 符合 [!UICONTROL 加入者] 數字 [!DNL Target] 報表。
* [!UICONTROL 活動轉換]  — 符合 [!UICONTROL 自訂轉換] 數字 [!DNL Target] 報表。

在 [!DNL Analysis Workspace]，請使用 [!UICONTROL Analytics for Target] 面板來分析 [!DNL Target] 提升度和可信度的活動和體驗。 如需詳細資訊，請參閱 [Analytics for Target(A4T)面板](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/panels/a4t-panel.html??lang=zh-Hant) 在 *Analytics工具指南*.

>[!IMPORTANT]
>
>若您的 [!UICONTROL 目標活動] 報告 [!DNL Analytics] 會列出「未指定」，而非列出您的活動，因此您布建的帳戶需要更新。 請聯絡客戶服務以解決此問題。

如需詳細資訊和範例，請開啟 [Analytics與Target:分析最佳實務](https://spark.adobe.com/page/Lo3Spm4oBOvwF/) 教學課程，由Adobe Experience League提供。

## 報表於 [!DNL Target] {#section_C0D1F17F88374B6690BF904D7B83B42E}

當 [!DNL Analytics] 作為報表來源時， [!DNL Target] 顯示從 [!DNL Analytics]. 報告與其他報告有些不同 [!DNL Target] 報表：

* 此 [!UICONTROL 對象] 清單顯示可供您 [!DNL Analytics] 報表套裝。
* 此 [!UICONTROL 量度] 清單顯示每個可用度量 [!DNL Analytics].

   每個量度都可供使用，包括任何內建的自訂或計算量度 [!DNL Analytics].

   任何增加的數字在報表中都會顯示為正數，即使您不希望增加亦然。 例如，即便您想要較低的跳出率仍顯示了較高的跳出率，因為獲勝者會具有最高的提升度。根據報表進行決策時，請注意這些和類似的量度，以及是否您偏好減少或增加這些數字。

您可以在 [!DNL Target] 在活動開始後，或甚至在測試完成後。 您事先完全不需要知道要測量什麼。

按一下以檢視完整 [!DNL Analytics] 直接從活動報表頁面報告。

## 活動建立 {#section_311586E3FF5541E7A91D1A3CE5F9ACE3}

在建立活動期間，您必須在[!UICONTROL 「設定」]頁面上指定活動的目標。此目標會成為報表的預設量度，且一律列為量度選取器的第一個選項。您無法像一般 Target 活動一樣選取要報告的區段。測試 [!DNL Analytics] uses [!DNL Adobe Analytics] 區段而非 [!DNL Target] 對象。

## 執行Adobe Target(A4T)的Analytics離線計算 {#section_B34BD016C8274C97AC9564F426B9607E}

您可以使用 [!DNL Target] [完整可信度電腦](/help/main/assets/complete_confidence_calculator.xlsx) Excel檔案，但需執行以下步驟： [!DNL Analytics].
