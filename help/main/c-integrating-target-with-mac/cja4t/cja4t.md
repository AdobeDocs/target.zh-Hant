...
description: 使用 [!DNL Target] 報告位置 [!DNL Adobe Customer Journey Analytics] 建立活動依據 [!DNL Customer Journey Analytics] 轉換量度和受眾區段及使用 [!DNL Customer Journey Analytics] 報告以檢查結果。
...
# [!DNL Target] 報告位置 [!DNL Adobe Customer Journey Analytics]
...
在使用之前，請考慮以下資訊 [!DNL Customer Journey Analytics] 和 [!DNL Target] 整合：

>[!IMPORTANT]
>
>此整合與以下專案不同 [[!UICONTROL Adobe Analytics for Target]](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T)。 實作和支援的活動型別不同。 將這項整合用於您的之前，請務必仔細閱讀本文章 [!DNL Target] 活動。
...
* 建立 [!DNL Target] 活動 [!DNL Customer Journey Analytics] 報告，您必須擁有&quot;[!UICONTROL Approver]&quot;或&#39;[!UICONTROL Editor]「」角色位於 [!DNL Target].
...
* 任何現有 [!DNL Target] 活動繼續使用 [!DNL Target] 資料收集，不受啟用此整合的影響。
* 若要使用此整合，慣用的實作方法為 [[!DNL Adobe Experience Platform]](https://experienceleague.adobe.com/en/docs/experience-platform){target=_blank} and [!DNL Target] implemented through the [[!DNL Adobe Experience Platform Web SDK]](https://experienceleague.adobe.com/en/docs/target-dev/developer/client-side/aep-web-sdk){target=_blank}.

  如果您目前尚未實作 [!DNL Adobe Experience Platform Web SDK]，您也可以建立 [[!DNL Adobe Analytics]  來源連線](https://experienceleague.adobe.com/en/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/analytics)，將資料帶入[!DNL Adobe Experience Platform]。

  >[!NOTE]
  >
  >如果您使用 [!DNL Adobe Analytics] 來源連線中，您將會同時擁有兩個報告 [!DNL Adobe Analytics] 和 [!DNL Customer Journey Analytics]. 不過，由於這兩個解決方案之間的演演算法不同，結果不太可能相符。

* 任何關於時程的問題，請參閱 *[!DNL Adobe Customer Analytics]指南*&#x200B;中&#x200B;*常見問題*&#x200B;的[延遲的注意事項](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-faq#latency){target=_blank}。

## 支援的活動類型 {#supported-activities}

使用 [Adobe Experience Platform Web SDK](https://experienceleague.adobe.com/en/docs/target-dev/developer/client-side/aep-web-sdk){target=_blank} or the [at.js](https://experienceleague.adobe.com/en/docs/target-dev/developer/client-side/at-js-implementation/overview){target=_blank} JavaScript 程式庫時，可支援以下活動類型：
...
1. 在 [!DNL Customer Journey Analytics]，位於 **[!UICONTROL Connections]** 頁面，按一下 **[!UICONTROL Create a new connection]**.

   ![在中建立新的連線連結 [!DNL Customer Journey Analytics]](/help/main/c-integrating-target-with-mac/cja4t/assets/create-connection.png)

1. 使用正確的資訊設定您的[連線和資料設定](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-connections/overview){target=_blank}。
...
1. 設定您的事件資料集。
   ...
   如需詳細資訊，請參閱 [新增和設定資料集](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-connections/create-connection#add-dataset){target=_blank} 在 *建立連線* 在 *[!DNL Adobe Customer Journey Analytics]指南*.

1. 使用設定您的查詢資料集 [!UICONTROL Key] 欄位設為&quot;key&quot;，以及 [!UICONTROL Matching] 具有下列路徑的索引鍵欄位：
...
1. 按一下 **[!UICONTROL Add datasets]**，然後按一下 **[!UICONTROL Save]** 在下一個畫面中完成連線。
...
1. 設定資料檢視，並確認其指向到您上方所建立的連線。
   ...
   如需詳細資訊，請參閱 [建立或編輯資料檢視](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-dataviews/create-dataview){target=_blank} 在 *[!DNL Adobe Customer Journey Analytics]指南*.
...
1. 使用 [!DNL Target] 中的維度 [!UICONTROL Experimentation] 面板，設定下列內容標籤：

   * 的 [!UICONTROL Activity Name]，使用「實驗中的實驗」。
...
1. 完成設定任何其他欄位，然後按一下 **[!UICONTROL Save and continue]** 完成時。
