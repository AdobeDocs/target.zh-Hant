...
有關RTCDP的詳細資訊，請參閱[Real-time Customer Data Platform概述](https://experienceleague.adobe.com/docs/experience-platform/rtcdp/overview.html?lang=zh-Hant){target=_blank}。
...
* [!UICONTROL Target Edge Destinations Card]具有治理和策略實施
...
### [!DNL Adobe Experience Platform Web SDK]或[!DNL Experience Platform Server-Side API]實現
...
### [!UICONTROL at.js]和[!DNL Platform Web SDK]實現的混合
...
使用在 [!DNL Adobe Experience Platform] 中建立的[客群](/help/main/c-target/c-audiences/audiences.md)可提供更豐富的客戶資料，進而帶來更具影響力的個人化。 [Real-time Customer Data Platform](https://experienceleague.adobe.com/docs/experience-platform/rtcdp/overview.html?lang=zh-Hant){target=_blank}(RTCDP)構建在[!DNL Adobe Experience Platform]上，可幫助公司將來自多個企業源的已知和匿名資料匯集到一起。 此流程允許您建立客戶輪廓，並且可將這些輪廓用來即時提供跨所有管道和裝置的個人化客戶體驗。
...
* 給定優惠中的屬性必須來自相同的[!UICONTROL Experience Platform]沙盒。 （換句話說，提供不能包含來自不同[!UICONTROL Experience Platform]沙箱的屬性。）
* 指定選件內的屬性可能來自不同的來源，即[!DNL Target]設定檔和[!UICONTROL Experience Platform]設定檔。 （換句話說，無論屬性來自[!DNL Target]還是來自[!UICONTROL Experience Platform]配置檔案，都可以組合。）
* 在定義聘用時，您可以為[!UICONTROL Real-Time CDP Profile Attributes]分配預設值，以防該屬性沒有顯式值。 例如，如果同意書或控管政策封鎖個人化服務內使用的屬性，則可以改用預設值。
* [!DNL Target]僅支援要在優惠中使用的[!DNL Adobe Experience Platform]配置檔案屬性的「字串」資料類型。 尚不支援「映射」和「陣列」類型屬性。

### JSON 使用案例範例

身為線上行銷人員，您希望 AEP/整合輪廓與以下人員共用屬性值[!DNL Target]以提供即時個人化。通過使用[!UICONTROL Real-Time CDP Profile Attributes]，可以使用令牌替換在[!UICONTROL Experience Platform]提供中顯示[!DNL Target]屬性的值。 例如，您可以根據客戶最喜歡使用的顏色 `${aep.profile.favoriteColor}`，或者他們使用權杖的忠誠度等級和忠誠度點值 `${aep.loyalty.tier}` 和 `${aep.loyalty.points}` 來進行個人化。

若要建立 JSON 產品建議以與 [!DNL Target] 共用 AEP/Unified 輪廓屬性。

1. 在[建立JSON優惠](/help/main/c-experiences/c-manage-content/create-json-offer.md)時，從&#x200B;**[!UICONTROL Select a source]**&#x200B;清單中選擇&#x200B;**[!UICONTROL Adobe Experience Platform]**。
1. 從&#x200B;**[!UICONTROL Select a profile sandbox name]**&#x200B;清單中，選擇所需的沙盒。
1. 從&#x200B;**[!UICONTROL Select a profile attribute]**&#x200B;清單中，選擇所需的屬性。
1. （可選）從&#x200B;**[!UICONTROL Insert a default value]**&#x200B;清單中選擇所需值。
1. 按一下 **[!UICONTROL Add]**。
...
有關詳細資訊，請參閱[平台Tutorials](https://experienceleague.adobe.com/docs/platform-learn/tutorials/experience-cloud/next-hit-personalization.html?lang=zh-Hant){target=_blank}指南中的&#x200B;*Real-Time CDP和Adobe Target的下一次個性化*。
...
[[!DNL Adobe] 宣佈使用 [!DNL Adobe Target] 和 [!DNL Real-time Customer Data Platform]](https://blog.adobe.com/en/publish/2021/10/05/adobe-announces-same-page-enhanced-personalization-with-adobe-target-real-time-customer-data-platform){target=_blank}實現同頁增強個性化
