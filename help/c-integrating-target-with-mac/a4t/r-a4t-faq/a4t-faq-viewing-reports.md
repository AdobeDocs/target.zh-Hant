...
本主題包含有關使用[!DNL Analytics]作為[!DNL Target](A4T)報告來源時，檢視報告時常被問及的問題的解答。
...
如需詳細資訊和範例，請開啟[Analytics &amp; Target:Adobe Experience League提供的分析最佳實務教學課程](https://spark.adobe.com/page/Lo3Spm4oBOvwF/)。
...
因此，當您分段某個活動以呈現在點擊中時，您將會取得該活動&#x200B;*以及*&#x200B;中其他任何持續存在於該點擊的體驗。
...
* 當使用者符合活動資格且從[!DNL [!DNL Target]]傳回內容時，會觸發上述所有度量。 這不一定表示使用者已經看到選件。若活動體驗位於下半部，且使用者並未向下捲動頁面，則雖然 [!DNL Target] 已提供選件，但使用者並未看到選件。
...
如果選取[!DNL Analytics]量度作為活動的轉換量度，「活動轉換」將會是空的。
...
若該活動已進行分類，而您仍在報表中看到「未指定」列，請確定報表未使用非[!DNL Target]度量來顯示資料。 除非報表使用[!DNL Target]特定度量，否則「未指定」列將包含與[!DNL Target]無關聯之呼叫的事件。 該列不包含任何[!DNL Target]相關資訊（例如訪客／瀏覽／印象）。
...
您可能會在到期期間後看到[!DNL Target]變數傳送至[!DNL Analytics]，因為到期時間為90天，但前提是該使用者從未看到其他啟用A4T的[!DNL Target]活動。 如果使用者在第 45 天回到網站並看到另一個活動，則整個 A4T eVar 值的計數器會重設為 90 天。這表示從第 1 天開始的第一個行銷活動，現在最多會持續到 45 + 90 = 135 天。如果使用者不斷回訪，您可能會看到許多較舊活動傳送至報表中[!DNL Analytics]的量度。 當使用者刪除 Cookie 且沒有回到網站時，該活動中的數字會下降，但您仍然會看到。
...
[!UICONTROL 獨特訪客]量度在[!DNL Analytics]和[!DNL Target]中只會在您查看比實際測試更短的時段時有所不同。 如果尚未達到您的樣本大小，測試就不太可靠。如需詳細資訊，請參閱 [Evan Miller 網站](https://www.evanmiller.org/index.html)上的 [如何不執行 A/B 測試](https://www.evanmiller.org/how-not-to-run-an-ab-test.html)。

[!UICONTROL 獨特訪客]度量顯示在指定時段內曾瀏覽過網站的測試訪客人數。 這些人仍然為測試的一部分，應該計算在內。如果只想看一週內接觸到的人數，您可以建立已有活動曝光的訪客區段，並套用至報表。

您可以縮短[!DNL Target]變數持續至作業的時間；不過，若測試中轉換事件在相同作業階段中發生的可能性不大，這通常會有問題。
...
當訪客符合A4T活動的資格、檢視內容或轉換時，[!DNL Target]會傳送事件資料至[!DNL Analytics]，這可讓[!DNL Analytics]將頁面上發生的轉換事件和其他點按流事件歸因於相關的[!DNL Target]活動和體驗。

在檢視[!DNL Analytics]報表時，請記住以下幾點：

* 一般而言，您的報告視窗應從活動的開始日期開始，這是最佳實務。
* 如果轉換發生在報表視窗外，轉換將不會顯示在[!DNL Analytics]中。
