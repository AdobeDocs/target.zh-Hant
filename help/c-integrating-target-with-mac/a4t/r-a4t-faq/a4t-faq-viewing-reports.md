...
區段也可以直接在自由表格中分層，但請注意，您必須將區段覆蓋整個表格，以便在A4T面板中保留提升度和可信度計算。 目前，面板不支援欄層級區段。
...
對於使用[!DNL Analytics]作為報告來源(A4T)的活動，目標量度一律會使用「[!UICONTROL 在活動中增加計數與保留使用者」和「在每次曝光時]」設定。 ][!UICONTROL 這是&#x200B;*not*&#x200B;可配置的。
