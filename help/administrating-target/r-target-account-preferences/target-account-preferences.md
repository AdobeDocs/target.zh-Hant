...
| 載入混合內容 | 使用增強的Experience Composer(EEC)開啓網站時，啓用混合內容。啓用此選項可避免透過Target Proxy伺服器載入靜態資源的額外負荷。<br>例如，如果您的內容安全政策(CSP)標題允許載入混合內容，而不使用EEC啓用的Proxy伺服器，這個選項就很有用。<br>如果您的HTTP網站面臨增加EEC的載入時間，這個選項也會有所幫助，因為JavaScript、影像等等都需要較長的時間透過proxy載入。 |
