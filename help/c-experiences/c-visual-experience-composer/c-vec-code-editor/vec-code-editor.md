...
description: 了解如何使用Adobe [!DNL Target] 中的「修改」面板來檢視頁面修改，以及新增其他修改（CSS選取器、Mbox和自訂程式碼）。
title: 我可以對頁面進行哪些修改？
...
[!DNL Adobe Target]中「修改」頁面的相關資訊，可讓您檢視頁面的修改內容並新增其他修改內容（CSS選取器、Mbox及自訂程式碼）。

「修改」頁面會顯示在可視化體驗撰寫器(VEC)中對您的頁面所做的所有變更，並可讓您按一下頁面上的每個元素並選取動作[來進行其他變更。 ](/help/c-experiences/c-visual-experience-composer/viztarget-options.md#reference_3BD1BEEAFA584A749ED2D08F14732E81)您所做的每項變更都顯示為[!UICONTROL 修改]清單中的個別動作或元素。您也可以新增修改，包括以下修改類型: CSS 選取器、Mbox。和自訂程式碼。
