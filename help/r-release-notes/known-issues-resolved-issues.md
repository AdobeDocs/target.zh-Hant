...
* 使用目標度量影響A4T活動的 [!DNL Analytics] 問題，造成A4T報表顯示未預期的流量分割或人為誇大的轉換。

   此問題會影響下列條件下的A4T報表：

   * 活動是在2020年9月15日到11月5日（太平洋標準時間上午4點）之間建立或保存的，並且
   * 活動已選取 [!DNL Analytics] 量度作為目標量度。
...
* 使用目標度量影響A4T活動的問 [!DNL Target] 題，造成A4T報表報告低或無轉換。
   ...
   * 活動已選取 [!DNL Target] 一個量度作為目標量度，且
   * 當訪客點擊活動的目標事件(例如 [!UICONTROL Clicked an Element])時，也會有與轉換事件相符的非A4T活動優先順序較低。 如果非A4T活動的設定量度與A4T活動相同，或是設定了「任何mbox」量度，則可能會發生此情況。
