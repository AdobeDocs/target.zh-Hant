...
| 日期 | 主題 | 變更 |
| --- | --- | --- |
| 17 年 11 月 | [活動設定 - A4T 常見問題集](/help/c-integrating-target-with-mac/a4t/r-a4t-faq/a4t-faq-activity-setup.md) | 新增下列常見問答：<ul><li>訪客是否可在使用A4T的Auto-Target活動中，在不同瀏覽中切換目標體驗和控制體驗？</li><li>是否建議我們使用自訂的Auto Target和A4T模型，並搭配90(Control)/10(Targeted)分割，直到模型建立為止？</li></ul> |
...
|  | [targetGlobalSettings()](/help/c-implementing-target/c-implementing-target-for-client-side-web/targetgobalsettings.md) | 已新增文字，指出如果您在傳回的 `serverState` 內容 `<script>` 中使用和使用標籤，請確定您的HTML內容使用 `<\/script>` 而非標籤 `</script>`。 |
...
|  | [Target 版本說明 (發行前)](/help/r-release-notes/target-release-notes.md) | 已更新 [!DNL Target Standard/Premium] 20.5.1版的附註（2020年6月17日），以包含有關A4T支援的資訊 [!DNL Analysis Workspace]。 |
...
| 21 年 5 月 | [允許列出目標邊緣節點](/help/c-implementing-target/c-considerations-before-you-implement-target/allowlist-edges.md) | 已新 `mboxedge30.tt.omtrdc.net` 增至清單。 |
