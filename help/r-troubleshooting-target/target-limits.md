---
description: 有關 Adobe Target 中會影響活動和其他元素之字元限制與其他限制(選件大小、對象、設定檔、值、參數等) 的資訊。
keywords: 字元限制;mbox 參數;批次傳送 api;描述檔參數;限制;內建描述檔;上限;限制;限制;字元;最佳作法;orderid;orderTotal;mbox3rdPartyID;類別;categoryID
seo-description: 有關 Adobe Target 中會影響活動和其他元素之字元限制與其他限制(選件大小、對象、設定檔、值、參數等) 的資訊。
seo-title: 限制
solution: Target
title: 限制
topic: Standard
uuid: 603fb800-a26c-43ec-b2d9-ef7a8ed8721e
translation-type: tm+mt
source-git-commit: 8038b4758e9ce72ff8e43e7427c273aeed734288

---


# 限制{#limits}

有關 Adobe Target 中會影響活動和其他元素之字元限制與其他限制(選件大小、對象、設定檔、值、參數等) 的資訊。

>[!NOTE]
>
>除非另行指定，否則下列限制應視為「硬體」限制。
>
>當接近或超出指定的限制時，效能可能會變慢。介面載入時間緩慢，也可能是由複雜的活動造成，例如一個活動中有許多對象、目標和體驗。
>
>複雜度高的活動應由 Adobe 諮詢服務審閱過，並先在有限制的環境中測試過，再正式上線。

## 活動

**建議的限制**：10,000個作用中的即時活動。

**建議的限制**：10,000個作用中儲存(且未結束)的縮寫。

## 活動名稱

**上限**: 250 個字元。

## 對象名稱

**上限**: 256 個字元。

超過 256 個字元的值會遭截斷。

## 對象

**限制**：每個mbox、量度或體驗的50個觀眾。

## categoryId 參數

**上限**: 250 個字元。

## 客戶屬性名稱

**上限**: 128 個字元。

## 客戶屬性別名 ID

**上限** 50 個字元。

## 實體自訂屬性

**上限**: 長度上限取決於語言。

* 15,000 個字元 (單值、單位元和雙位元語言)
* 500 個值，每個值 100 個字元 (多值)

單一值實體自訂屬性的長度上限為15,000個字元(單位元組和雙位元組UTF-8編碼語言)或10,000個字元(對於三位元組UTF-8編碼語言，例如中文、日文和韓文)。

多值實體自訂屬性不得包含超過 500 個值。每個個別值限定為 100 個字元以內。所有值的總字元數必須符合單值實體自訂屬性之長度上限的限制 (如上所述)。

## entityID 參數

**上限**: 1,000 個字元。

## excludedIds {#excludedid}

**上限**: 5 KB (適用於 POST 要求)。2,083 個字元減去 GET 要求的 URL 的長度。

若為 GET 要求，雖然後端的上限為 5 KB，由於事實上 Microsoft Internet Explorer 將 URL 限制為 2,083 個字元，實際上限為 2,083 個字元減去目前的 URL 長度。

## 體驗名稱

**上限**: 20 個字元。

## 每個活動的體驗

**限制**：每個體驗定位(XT)、A/B測試、多變數測試(MVT)和自動Target活動的2,000個體驗。

每次自動個人化(AP)活動的30，000個體驗。

## In-mbox 設定檔屬性值

**上限**: 256 個字元。

超過限制的值會遭截斷。

## mbox 要求中的 In-mbox 設定檔

**上限**: 50 個設定檔。

第 50 個之後的描述檔全部會被忽略。

## In-mbox 設定檔名稱

**上限**: 128 個字元。

## mbox 名稱

**上限**: 250 個字元。

## mbox 參數

**上限**: 下列上限適用於 mbox 參數:

* mbox 參數: 每個 mbox 500 個參數。
* 設定檔參數: 500 個參數。
* 每個 mbox 的設定檔參數:
* 其他參數 (URL、轉介 URL 等): 各參數類型每個 mbox 50 個。

針對 Target 資料庫中記錄的參數，上述上限適用於標準 mbox 要求。除非要求因網頁瀏覽器限制而縮短，否則適用上述限制。

如果您在 Mobile Services SDK 使用[批次傳送 API](https://developers.adobetarget.com/api/#server-side-batch-delivery)，50 個 mbox 參數、50 個描述檔參數和 50 個其他參數類型限制，即為 API 本身限制。無法使用批次傳送 API，傳送超過上述數量限制的要求。如果要求超過上述限制，API 會傳回下列錯誤訊息:「mboxParameters 數量不得超過 100。」

## mbox 要求 URL

**上限**: 2,083 個字元。

此上限是因為 Microsoft Internet Explorer URL 長度限制。

## mbox3rdPartyId 參數

**上限**: 60 個字元。

## 選件名稱

**上限**: 250 個字元。

## 選件大小

**上限**: 下列大小上限適用於選件:

* HTML 選件為 256 KB.
* 來自 UI 的視覺選件為 64 KB.
* 來自 API 512 KB.

如果使用全域 mbox，此上限適用於針對頁面傳回的整組內容。限制選件大小可改進頁面載入時間。如果超過限制，會顯示下列訊息: 

「體驗的內容過大，無法傳送。請修改體驗，降低影響的頁面代碼。」

## 選件

**建議的限制**：50，000個優惠。

## orderId 參數

**建議的限制**：120個字元。

## orderTotal 參數

**建議的限制**：120個字元。

## productPurchasedId 參數

**上限**: 每個逗號分隔值最多 47 個字元。

系統會截斷超出的部分。

## 設定檔指令碼

**建議的限制**：300個作用中設定檔指令碼。

**建議的限制**：每個描述檔指令碼的回圈上限500,000個。

## 屬性

**建議的限制**：5,000個屬性。

## 報告觀眾/區段

**限制**：50報告每個活動的觀眾/區段。

## 可重複使用的對象/帳戶

**建議的限制**：75個受眾。

如果太多，介面會發生 JavaScript 逾時。

## Target UI 中的指令碼設定檔輸入方塊

**建議的限制**：2,000個字元。

依編碼字串的大小而定，可能比原始字串大許多。如果字串太長，還未送到 Adobe Target 就會失敗。

## 指令碼設定檔名稱

**上限**: 50 個字元。

## 指令碼設定檔值

**上限**: 2,048 個字元。

基於效能考量，我們建議傳回的值不要超過 256 個字元。

針對字串傳回值，如果傳回值的大小超過 2,048 個字元，系統會停用指令碼。

針對陣列傳回值，如果陣列的連接值大小超過 2,048 個字元，系統會停用指令碼。

## 成功量度

**限制**：每個活動200個。

## Target 條件

**建議的限制**：1,000個值。

這是指定位文字區域中以行分隔之值的數目。例如，在郵遞區號定位中輸入 1,000 個郵遞區號。

## 定位規則

**建議的限制**：每個定位規則有2,500個唯一值。

**建議的限制**：目標規則中每個對象的30，000個獨特值。

**建議的限制**：每個活動的100，000個唯一定位規則值。
