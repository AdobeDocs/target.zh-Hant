---
keywords: 實施；javascript library;js;atjs；裝置上決策；裝置上決策；規則工件
description: 瞭解裝置上決策所使用的JSON規則工件。
title: JSON規則對象在裝置上決策中有何作用？
feature: at.js
role: Developer
...
規則物件是您[!DNL Adobe Target]裝置上決策活動的JSON表示法。 規則物件由[!DNL Target]產生並傳播至Akamai CDN，以確保有盡可能接近訪客的規則物件。 規則物件包含中繼資料，可確保活動的精確執行和傳遞，同時允許透過事件追蹤進行即時分析。
