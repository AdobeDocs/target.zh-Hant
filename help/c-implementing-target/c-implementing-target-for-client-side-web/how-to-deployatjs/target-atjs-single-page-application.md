...
description: 了解如何將Adobe [!DNL Target] at.js 2.x to implement [!DNL Target] 用於單頁應用程式(SPA)。
title: 我可以為單頁應用程式(SPA)實作 [!DNL Target] 嗎？
feature: 實作伺服器端
role: Developer
...
1. 安裝 at.js 2.x。
   ...
   透過位於[!UICONTROL 管理>實作]的Adobe Target UI下載at.js 2.x。 也可透過[!DNL Adobe Experience Platform]中的標籤來部署at.js 2.x。 不過，[!DNL Adobe Target]擴充功能目前不是最新且受支援。
...
雖然這些範例使用JavaScript程式碼，但如果您使用標籤管理程式，例如[Adobe Experience Platform](/help/c-implementing-target/c-implementing-target-for-client-side-web/how-to-deployatjs/cmp-implementing-target-using-adobe-launch.md)中的標籤，這一切都可簡化。

如果您按照上述步驟進行，您應該能擁有適用於 SPA 的健全 A4T 解決方案。

## 實作最佳實務 {#bp}

at.js 2.x API可讓您以許多方式自訂[!DNL Target]實作，但在此程式中請務必遵循正確的作業順序。
...
| 步驟 | Action | 詳細資料 |
| --- | --- | --- |
| 3 | 載入VisitorAPI JS | 此程式庫負責指派ECID給訪客。 此ID稍後會由網頁上的其他[!DNL Adobe]解決方案使用。 |
| 2 | 載入at.js 2.x | at.js 2.x會載入您用來實作[!DNL Target]要求和檢視的所有必要API。 |
| 3 | 執行[!DNL Target]請求 | 如果您有資料層，建議您在執行[!DNL Target]請求之前，先載入傳送至[!DNL Target]所需的重要資料。 這可讓您使用`targetPageParams`來傳送您要用於鎖定目標的任何資料。 您必須確定在此API呼叫中要求execute > pageLoad以及預先擷取>檢視。 若您已設定`pageLoadEnabled`和`viewsEnabled`，則步驟2會自動同時執行> pageLoad和prefetch >檢視；否則，您需要使用`getOffers()` API來提出此要求。 |
| 4 | 呼叫 `triggerView()` | 因為您在步驟3中啟動的[!DNL Target]要求可能會同時傳回頁面載入執行和檢視的體驗，請確定在傳回[!DNL Target]要求並完成將選件套用至快取後，才呼叫`triggerView()`。 每個檢視只需執行此步驟一次。 |
| 5 | 呼叫[!DNL Analytics]頁面檢視信標 | 此信標會將與步驟3和4相關聯的SDID傳送至[!DNL Analytics]以進行資料匯整。 |
| 6 | 呼叫其他`triggerView({"page": false})` | 這是SPA架構的選用步驟，可能在不發生檢視變更的情況下，重新呈現頁面上的特定元件。 在這種情況下，請務必叫用此API，以確保在SPA架構重新轉譯元件後重新套用[!DNL Target]體驗。 您可以執行此步驟的次數，多多少次，以確保[!DNL Target]體驗持續存在於SPA檢視中。 |

### SPA檢視變更的作業順序（未重新載入完整頁面）

| 步驟 | 動作 | 詳細資料 |
| --- | --- | --- |
| 3 | 呼叫 `visitor.resetState()` | 此API可確保在新檢視載入時，為其重新產生SDID。 |
| 2 | 呼叫`getOffers()` API以更新快取 | 如果此檢視變更可能會讓目前的訪客符合更多[!DNL Target]活動的資格，或使其不符合活動的資格，則此為選用步驟。 此時，您也可以選擇傳送其他資料至[!DNL Target]以啟用進一步的定位功能。 |
| 3 | 呼叫 `triggerView()` | 如果您已執行步驟2，則必須等待[!DNL Target]要求並套用選件至快取，才執行此步驟。 每個檢視只需執行此步驟一次。 |
| 4 | 呼叫 `triggerView()` | 如果您尚未執行步驟2，則可在您完成步驟1後立即執行此步驟。 如果您已執行步驟2和步驟3，則應略過此步驟。 每個檢視只需執行此步驟一次。 |
| 5 | 呼叫[!DNL Analytics]頁面檢視信標 | 此信標會將與步驟2、3和4相關聯的SDID傳送至[!DNL Analytics]以進行資料匯整。 |
| 6 | 呼叫其他`triggerView({"page": false})` | 這是SPA架構的選用步驟，可能在不發生檢視變更的情況下，重新呈現頁面上的特定元件。 在這種情況下，請務必叫用此API，以確保在SPA架構重新轉譯元件後重新套用[!DNL Target]體驗。 您可以執行此步驟的次數，多多少次，以確保[!DNL Target]體驗持續存在於SPA檢視中。 |
...
### 在SPA ![教學課程徽章](/help/assets/tutorial.png)中實作at.js 2.x

>[!VIDEO](https://video.tv.adobe.com/v/26248)

如需詳細資訊，請參閱在單頁應用程式(SPA)](https://helpx.adobe.com/target/kt/using/atjs2-single-page-application-technical-video-implement.html)中實作Adobe Target的at.js 2.x 。[
