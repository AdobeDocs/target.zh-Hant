...
**控制誰能看到新功能。** Target發行給特定使用者、組織、區域或自訂屬性。 從小組開始，驗證體驗，然後展開 — 全部從主控台進行，沒有程式碼變更。

**執行A/B實驗。**&#x200B;為您的對象和測量中表現較佳的不同區段提供不同的變體。 在完整版發行之前，使用結果來做出明智的產品決策。

**降低發行風險。**&#x200B;將大型變更分成較小且受控制的轉出。 如果出現錯誤或效能問題，請僅停用受影響的功能 — 應用程式的其餘部分會維持穩定。

**跨應用程式協調。**&#x200B;功能群組可讓您一起管理多個功能標幟，並在多個應用程式間共用相同的轉出對象。

**匯出您的資料。**&#x200B;將標幟資料匯出至您偏好的報告環境，以便與其他Adobe資料一起進行分析和測量。
